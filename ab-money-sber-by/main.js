...
function fetchCard(jsonInfo, baseurl){
	...
	var re = new RegExp('GetRest[\\s\\S]*?(\\d+' + cardnum + ')', 'i');
	...
	re = new RegExp('GetRest[\\s\\S]*?\\d+' + cardnum + '[\\s\\S]*?(\\d+)\\)', 'i');
	var curr = getParam(html, null, null, re, null);
	
	getParam(card, result, 'cardnum', null, replaceTagsAndSpaces);
	getParam(card, result, '__tariff', null, replaceTagsAndSpaces);
	...
	getParam(html, result, 'balance', /RST="([\s\S]*?)"/i, replaceTagsAndSpaces, parseBalance);
