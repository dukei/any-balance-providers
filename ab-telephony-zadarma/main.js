...
function main(){
	...
	var result = {success: true};
        ...
        if(AnyBalance.isAvailable('phone0', 'phone0till', 'phone1', 'phone1till', 'phone2', 'phone2till')){
		info = AnyBalance.requestGet(baseurl + 'dirnum/');
                var numbers = sumParam(info, null, null, /<h2[^>]*>(?:Бесплатный|Безкоштовний|Free)(?:[^<](?!c донабором|з донабором|with dtmf))*<\/h2>[\s\S]*?(?:<h2|Номер действителен|Номер діє|The number works)/ig);
                for(var i=0; i<Math.min(numbers.length, 3); ++i){
                    getParam(numbers[i], result, 'phone' + i, /(?:Вам подключен прямой номер|Вам надано прямий номер|Your connected number|Вам підключено прямий номер):[\s\S]*?<b[^>]*>([\s\S]*?)<\/b>/i, replaceTagsAndSpaces, html_entity_decode);
