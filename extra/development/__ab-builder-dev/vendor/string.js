/**!
 * Ќабор полезных методов, дополн€ющих объект String
 * 
...
(function () {
	...
	String.HTML_ENTITY_TABLE = {
		...
		sect   : 0x00A7,  //"\xc2\xa7" [І] section sign
		...
		laquo  : 0x00AB,  //"\xc2\xab" [Ђ] left-pointing double angle quotation mark = left pointing guillemet
		not    : 0x00AC,  //"\xc2\xac" [ђ] not sign
		shy    : 0x00AD,  //"\xc2\xad" [ ] soft hyphen = discretionary hyphen
		reg    : 0x00AE,  //"\xc2\xae" [Ѓ] registered sign = registered trade mark sign
		...
		para   : 0x00B6,  //"\xc2\xb6" [ґ] pilcrow sign = paragraph sign
		middot : 0x00B7,  //"\xc2\xb7" [Ј] middle dot = Georgian comma = Greek middle dot
		...
		raquo  : 0x00BB,  //"\xc2\xbb" [ї] right-pointing double angle quotation mark = right pointing guillemet
		...
		ndash   : 0x2013,  //"\xe2\x80\x93" [Ц] en dash
		mdash   : 0x2014,  //"\xe2\x80\x94" [Ч] em dash
		lsquo   : 0x2018,  //"\xe2\x80\x98" [С] left single quotation mark
		rsquo   : 0x2019,  //"\xe2\x80\x99" [Т] right single quotation mark (and apostrophe!)
		sbquo   : 0x201A,  //"\xe2\x80\x9a" [В] single low-9 quotation mark
		ldquo   : 0x201C,  //"\xe2\x80\x9c" [У] left double quotation mark
		rdquo   : 0x201D,  //"\xe2\x80\x9d" [Ф] right double quotation mark
		bdquo   : 0x201E,  //"\xe2\x80\x9e" [Д] double low-9 quotation mark
		dagger  : 0x2020,  //"\xe2\x80\xa0" [Ж] dagger
		Dagger  : 0x2021,  //"\xe2\x80\xa1" [З] double dagger
		bull    : 0x2022,  //"\xe2\x80\xa2" [Х] bullet = black small circle
		hellip  : 0x2026,  //"\xe2\x80\xa6" [Е] horizontal ellipsis = three dot leader
		permil  : 0x2030,  //"\xe2\x80\xb0" [Й] per mille sign
		prime   : 0x2032,  //"\xe2\x80\xb2" [?] prime = minutes = feet (дл€ обозначени€ минут и футов)
		Prime   : 0x2033,  //"\xe2\x80\xb3" [?] double prime = seconds = inches (дл€ обозначени€ секунд и дюймов).
		lsaquo  : 0x2039,  //"\xe2\x80\xb9" [Л] single left-pointing angle quotation mark
		rsaquo  : 0x203A,  //"\xe2\x80\xba" [Ы] single right-pointing angle quotation mark
		oline   : 0x203E,  //"\xe2\x80\xbe" [?] overline = spacing overscore
		frasl   : 0x2044,  //"\xe2\x81\x84" [?] fraction slash
		euro    : 0x20AC,  //"\xe2\x82\xac" [И] euro sign
		...
		trade   : 0x2122,  //"\xe2\x84\xa2" [Щ] trade mark sign
	...
	/**
	 * ¬ырезает все теги из HTML.
	 * √ораздо умнее, чем str.replace(/<[^>]*>/, '')
	 *  орректно обрабатываетс€ "гр€зный" html: 
	 *		* в тексте могут втречатьс€ конструкции типа "a < b > c"
	 *		* в значени€х атрибутов тэгов могут встречатьс€ символы < >
	 * —тараетс€ сохранить форматирование
	 * 
	...
	/**
	 * –еализаци€ метода String.match() с учЄтом рекурсии
	 * 
	 * @link https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/match
	 * @param {RegExp} pattern	–егул€рное выражение должно быть сконструировано с группой с двум€ или более альтернативами. ‘ормат:
	 *							`(OpenPattern)|(ClosePattern)`
	 *							`(OpenPattern)|(ClosePattern)|OuterPattern|InnerPattern|...`
	 *							»наче возвращаетс€ строка (если совпадение найдено) или Null (если совпадение не найдено).
	 * @param {Object} options	ќбъект с опци€ми. ѕример: `{open: 1, close: 2, parts: true}`
	 *							'open' и 'close' -- номера карманов дл€ открывающего и закрывающего совпадений дл€ регул€рного выражени€ 
	 *							'parts' -- если true, то вместо строк возвращает объекты 
	 * @returns {Array|String|Null}	≈сли передан флаг 'g', то возвращаетс€ массив, иначе строка
	 *								≈сли в опци€х указан флаг 'parts' то вместо вместо строк возвращает объекты в формате:
	 *								```
	 *								{
