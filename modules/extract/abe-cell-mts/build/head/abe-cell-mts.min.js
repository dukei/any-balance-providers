...
function fetchAccumulatedCounters(a,b){AnyBalance.trace("Parsing accumulated counters...");b.remainders||(b.remainders={});checkIHError(a,b);getParam(a,b.remainders,"remainders.tourist",/Счетчик Туристическая СИМ-карта от МТС\.[\s\S]*?<td[^>]+class="counter-value"[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces,parseBalance)}
...
b.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+a);return b}
function getLKJson2(a){a=AnyBalance.requestGet("https://oauth.mts.ru/webapi-1.4/customers/@me",addHeaders({"X-Requested-With":"XMLHttpRequest",Authorization:"Bearer sso_1.0_websso_cookie"}));var b=getParam(a,null,null,/^\{[\s\S]*?\}$/i,null,getJson);if(!b){AnyBalance.trace(a);if(b=getParam(a,null,null,/<div[^>]+class="red-status"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(b);throw new AnyBalance.Error("Не удалось найти Json с описанием пользователя, сайт изменен?");
}a={};for(var c=0;c<b.genericRelations.length;c++){var d=b.genericRelations[c];isset(d.target.balance)&&(a.balance=getParam(d.target.balance+"",null,null,null,null,parseBalanceRound));isset(d.target.productResources)&&isset(d.target.productResources[0])&&(a.tariff=d.target.productResources[0].product.name["ru-RU"]);isset(d.target.address)&&(a.phone=d.target.address,a.phone_formatted=replaceAll(a.phone,replaceNumber));isset(d.target.displayNameNat)&&(a.fio=d.target.displayNameNat)}return a}
function isAnotherNumber(){var a=AnyBalance.getPreferences();return a.phone&&a.phone!=a.login}
