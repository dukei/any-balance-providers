...
function getLKJson2(a){a=AnyBalance.requestGet("https://login.mts.ru/api/profile",addHeaders({Referer:g_baseurl+"/"}));var b=getJson(a),c;(""+b["mobile:phone"]).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4");c=""+b["mobile:phone"];b["mobile:balance"]||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+a);c||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+a);return b}
