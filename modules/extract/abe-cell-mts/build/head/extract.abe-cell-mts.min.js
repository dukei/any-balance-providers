...
b=callNewLKApiToken("creditLimit");b=callNewLKApiResultAlt(b);getParam(b.currentCreditLimitValue,a.remainders,"remainders.credit")}function mainLK(a,b){AnyBalance.trace("Мы в личном кабинете...");processInfoLK(a,b);try{processCountersLK(b)}catch(c){AnyBalance.trace("Ошибка получения остатков из кабинета. Попробуйте ещё раз позднее. Ошибка: "+c.message),b.were_errors=!0}}function sleep(a){AnyBalance.trace("Sleeping "+a+" ms");AnyBalance.sleep(a)}
function parseBalanceRound(a){a=parseBalance(a);isset(a)&&(a=Math.round(100*a)/100);return a}
function loginWithoutPassword(){AnyBalance.trace("Entering lk without password...");var a=AnyBalance.getPreferences();checkEmpty(!a.password||/^\w{6,26}$/.test(a.password),"Желаемый пароль должен содержать от 6 до 26 символов");a.password&&checkEmpty(/^[a-zA-Z0-9]+$/.test(a.password)&&/[a-z]/.test(a.password)&&/[A-Z]/.test(a.password)&&/[0-9]/.test(a.password),"Желаемый пароль должен состоять только из латинских букв и цифр и должен содержать хотя бы одну строчную букву, заглавную букву и цифру");var b=
