...
function loginWithPassword(){AnyBalance.trace("Пробуем войти в личный кабинет...");var a=AnyBalance.getPreferences();b=loadProtectedPage("https://lk.mts.ru/",g_headers);if(/Мой МТС/i.test(b))AnyBalance.trace("Сессия сохранена. Входим автоматически...");else if(AnyBalance.trace("Сессия новая. Будем логиниться заново..."),clearAllCookiesExceptProtection(),/qrator/i.test(b))AnyBalance.trace("Обнаружена защита. Используем новый вход"),b=enterLKNew({login:a.login,password:a.password,baseurl:"https://lk.mts.ru",
