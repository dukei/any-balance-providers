...
{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7","Accept-Language":"ru-RU,ru;q=0.9","Cache-Control":"max-age=0",Connection:"keep-alive","Upgrade-Insecure-Requests":"1","User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36","If-Modified-Since":null},replaceNumber=[replaceTagsAndSpaces,/\D/g,"",/.*(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7 $1 $2-$3-$4"];
function getParamByName(a,b){return getParam(a,null,null,new RegExp("name=[\"']"+b+'["\'][^>]*value=["\']([^"\']+)"',"i"))}function isInOrdinary(a){return/<div[^>]+class="main-menu"/i.test(a)}function uniq_fast(a){for(var b={},c=[],d=a.length,e=0,f=0;f<d;f++){var g=a[f];1!==b[g]&&(b[g]=1,c[e++]=g)}return c}
...
json={}}return{}}
function getLKJsonNew(a){AnyBalance.getPreferences();a=AnyBalance.requestGet("https://lk.mts.ru/api/login/user-info",addHeaders({Accept:"application/json, text/plain, */*","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"}));var b={},c=getJson(a).userProfile;b.fio=c.displayName;b.phone_formatted=(""+c.login).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4");b.phone=""+c.login;b.license=""+c.accountNumber;b.region=""+c.regionTitle;b.tariff=getParam(""+c.tariff,null,null,/(?:[\s\S]*?-\s)([\s\S]*?)(?:\s\([\s\S]*?)?$/i,
replaceTagsAndSpaces);if(!b.balance){c=callNewLKApiToken("accountInfo/mscpBalance");var d=callNewLKApiResult(c);b.balance=getParam(Math.round(100*d.amount)/100)}b.balance||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+JSON.stringify(d));b.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+a);return b}
function getLKJson2(a){a=AnyBalance.requestGet("https://lk.mts.ru/api/login/profile",addHeaders({Referer:g_baseurl+"/","X-Login":"7"+fetchNumber()}));var b=getJson(a),c={},d=b.account;c.fio=b.name;c.phone_formatted=(""+d.phone).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4");c.phone=""+d.phone;c.license=""+d.number;c.region=getParam(""+d.tariff.system,null,null,/([\s\S]*?)(?:\s-\s[\s\S]*?)?$/i,replaceTagsAndSpaces);c.tariff=""+d.tariff.name;if(!c.balance){b=callNewLKApiToken("accountInfo/mscpBalance");
var e=callNewLKApiResult(b);c.balance=getParam(Math.round(100*e.amount)/100)}c.balance||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+JSON.stringify(e));c.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+a);return c}
function getLKJson1(a){a=AnyBalance.requestGet("https://login.mts.ru/profile/header?ref=https%3A//lk.mts.ru/&scheme=https&style=2015v2",addHeaders({Referer:g_baseurl+"/","X-Login":"7"+fetchNumber()}));var b={};b.fio=getElement(a,/<div[^>]+b-header_lk__name[^>]*>/i,replaceTagsAndSpaces);b.phone_formatted=getElement(a,/<div[^>]+b-header_lk__phone[^>]*>/i,replaceTagsAndSpaces);b.phone=replaceAll(b.phone_formatted,[/\+7/,"",/\D/g,""]);b.balance=getElement(a,/<div[^>]+b-header_balance[^>]*>/i,replaceTagsAndSpaces,
parseBalance);b.balance||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+a);b.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+a);return b}function isAnotherNumber(){var a=AnyBalance.getPreferences();return a.phone&&a.phone!=a.login}
...
function loginWithPassword(){AnyBalance.trace("Пробуем войти в личный кабинет...");var a=AnyBalance.getPreferences();g_savedData||(g_savedData=new SavedData("mts",a.login));g_savedData.restoreCookies();AnyBalance.requestGet("https://profile.mts.ru/account",addHeaders({referer:"https://lk.mts.ru/"}));var b=AnyBalance.requestGet("https://login.mts.ru/amserver/oauth2/sso/validate",addHeaders({Referer:"https://profile.mts.ru/"}));AnyBalance.trace(b);/"isSessionCookieValid":\s*?true/i.test(b)?AnyBalance.trace("Сессия сохранена. Входим автоматически..."):
...
function processInfoLK(a,b){if(AnyBalance.isAvailable("balance","tariff","info.licschet","info.region","info.phone","info.fio"))try{var c=getLKJson(a,!0);c&&AnyBalance.trace(JSON.stringify(c));getParam(c.balance,b,"balance");getParam(c.tariff,b,"tariff");AnyBalance.isAvailable("info")&&(b.info||(b.info={}),getParam(c.license,b.info,"info.licschet"),getParam(c.region,b.info,"info.region"),getParam(c.phone,b.info,"info.phone",null,replaceNumber),getParam(c.fio,b.info,"info.fio"))}catch(d){AnyBalance.trace("Не удалось получить данные о пользователе, скорее всего, виджет временно недоступен... "+
...
function callNewLKApiToken(a,b){var c="GET",d="";if(b){var e=getCsrfToken();d=JSON.stringify(b);c="POST";b=addHeaders({Accept:"application/json, text/plain, */*","Content-Type":"application/json","X-Csrf-Token":e,"X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"})}else b=addHeaders({Accept:"application/json, text/plain, */*","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"});c=AnyBalance.requestPost("https://lk.mts.ru/api/"+a,d,b,{HTTP_METHOD:c});getJson(c);d=getJson(c);if("string"!==
typeof d)throw AnyBalance.trace(c),new AnyBalance.Error("Неверный ответ API "+a);return{token:d,verb:"api/"+a}}
...
function callNewLKApiData(a,b){var c="GET",d="";if(b){var e=getCsrfToken();d=JSON.stringify(b);c="POST";b=addHeaders({Accept:"application/json, text/plain, */*","Content-Type":"application/json","X-Csrf-Token":e,"X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"})}else b=addHeaders({Accept:"application/json, text/plain, */*","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"});c=AnyBalance.requestPost("https://lk.mts.ru/api/"+a,d,b,{HTTP_METHOD:c});c=getJson(c);AnyBalance.trace("Получен результат api/"+
...
"). Это посуточный дополнительный пакет. Пропускаем...");else if(d.isUnlimited)AnyBalance.trace("Найден счетчик "+d.name+" ("+d.packageType+"). Это безлимитный пакет. Пропускаем...");else if(AnyBalance.trace("Найден счетчик "+d.name+" ("+d.packageType+")"),"Calling"===d.packageType){AnyBalance.trace("Это минуты");var e=1;"Second"===d.unitType&&(e=60);getParam(d.deadlineDate,a.remainders,"remainders.min_till",null,null,parseDateISO);getParam(d.usedAmount/e,a.remainders,"remainders.min_local");getParam(d.totalAmount/
e,a.remainders,"remainders.min_total");getParam((d.totalAmount-d.usedAmount)/e,a.remainders,"remainders.min_left")}else"Messaging"===d.packageType?(AnyBalance.trace("Это сообщения"),getParam(d.deadlineDate,a.remainders,"remainders.sms_till",null,null,parseDateISO),getParam(d.usedAmount,a.remainders,"remainders.sms_used"),getParam(d.totalAmount,a.remainders,"remainders.sms_total"),getParam(d.totalAmount-d.usedAmount,a.remainders,"remainders.sms_left")):"Internet"===d.packageType?(AnyBalance.trace("Это трафик"),
getParam(d.deadlineDate,a.remainders,"remainders.traffic_left_till",null,null,parseDateISO),getParam(d.usedAmount+" "+d.unitType,a.remainders,"remainders.traffic_used_mb",null,null,parseTraffic),getParam(d.totalAmount+" "+d.unitType,a.remainders,"remainders.traffic_total_mb",null,null,parseTraffic),getParam(d.usedByAcceptors+" "+d.unitType,a.remainders,"remainders.traffic_used_by_acceptors_mb",null,null,parseTraffic),getParam(d.totalAmount-d.usedAmount+" "+d.unitType,a.remainders,"remainders.traffic_left_mb",
null,null,parseTraffic)):AnyBalance.trace("Неизвестный счетчик: "+JSON.stringify(d))}isAvailable("remainders.cashback")&&(b=callNewLKApiToken("cashback/account"),b=callNewLKApiResult(b),getParam(b.balance,a.remainders,"remainders.cashback"));if(isAvailable(["remainders.statuslock","remainders.services","remainders.services_free","remainders.services_paid","remainders.services_abon"]))for(b=callNewLKApiToken("services/list/active"),b=callNewLKApiResult(b),getParam({Unblocked:"Номер не блокирован",
OnlyInboundCalls:"Частичная блокировка",BlockDueToInsufficiencyOfMoney:"Номер заблокирован",Blocked:"Номер заблокирован"}[b.accountBlockStatus]||b.accountBlockStatus,a.remainders,"remainders.statuslock"),getParam(b.services.length,a.remainders,"remainders.services"),getParam(0,a.remainders,"remainders.services_free"),getParam(0,a.remainders,"remainders.services_paid"),getParam(0,a.remainders,"remainders.services_abon"),c=0;c<b.services.length;++c)d=b.services[c],!1===d.isSubscriptionFee||0===d.primarySubscriptionFee.value?
(AnyBalance.trace("Найдена бесплатная услуга "+d.name),sumParam(1,a.remainders,"remainders.services_free",null,null,parseBalanceSilent,aggregate_sum)):!0===d.isSubscriptionFee&&0!==d.primarySubscriptionFee.value?(AnyBalance.trace("Найдена платная услуга "+d.name+": "+d.primarySubscriptionFee.quotaPeriodicity+" "+d.primarySubscriptionFee.value+" ₽"),e=new Date,sumParam(1,a.remainders,"remainders.services_paid",null,null,parseBalanceSilent,aggregate_sum),e="Month"!==d.primarySubscriptionFee.unitOfMeasure?
(new Date(e.getFullYear(),e.getMonth()+1,0)).getDate():1,sumParam(d.primarySubscriptionFee.value*e,a.remainders,"remainders.services_abon",null,null,parseBalanceSilent,aggregate_sum)):AnyBalance.trace("Неизвестный тип услуги: "+JSON.stringify(d));isAvailable("remainders.credit")&&(b=callNewLKApiToken("creditLimit"),b=callNewLKApiResult(b),getParam(b.currentCreditLimitValue,a.remainders,"remainders.credit"));isAvailable(["remainders.cashback_mts","remainders.cashback_mts_pending","remainders.cashback_mts_burning",
"remainders.cashback_mts_burning_date","remainders.premium_state"])&&processCashbackLK(a);isAvailable("messages.total_msg","messages.unread_msg")&&processMessagesLK(a);isAvailable(["expenses.usedinthismonth","expenses.usedinprevmonth","expenses.abonservice","expenses.refill"])&&processExpensesLK(a);isAvailable(["payments.sum","payments.date"])&&processTransactionsLK(a)}
function getCsrfToken(){AnyBalance.requestGet("https://lk.mts.ru/api/login/refreshCsrfToken",addHeaders({Accept:"application/json, text/plain, */*","X-Login":fetchNumber()}));var a=AnyBalance.getLastResponseHeader("X-Refresh-Csrf-Token");a||AnyBalance.trace("Не удалось получить csrfToken");return a}
function processCashbackLK(a){a.remainders||(a.remainders={});var b=AnyBalance.requestGet("https://login.mts.ru/amserver/rest/v1/cashback",g_headers);b=getJson(b);AnyBalance.trace(JSON.stringify(b));getParam(b.cashBackValue,a.remainders,"remainders.cashback_mts");getParam(b.pendingCashBackValue,a.remainders,"remainders.cashback_mts_pending");b.expiringCashBack&&(getParam(b.expiringCashBack.expiringCashBackValue,a.remainders,"remainders.cashback_mts_burning"),getParam(b.expiringCashBack.expiringCashBackDate,
a.remainders,"remainders.cashback_mts_burning_date",null,null,parseDateISO));getParam({PREMIUM:"Активна",NO_PREMIUM:"Не активна"}[b.premiumStatus]||b.premiumStatus,a.remainders,"remainders.premium_state")}
...
function processExpensesLK(a){a.expenses||(a.expenses={});var b=new Date,c=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-01T00:00:00+03:00";b=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-"+n2(b.getDate())+"T23:59:59+03:00";try{html=AnyBalance.requestPost("https://federation.mts.ru/graphql",JSON.stringify({operationName:"GetExpensesInfoBaseQueryInput",variables:{pageIndex:1,filter:{dateFrom:c,dateTo:b,showFree:!1,tabType:null,categoryId:null,textSearch:"",transactionsDirection:null,cashbackType:"ALL"}},query:"query GetExpensesInfoBaseQueryInput($pageIndex: Int!, $filter: TransactionsFilterInput!) {\n  transactionsByFilter(input: {pageIndex: $pageIndex, filter: $filter}) {\n    totalInfo {\n      currency\n      incomeAmount\n      outcomeAmount\n      cashback {\n        incomeAmount\n        outcomeAmount\n        __typename\n      }\n      period {\n        dateFrom\n        dateTo\n        __typename\n      }\n      __typename\n    }\n    categories {\n      alias\n      value\n      name\n      color\n      amount\n      percentage\n      __typename\n    }\n    totalPages\n    transactions {\n      name\n      type\n      subtitle\n      dateTime\n      unitValue\n      value\n      direction\n      amount\n      icon\n      darkIcon\n      description\n      productType\n      globalCode\n      actionLabel\n      hints\n      cashbackInfo {\n        amount\n        direction\n        __typename\n      }\n      chargePeriod {\n        dateFrom\n        dateTo\n        __typename\n      }\n      __typename\n    }\n    __typename\n  }\n}"}),
addHeaders({"Content-Type":"application/json",Referer:g_baseurl+"/","X-Client-Id":"LK","X-Login":fetchNumber()}));var d=getJson(html)}catch(f){AnyBalance.trace("Ошибка получения данных по расходам за этот месяц: "+f.message);return}AnyBalance.trace(JSON.stringify(d));c=d.data;if(c.transactionsByFilter&&c.transactionsByFilter.categories&&0<c.transactionsByFilter.categories.length)for(b=0;c.transactionsByFilter.categories&&b<c.transactionsByFilter.categories.length;++b){var e=c.transactionsByFilter.categories[b];
AnyBalance.trace('Найдена категория "'+e.name+'": '+e.amount+" ₽");/Абонентская плата/i.test(e.name)&&sumParam(e.amount,a.expenses,"expenses.abonservice",null,null,parseBalanceSilent,aggregate_sum)}else AnyBalance.trace("Не удалось получить данные по категориям расходов за этот месяц");c.transactionsByFilter&&c.transactionsByFilter.totalInfo?(getParam(c.transactionsByFilter.totalInfo.incomeAmount,a.expenses,"expenses.refill"),getParam(c.transactionsByFilter.totalInfo.outcomeAmount,a.expenses,"expenses.usedinthismonth")):
AnyBalance.trace("Не удалось получить сводные данные по расходам за этот месяц");if(isAvailable("expenses.usedinprevmonth")){b=new Date;b=new Date(b.getFullYear(),b.getMonth(),0);c=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-01T00:00:00+03:00";b=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-"+n2(b.getDate())+"T23:59:59+03:00";try{html=AnyBalance.requestPost("https://federation.mts.ru/graphql",JSON.stringify({operationName:"GetExpensesInfoBaseQueryInput",variables:{pageIndex:1,filter:{dateFrom:c,dateTo:b,
showFree:!1,tabType:null,categoryId:null,textSearch:"",transactionsDirection:null,cashbackType:"ALL"}},query:"query GetExpensesInfoBaseQueryInput($pageIndex: Int!, $filter: TransactionsFilterInput!) {\n  transactionsByFilter(input: {pageIndex: $pageIndex, filter: $filter}) {\n    totalInfo {\n      currency\n      incomeAmount\n      outcomeAmount\n      cashback {\n        incomeAmount\n        outcomeAmount\n        __typename\n      }\n      period {\n        dateFrom\n        dateTo\n        __typename\n      }\n      __typename\n    }\n    categories {\n      alias\n      value\n      name\n      color\n      amount\n      percentage\n      __typename\n    }\n    totalPages\n    transactions {\n      name\n      type\n      subtitle\n      dateTime\n      unitValue\n      value\n      direction\n      amount\n      icon\n      darkIcon\n      description\n      productType\n      globalCode\n      actionLabel\n      hints\n      cashbackInfo {\n        amount\n        direction\n        __typename\n      }\n      chargePeriod {\n        dateFrom\n        dateTo\n        __typename\n      }\n      __typename\n    }\n    __typename\n  }\n}"}),
addHeaders({"Content-Type":"application/json",Referer:g_baseurl+"/","X-Client-Id":"LK","X-Login":fetchNumber()})),d=getJson(html)}catch(f){AnyBalance.trace("Ошибка получения данных по расходам за прошлый месяц: "+f.message);return}AnyBalance.trace(JSON.stringify(d));c=d.data;c.transactionsByFilter&&c.transactionsByFilter.totalInfo?(getParam(c.transactionsByFilter.totalInfo.incomeAmount,a.expenses,"expenses.refillinprevmonth"),getParam(c.transactionsByFilter.totalInfo.outcomeAmount,a.expenses,"expenses.usedinprevmonth")):
...
function mainLK(a,b){var c=AnyBalance.getPreferences();AnyBalance.trace("Мы в личном кабинете...");if(isAnotherNumber()){AnyBalance.trace("Пробуем переключиться на номер "+c.phone);a=AnyBalance.requestGet("https://lk.mts.ru/api/login/profile",addHeaders({Referer:g_baseurl+"/","X-Login":"7"+c.phone}));var d=getJson(a).slaves;if(1>d.length)AnyBalance.trace("У логина "+c.login+" нет ни одного прикрепленного номера");else{for(a=0;a<d.length;++a){var e=d[a];AnyBalance.trace("Найден номер "+e.phone);if(!f&&
(!c.phone||endsWith(e.phone,c.phone))){AnyBalance.trace("Выбран номер "+e.phone);var f=e}}if(!f)throw new AnyBalance.Error("Не удалось переключиться на номер "+c.phone+". Вероятно, он не принадлежит логину "+c.login);d=f.id;f=getCsrfToken();a=AnyBalance.requestPost("https://auth-lk.ssl.mts.ru/account/switch/"+d,null,addHeaders({"Content-Type":"application/json",Referer:g_baseurl+"/","X-Csrf-Token":f,"X-Login":"7"+c.login}));for(d=5;!/"loginStatus":\s*?"Success"/i.test(a)&&0<d--;){a=AnyBalance.requestGet("https://lk.mts.ru/api/login/user-info",
...
}}processInfoLK(a,b);try{processCountersLK(b)}catch(g){AnyBalance.trace("Ошибка получения остатков из кабинета. Попробуйте ещё раз позднее. Ошибка: "+g.message),b.were_errors=!0}}function sleep(a){AnyBalance.trace("Sleeping "+a+" ms");AnyBalance.sleep(a)}function parseBalanceRound(a){a=parseBalance(a);isset(a)&&(a=Math.round(100*a)/100);return a}
...
function processPayments(a,b){try{AnyBalance.trace("Получаем платежи...");var c=AnyBalance.requestGet(a+"payment-full-history.aspx",g_headers);checkIHError(c,!0);var d=getElement(c,/<form[^>]+aspnetForm[^>]*>/i),e=new Date,f=new Date(e.getFullYear()-1,e.getMonth(),e.getDate()),g=createFormParams(d,function(l,p,m,n){return/from$/i.test(m)?fmtDate(f):/to$/i.test(m)?fmtDate(e):n});setCsrfCookie(a,c);c=AnyBalance.requestPost(a+"payment-full-history.aspx",g,addHeaders({Referer:AnyBalance.getLastUrl()}));
checkIHError(c,!0);var h=getElement(c,/<div[^>]+paymentsGridium[^>]*>/i);if(!h){var k=getElement(c,/<div[^>]+panelMessage[^>]*>/i,replaceTagsAndSpaces,html_entity_decode);if(k&&/платежей не было/i.test(k)){AnyBalance.trace(k);b.payments=[];return}AnyBalance.trace(c);throw new AnyBalance.Error("Не найдена таблица платежей. Сайт изменен?");}h=replaceAll(h,[/<tfoot[^>]*>[\s\S]*?<\/tfoot>/i,""]);b.payments=[];processTable(h,b.payments,"payments.",{date:{re:/Дата/i,result_func:parseDate},sum:{re:/Сумма/i},
descr:{re:/Тип платежа/i,result_func:html_entity_decode}})}catch(l){AnyBalance.trace("Не удалось получить платежи: "+l.message+"\nStack: "+l.stack)}}
function turnOffLoginSMSNotify(){var a=AnyBalance.requestGet("https://profile.mts.ru/account",addHeaders({Referer:"https://login.mts.ru/"})),b=getParam(a,/\/_next\/static\/([\d\S]*?)\/_buildManifest\.js/i,replaceHtmlEntities);a&&b?(a=AnyBalance.requestGet("https://profile.mts.ru/_next/data/"+b+"/account/safety.json",addHeaders({Accept:"*/*",Referer:"https://profile.mts.ru/account"})),(a=getJson(a))&&a.pageProps?"1"!==a.pageProps.initialState.settings.authuserlevel?(AnyBalance.trace("SMS для входа в кабинет включено. Выключаем..."),
a=AnyBalance.requestPost("https://profile.mts.ru/api",JSON.stringify({call:"updateAuthUserLevel",arg:"1"}),addHeaders({"Content-Type":"application/json",Referer:"https://profile.mts.ru/account/safety/auth-level"})),a=getJson(a),AnyBalance.trace(JSON.stringify(a)),"success"!==a.result?AnyBalance.trace("Не удалось отключить SMS для входа"):AnyBalance.trace("SMS для входа успешно отключено")):AnyBalance.trace("SMS для входа уже отключено"):AnyBalance.trace("Не удалось получить страницу настроек безопасности. Пропускаем проверку способа входа")):
