...
function enterLK(a){var b=AnyBalance.requestGet(g_baseurl,g_headers);500<=AnyBalance.getLastStatusCode()&&(AnyBalance.trace("МТС вернул 500. Пробуем ещё разок..."),b=AnyBalance.requestGet(g_baseurl,g_headers));if(500<=AnyBalance.getLastStatusCode())throw new AnyBalance.Error("Ошибка на сервере МТС, сервер не смог обработать запрос. Можно попытаться позже...");var b=redirectIfNeeded(b),c=getParam(b,/Продолжить вход в Личный кабинет МТС с номером\s*<b[^>]*>([\s\S]*?)<\/b>/i,[replaceTagsAndSpaces,/\D+/g,
""]);if(c){AnyBalance.trace("Предлагает автоматически залогиниться на "+c);var b=getElement(b,/<form[^>]+name="Login"/i),d;a.login!=c?(AnyBalance.trace("А нам нужен номер "+a.login+". Отказываемся..."),d="Ignore"):(AnyBalance.trace("А нам этот номер и нужен. Соглашаемся..."),d="Login");b=createFormParams(b,function(a,b,c,e){"IDButton"==c&&(e=d);return e});b=AnyBalance.requestPost(AnyBalance.getLastUrl(),b,addHeaders({Referer:AnyBalance.getLastUrl()}))}b=checkLoginState(b,{automatic:!0});AnyBalance.trace("isLoggedIn(html) = "+
isLoggedIn(b));if(isLoggedIn(b)){c=function(){var a=AnyBalance.requestGet(g_baseurlLogin+"/amserver/UI/Logout",g_headers);if(isLoggedIn(a))throw AnyBalance.trace(a),new AnyBalance.Error("Не удаётся выйти из личного кабинета, чтобы зайти под правильным номером. Сайт изменен?");return a};AnyBalance.trace("Уже залогинены, проверяем, что на правильный номер...");var e=getLKJson(b).phone;e?e!=a.login?(AnyBalance.trace("Залогинены на неправильный номер: "+e+", выходим"),b=c()):AnyBalance.trace("Залогинены на правильный номер: "+
e):(AnyBalance.trace(b),AnyBalance.trace("Не удалось определить текущий номер в кабинете, сайт изменен? Попробуем выйти и зайти с логином-паролем."),b=c())}if(!isLoggedIn(b)){if(a.onlyAutomatic)throw new AnyBalance.Error("Ручной вход запрещен");b=enterMTS(joinObjects(a,{html:b,service:"lk",url:AnyBalance.getLastUrl()}));b=checkLoginState(b)}if(!isLoggedIn(b)){if(getParam(b,null,null,/(auth-status=0)/i))throw new AnyBalance.Error("Неверный логин или пароль. Повторите попытку или получите новый пароль на сайте "+
g_baseurl+"/.",!1,!0);AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в личный кабинет. Он изменился или проблемы на сайте.");}__setLoginSuccessful();return b}function loginWithPassword(){AnyBalance.trace("Entering lk...");var a=AnyBalance.getPreferences();return enterLK({login:a.login,password:a.password})}
...
function processTrafficLKApi(a){if(isAvailable("remainders.traffic_left_mb","remainders.traffic_used_mb","remainders.traffic_left_till")){AnyBalance.trace("Пробуем получить трафик по апи из ЛК..");var b=AnyBalance.requestGet(g_baseurl+"/api/myInternet/trafficAvailable",addHeaders({Referer:g_baseurl,"X-Requested-With":"XMLHttpRequest"})),b=getJson(b);processTrafficH2O(b.h2OExtended,a)}}
