...
function checkLoginState(a,b){var c=b&&b.baseurl||g_baseurl,d=AnyBalance.getLastUrl(),e=getParam(a,/<META[^>]+http-equiv="refresh"[^>]+content="\d+;URL=([^"]*)"/i,replaceHtmlEntities);e&&(AnyBalance.trace("Meta redirect to "+e),AnyBalance.requestGet(c+"/sitesettings/RequireConfig",addHeaders({Referer:d})),a=AnyBalance.requestGet(c+"/sitesettings/Settings.js",addHeaders({Referer:d})));if(/checkAuthStatus|дождитесь окончания процесса авторизации/i.test(a)){a={};for(e=20;"Success"!=a.Data&&0<e--;){a=
AnyBalance.requestGet(c+"/WaitAuth/CheckAuth?_="+(new Date).getTime(),addHeaders({Referer:d}));a=getJson(a);"PreSuccess"==a.Data&&(a=AnyBalance.requestGet(c+"/WaitAuth/CompleteAuth?_="+(new Date).getTime(),addHeaders({Referer:d})),a=getJson(a),AnyBalance.trace("Received PreSuccess, called CompleteAuth: "+JSON.stringify(a)));if("Success"==a.Data)break;sleep(1E3)}if("Success"!=a.Data){AnyBalance.trace("Слишком долго ждали авторизацию: "+JSON.stringify(a));if(b&&b.automatic)return AnyBalance.trace("МТС не пустил нас в ЛК после ожидания авторизации. Ладно, попробуем с логином и паролем войти."),
AnyBalance.requestGet(g_baseurlLogin.replace(/\/Login.*/,"/Logout",addHeaders({Referer:c})));throw new AnyBalance.Error("МТС не пустил нас в "+c+" после ожидания авторизации. Это проблема на сайте МТС, как только работа сайта наладится - данные отобразятся.");}return AnyBalance.requestGet(c,addHeaders({Referer:d}))}return a}
...
null,null,/(auth-status=0)/i))throw new AnyBalance.Error("Неверный логин или пароль. Повторите попытку или получите новый пароль на сайте "+g_baseurl+"/.",!1,!0);AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в личный кабинет. Он изменился или проблемы на сайте.");}__setLoginSuccessful();return b}function loginWithPassword(){AnyBalance.trace("Entering lk...");var a=AnyBalance.getPreferences();return enterLK({login:a.login,password:a.password})}
