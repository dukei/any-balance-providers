...
function isLoggedIn(a){return getParam(a,/(<meta[^>]*name="lkMonitorCheck")/i)||/js-logout/i.test(a)}function getLKJson(a,b){try{return getLKJson2(a)}catch(c){AnyBalance.trace(c.message),AnyBalance.trace("Не удалось найти Json с описанием пользователя первым способом, сайт изменен?")}try{return getLKJson1(a)}catch(c){if(b)throw c;AnyBalance.trace(c.message);AnyBalance.trace("Не удалось найти Json с описанием пользователя двумя способами, сайт изменен?");json={}}return{}}
