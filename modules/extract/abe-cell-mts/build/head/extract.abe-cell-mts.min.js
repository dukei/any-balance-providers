...
function getLKJson2(a){a=AnyBalance.requestGet("https://login.mts.ru/api/profile",addHeaders({Referer:g_baseurl+"/"}));var b=getJson(a),c={};c.fio=b["profile:name"];c.phone_formatted=(""+b["mobile:phone"]).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4");c.phone=""+b["mobile:phone"];c.balance=Math.round(100*b["mobile:balance"])/100;c.balance||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+a);c.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+
a);return c}
...
function loginWithPassword(){AnyBalance.trace("Entering lk...");var a=AnyBalance.getPreferences();return enterLK({login:a.login,password:a.password,baseurl:"https://lk.mts.ru",url:"https://login.mts.ru/amserver/UI/Login?service=lk&goto=http%3A%2F%2Flk.mts.ru%2F"})}
...
var b={};b.login=a.login;var c,d;try{c=enterLK({login:a.login,onlyAutomatic:!0}),a.password||(d=generatePassword()),(a.password||d)&&changePassword(void 0,a.password||d)}catch(e){AnyBalance.trace("Автоматический вход в кабинет не удался. Пробуем получить пароль через СМС"),d=getPasswordBySMS(a.login),c=enterLK({login:a.login,password:d,baseurl:"https://lk.mts.ru",url:"https://login.mts.ru/amserver/UI/Login?service=lk&goto=http%3A%2F%2Flk.mts.ru%2F"}),a.password&&a.password!=d&&changePassword(d,a.password)}b.password=
a.password||d;b.html=c;return b}function initialize(){var a=loginWithoutPassword(),b={success:!0,__initialization:!0};b.login=prefs.login;b.password=a.password;AnyBalance.setResult(b)}
