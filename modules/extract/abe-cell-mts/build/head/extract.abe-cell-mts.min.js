...
function checkLoginState(a,b){var c=b&&b.baseurl||g_baseurl,d=AnyBalance.getLastUrl();if(/checkAuthStatus|дождитесь окончания процесса авторизации/i.test(a)||/waitauth/i.test(d)){a={};for(var e=20;"Success"!=a.Data&&0<e--;){a=AnyBalance.requestGet(c+"/WaitAuth/CheckAuth?_="+(new Date).getTime(),addHeaders({Referer:d}));a=getJson(a);"PreSuccess"==a.Data&&(a=AnyBalance.requestGet(c+"/WaitAuth/CompleteAuth?_="+(new Date).getTime(),addHeaders({Referer:d})),a=getJson(a),AnyBalance.trace("Received PreSuccess, called CompleteAuth: "+
JSON.stringify(a)));if("Success"==a.Data)break;sleep(1E3)}if("Success"!=a.Data){AnyBalance.trace("Слишком долго ждали авторизацию: "+JSON.stringify(a));if(b&&b.automatic)return AnyBalance.trace("МТС не пустил нас в ЛК после ожидания авторизации. Ладно, попробуем с логином и паролем войти."),AnyBalance.requestGet(g_baseurlLogin.replace(/\/Login.*/,"/Logout",addHeaders({Referer:c})));throw new AnyBalance.Error("МТС не пустил нас в "+c+" после ожидания авторизации. Это проблема на сайте МТС, как только работа сайта наладится - данные отобразятся.");
