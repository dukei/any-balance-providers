...
"remainders.traffic_left_mb",/Подбаланс gprs:[^\d]*(\d+[\.,]?\d*\s*([kmgкмг][бb]|байт|bytes))/ig,null,parseTraffic,aggregate_sum),sumParam(a,b.remainders,"remainders.traffic_left_till",[/Подбаланс gprs:[^<]*?[kmgкмг][бb]\s*до\s*([\s\S]*?)<\//ig,/Остаток GPRS-пакета[^<]*[мm][бb][^<]*действует до([^<]*)/ig],null,parseDate,aggregate_min),getParam(a,b.remainders,"remainders.bonus_balance",/Остаток бонуса:?\s*([\d\.,]+)\s*р/i,replaceTagsAndSpaces,parseBalance),sumParam(a,b.remainders,"remainders.traffic_used_mb",
/Использовано:?\s*(\d+\s*[кkmмгg][бb])/ig,replaceTagsAndSpaces,parseTrafficFromKb,aggregate_sum));AnyBalance.isAvailable("info")&&(b.info||(b.info={}),getParam(a,b.info,"info.licschet",/№([\s\S]*?)[:<]/,replaceTagsAndSpaces),getParam(a,b.info,"info.sim",/Номер SIM-карты:([\s\S]*?)[:<]/,replaceTagsAndSpaces));getParam(a,b,"debt",/Сумма [^<]*по неоплаченным счетам(?:[\s\S](?!<\/td|<\/p))*?([-\d\.,]+)\s+руб/i,replaceTagsAndSpaces,parseBalance);getParam(a,b,"pay_till",/оплатить до\s*([\d\.,\/]+)/i,replaceTagsAndSpaces,
parseDate);getParam(a,b,"statuslock",/<(?:p|div)[^>]+class="account-status-lock"[^>]*>([\s\S]*?)<\/(?:p|div)>/i,replaceTagsAndSpaces);getParam(a,b,"credit",/(?:Лимит|Сумма кредитного лимита)[\s\S]*?([-\d\.,]+)\s*\(?руб/i,replaceTagsAndSpaces,parseBalance);getParam(a,b,"usedinthismonth",/Израсходовано [^<]*?(?:<[^>]*>)?([\d\.,]+) \(?руб/i,replaceTagsAndSpaces,parseBalance)}function isLoggedIn(a){return getParam(a,null,null,/(<meta[^>]*name="lkMonitorCheck")/i)}
