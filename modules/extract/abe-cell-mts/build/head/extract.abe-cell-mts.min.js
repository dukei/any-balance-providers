...
function processCountersLK(a){var b=callNewLKApiToken("sharing/counters");b=callNewLKApiResult(b);a.remainders||(a.remainders={});for(var c=0;c<b.counters.length;++c){var d=b.counters[c];AnyBalance.trace("Найден счетчик "+d.name+" ("+d.packageType+")");if("Calling"===d.packageType){AnyBalance.trace("Это минуты");var e=1;"Second"===d.unitType&&(e=60);getParam(d.deadlineDate,a.remainders,"remainders.min_till",null,null,parseDateISO);getParam(d.usedAmount/e,a.remainders,"remainders.min_local");getParam((d.totalAmount-
...
d.unitType,a.remainders,"remainders.traffic_used_mb",null,null,parseTraffic),getParam(d.totalAmount-d.usedAmount+d.unitType,a.remainders,"remainders.traffic_left_mb",null,null,parseTraffic)):AnyBalance.trace("Неизвестный счетчик: "+JSON.stringify(d))}}
...
var b={};b.login=a.login;var c;try{var d=enterLK({login:a.login,onlyAutomatic:!0});a.password||(c=generatePassword());(a.password||c)&&changePassword(void 0,a.password||c)}catch(e){AnyBalance.trace("Автоматический вход в кабинет не удался. Пробуем получить пароль через СМС"),c=getPasswordBySMS(a.login),d=enterLK({login:a.login,password:c,baseurl:"https://lk.mts.ru",url:"https://login.mts.ru/amserver/UI/Login?service=lk&goto=http%3A%2F%2Flk.mts.ru%2F"}),a.password&&a.password!=c&&changePassword(c,
a.password)}b.password=a.password||c;b.html=d;return b}function initialize(){var a=loginWithoutPassword(),b={success:!0,__initialization:!0};b.login=prefs.login;b.password=a.password;AnyBalance.setResult(b)}
...
function processPayments(a,b){try{AnyBalance.trace("Получаем платежи...");var c=AnyBalance.requestGet(a+"payment-full-history.aspx",g_headers);checkIHError(c,!0);var d=getElement(c,/<form[^>]+aspnetForm[^>]*>/i),e=new Date,h=new Date(e.getFullYear()-1,e.getMonth(),e.getDate()),f=createFormParams(d,function(a,b,c,d){return/from$/i.test(c)?fmtDate(h):/to$/i.test(c)?fmtDate(e):d});setCsrfCookie(a,c);c=AnyBalance.requestPost(a+"payment-full-history.aspx",f,addHeaders({Referer:AnyBalance.getLastUrl()}));
checkIHError(c,!0);var k=getElement(c,/<div[^>]+paymentsGridium[^>]*>/i);if(!k){var g=getElement(c,/<div[^>]+panelMessage[^>]*>/i,replaceTagsAndSpaces,html_entity_decode);if(g&&/платежей не было/i.test(g)){AnyBalance.trace(g);b.payments=[];return}AnyBalance.trace(c);throw new AnyBalance.Error("Не найдена таблица платежей. Сайт изменен?");}k=replaceAll(k,[/<tfoot[^>]*>[\s\S]*?<\/tfoot>/i,""]);b.payments=[];processTable(k,b.payments,"payments.",{date:{re:/Дата/i,result_func:parseDate},sum:{re:/Сумма/i},
