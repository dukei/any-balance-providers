...
{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","Accept-Language":"en-US,en;q=0.9",Connection:"keep-alive",Referer:"https://login.mts.ru/","Upgrade-Insecure-Requests":"1","User-Agent":"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.75 Safari/537.36"},replaceNumber=[replaceTagsAndSpaces,/\D/g,"",/.*(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7 $1 $2-$3-$4"];
...
replaceTagsAndSpaces,parseBalance)}function isLoggedIn(a){return getParam(a,/(<meta[^>]*name="lkMonitorCheck")/i)||/js-logout/i.test(a)||/UI\/Logout/i.test(a)||/Вы управляете:/i.test(a)}
function getLKJson(a,b){try{return getLKJson2(a)}catch(c){AnyBalance.trace(c.message),AnyBalance.trace("Не удалось найти Json с описанием пользователя первым способом, сайт изменен?")}try{return getLKJson1(a)}catch(c){if(b)throw c;AnyBalance.trace(c.message);AnyBalance.trace("Не удалось найти Json с описанием пользователя двумя способами, сайт изменен?");json={}}return{}}
...
a);return c}
function getLKJson1(a){a=AnyBalance.requestGet("http://login.mts.ru/profile/header?ref=https%3A//lk.mts.ru/&scheme=https&style=2015v2",addHeaders({Referer:g_baseurl+"/"}));var b={};b.fio=getElement(a,/<div[^>]+b-header_lk__name[^>]*>/i,replaceTagsAndSpaces);b.phone_formatted=getElement(a,/<div[^>]+b-header_lk__phone[^>]*>/i,replaceTagsAndSpaces);b.phone=replaceAll(b.phone_formatted,[/\+7/,"",/\D/g,""]);b.balance=getElement(a,/<div[^>]+b-header_balance[^>]*>/i,replaceTagsAndSpaces,parseBalance);b.balance||
...
d?d!=a.login?(AnyBalance.trace("Залогинены на неправильный номер: "+d+", выходим"),b=c()):AnyBalance.trace("Залогинены на правильный номер: "+d):(AnyBalance.trace(b),AnyBalance.trace("Не удалось определить текущий номер в кабинете, сайт изменен? Попробуем выйти и зайти с логином-паролем."),b=c())}if(!isLoggedIn(b)){if(a.onlyAutomatic)throw new AnyBalance.Error("Ручной вход запрещен");b=enterMtsLK(joinObjects(a,{html:b,service:"lk",url:AnyBalance.getLastUrl()}));b=checkLoginState(b)}if(!isLoggedIn(b)){if(getParam(b,
null,null,/(auth-status=0)/i))throw new AnyBalance.Error("Неверный логин или пароль. Повторите попытку или получите новый пароль на сайте "+g_baseurl+"/.",!1,!0);AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в личный кабинет. Он изменился или проблемы на сайте.");}__setLoginSuccessful();turnOffLoginSMSNotify(b);return b}
function loginWithPassword(){AnyBalance.trace("Entering lk...");var a=AnyBalance.getPreferences();g_savedData||(g_savedData=new SavedData("mts",a.login));g_savedData.restoreCookies();c=AnyBalance.requestGet("https://lk.mts.ru/",g_headers);if(/Мой МТС/i.test(c))AnyBalance.trace("Сессия сохранена. Входим автоматически...");else{AnyBalance.trace("Сессия новая. Будем логиниться заново...");clearAllCookies();var b=getParam(a.login,null,null,null,[/.*(\d{3})(\d{3})(\d{2})(\d{2})$/,"+7 ($1) $2-$3-$4"]),
c=AnyBalance.requestPost("https://login.mts.ru/amserver/json/authenticate?authIndexType=service&authIndexValue=login-spa",null,addHeaders({Accept:"*/*","Accept-API-Version":"resource=4.0, protocol=1.0","Content-Type":"application/json",Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/NUI/"})),d=getJson(c);AnyBalance.trace("Проверка доступа: "+JSON.stringify(d));var e=d.authId;c=AnyBalance.requestPost("https://login.mts.ru/amserver/json/authenticate?authIndexType=service&authIndexValue=login-spa",
JSON.stringify({authId:e,template:"NetworkHeaderRedirect.jsp",stage:"login-nodes-dslnull",header:"trusted-network",infoText:null,callbacks:[{type:"ConfirmationCallback",output:[{name:"prompt",value:"Confirm"},{name:"messageType",value:0},{name:"options",value:["IGNORE","OK"]},{name:"optionType",value:-1},{name:"defaultOption",value:1}],input:[{name:"IDToken1",value:1}]},{type:"MetadataCallback",output:[{name:"data",value:{acceptableOperators:["MTS","OTHER"],hiddenLogo:!1,restrictXTethered:!0,acceptableOperatorsErrorMessage:"Введен неверный номер телефона.",
accessConditions:"https://static.ssl.mts.ru/mts_rf/images/usloviya-edinogo-dostupa-k-servisam-MTS.html",correlationID:"7cef4c35-5349-4fe6-8f0f-7db3237b8fad",label:"Введите номер телефона",turnOffFeedbackService:!0,feedbackServiceUrl:"feedback-prod.stage2.websso.msk.mts.ru"}}]}]}),addHeaders({Accept:"*/*","Accept-API-Version":"resource=4.0, protocol=1.0","Content-Type":"application/json",Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/NUI/"}));d=getJson(c);AnyBalance.trace("Проверка сети: "+
JSON.stringify(d));c=AnyBalance.requestPost("https://login.mts.ru/amserver/json/authenticate?authIndexType=service&authIndexValue=login-spa",JSON.stringify({authId:e,template:"EnterPhone.jsp",stage:"login-nodes-dslnull",header:"enter-phone",infoText:null,callbacks:[{type:"NameCallback",output:[{name:"prompt",value:"Введите номер телефона"}],input:[{name:"IDToken1",value:"7"+a.login}]},{type:"ConfirmationCallback",output:[{name:"prompt",value:"Confirm"},{name:"messageType",value:0},{name:"options",
value:["IGNORE","OK"]},{name:"optionType",value:-1},{name:"defaultOption",value:0}],input:[{name:"IDToken2",value:1}]},{type:"MetadataCallback",output:[{name:"data",value:{acceptableOperators:["MTS","OTHER"],hiddenLogo:!1,restrictXTethered:!0,acceptableOperatorsErrorMessage:"Введен неверный номер телефона.",accessConditions:"https://static.ssl.mts.ru/mts_rf/images/usloviya-edinogo-dostupa-k-servisam-MTS.html",correlationID:"7cef4c35-5349-4fe6-8f0f-7db3237b8fad",label:"Введите номер телефона",turnOffFeedbackService:!0,
feedbackServiceUrl:"feedback-prod.stage2.websso.msk.mts.ru"}}]}]}),addHeaders({Accept:"*/*","Accept-API-Version":"resource=4.0, protocol=1.0","Content-Type":"application/json",Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/NUI/"}));d=getJson(c);AnyBalance.trace("Отправка номера телефона: "+JSON.stringify(d));"verify-otp"==d.header&&(AnyBalance.trace("МТС запросил проверку с помощью кода из SMS"),c=AnyBalance.retrieveCode("Пожалуйста, введите код подтверждения, высланный на номер "+
b+'\n\nЕсли вы не хотите постоянно вводить SMS-пароли при входе, выберите способ входа "Пароль" в настройках безопасности вашего личного кабинета МТС',null,{inputType:"number",time:3E5}),c=AnyBalance.requestPost("https://login.mts.ru/amserver/json/authenticate?authIndexType=service&authIndexValue=login-spa",JSON.stringify({authId:e,template:"VerifyOTP.jsp",stage:"login-nodes-dslnull",header:"verify-otp",infoText:null,callbacks:[{type:"PasswordCallback",output:[{name:"prompt",value:"Verify otp"}],
input:[{name:"IDToken1",value:c}]},{type:"ConfirmationCallback",output:[{name:"prompt",value:"Confirm"},{name:"messageType",value:0},{name:"options",value:["IGNORE","OK","RESTART","REFRESH","CANTGET"]},{name:"optionType",value:-1},{name:"defaultOption",value:0}],input:[{name:"IDToken2",value:1}]},{type:"MetadataCallback",output:[{name:"data",value:{hiddenLogo:!1,numberType:"MOBILE",restrictXTethered:!0,retryCount:3,isAux:!1,acceptableOperatorsErrorMessage:"Введен неверный номер телефона.",channel:"SMS",
accessConditions:"https://static.ssl.mts.ru/mts_rf/images/usloviya-edinogo-dostupa-k-servisam-MTS.html",label:"Введите номер телефона",abonent:"mMTS",userId:"dfcc9720-019a-11ea-88ba-339a25f0d609",acceptableOperators:["MTS","OTHER"],invalidCount:0,recipient:b,authUserLevel:"0",correlationID:"01a59697-d872-4a42-a3b3-c86877ad4bc8",isRestoreMode:!1,turnOffFeedbackService:!0,feedbackServiceUrl:"feedback-prod.stage2.websso.msk.mts.ru"}}]}]}),addHeaders({Accept:"*/*","Accept-API-Version":"resource=4.0, protocol=1.0",
"Content-Type":"application/json",Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/NUI/"})),d=getJson(c),AnyBalance.trace("Отправка кода из SMS: "+JSON.stringify(d)));if("verify-otp"==d.header)throw a=d.callbacks[2].output[0].value.retryCount-d.callbacks[2].output[0].value.invalidCount,AnyBalance.trace(c),new AnyBalance.Error("Неверный код! Осталось попыток: "+a);"verify-password"==d.header&&(c=AnyBalance.requestPost("https://login.mts.ru/amserver/json/authenticate?authIndexType=service&authIndexValue=login-spa",
JSON.stringify({authId:e,template:"VerifyPassword.jsp",stage:"login-nodes-dslnull",header:"verify-password",infoText:null,callbacks:[{type:"PasswordCallback",output:[{name:"prompt",value:"Verify password"}],input:[{name:"IDToken1",value:a.password}]},{type:"ConfirmationCallback",output:[{name:"prompt",value:"Confirm"},{name:"messageType",value:0},{name:"options",value:["IGNORE","OK","RESTART","RESTORE"]},{name:"optionType",value:-1},{name:"defaultOption",value:0}],input:[{name:"IDToken2",value:1}]},
{type:"MetadataCallback",output:[{name:"data",value:{lockoutCount:3,hiddenLogo:!1,numberType:"MOBILE",restrictXTethered:!0,acceptableOperatorsErrorMessage:"Введен неверный номер телефона.",accessConditions:"https://static.ssl.mts.ru/mts_rf/images/usloviya-edinogo-dostupa-k-servisam-MTS.html",label:"Введите номер телефона",abonent:"mMTS",userId:"c25f25f0-588d-11ea-98aa-b9cf018c481e",acceptableOperators:["MTS","OTHER"],invalidCount:0,recipient:b,authUserLevel:"1",correlationID:"7cef4c35-5349-4fe6-8f0f-7db3237b8fad",
turnOffFeedbackService:!0,feedbackServiceUrl:"feedback-prod.stage2.websso.msk.mts.ru"}}]}]}),addHeaders({Accept:"*/*","Accept-API-Version":"resource=4.0, protocol=1.0","Content-Type":"application/json",Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/NUI/"})),d=getJson(c),AnyBalance.trace("Отправка пароля: "+JSON.stringify(d)));if(d.tokenId&&d.successUrl)c=AnyBalance.requestGet(d.successUrl,addHeaders({Referer:"https://login.mts.ru/"}));else throw AnyBalance.trace(c),new AnyBalance.Error("Неверный логин или пароль!");
}g_savedData.setCookies();g_savedData.save();return c}
...
function parseTrafficFromKb(a){return parseTraffic(a,"kb")}function callNewLKApiToken(a){var b=AnyBalance.requestGet("https://lk.mts.ru/api/"+a,addHeaders({Referer:"https://lk.mts.ru/",Origin:"https://lk.mts.ru","X-Requested-With":"XMLHttpRequest",Accept:"application/json, text/plain, */*"})),c=getJson(b);if("string"!==typeof c)throw AnyBalance.trace(b),new AnyBalance.Error("Неверный ответ API "+a);return{token:c,verb:"api/"+a}}
function callNewLKApiResult(a){for(var b=0;30>b;++b){var c=AnyBalance.requestGet("https://lk.mts.ru/api/longtask/check/"+a.token+"?for="+a.verb,addHeaders({Referer:"https://lk.mts.ru/",Origin:"https://lk.mts.ru","X-Requested-With":"XMLHttpRequest",Accept:"application/json, text/plain, */*"}));AnyBalance.trace("Ожидание результата "+a.verb+" "+(b+1)+"/30");if(400<=AnyBalance.getLastStatusCode())throw AnyBalance.trace("Waiting for result error: "+c),new AnyBalance.Error(c);if(c)return b=getJson(c),
AnyBalance.trace("Получен результат "+a.verb+" от "+b.refreshDate),b.data;AnyBalance.sleep(2E3)}throw new AnyBalance.Error("Не удалось получить результат "+a.verb);}
function callNewLKApiResultAlt(a){for(var b=0;30>b;++b){var c=AnyBalance.requestGet("https://lk.mts.ru/api/longtask/check/"+a.token+"?for="+a.verb,addHeaders({Referer:"https://lk.mts.ru/",Origin:"https://lk.mts.ru","X-Requested-With":"XMLHttpRequest",Accept:"application/json, text/plain, */*"}));AnyBalance.trace("Ожидание результата "+a.verb+" "+(b+1)+"/30");if(400<=AnyBalance.getLastStatusCode())throw AnyBalance.trace("Waiting for result error: "+c),new AnyBalance.Error(c);if(c)return b=getJson(c),
AnyBalance.trace("Получен результат "+a.verb),b;AnyBalance.sleep(2E3)}throw new AnyBalance.Error("Не удалось получить результат "+a.verb);}
function processCountersLK(a){a.remainders||(a.remainders={});var b=callNewLKApiToken("sharing/counters");b=callNewLKApiResult(b);for(var c=0;c<b.counters.length;++c){var d=b.counters[c];AnyBalance.trace("Найден счетчик "+d.name+" ("+d.packageType+")");if("Calling"===d.packageType){AnyBalance.trace("Это минуты");var e=1;"Second"===d.unitType&&(e=60);getParam(d.deadlineDate,a.remainders,"remainders.min_till",null,null,parseDateISO);getParam(d.usedAmount/e,a.remainders,"remainders.min_local");getParam((d.totalAmount-
...
d.unitType,a.remainders,"remainders.traffic_used_mb",null,null,parseTraffic),getParam(d.usedByAcceptors+d.unitType,a.remainders,"remainders.traffic_used_by_acceptors_mb",null,null,parseTraffic),getParam(d.totalAmount-d.usedAmount+d.unitType,a.remainders,"remainders.traffic_left_mb",null,null,parseTraffic)):AnyBalance.trace("Неизвестный счетчик: "+JSON.stringify(d))}b=callNewLKApiToken("cashback/account");b=callNewLKApiResult(b);getParam(b.balance,a.remainders,"remainders.cashback");b=callNewLKApiToken("services/list/active");
b=callNewLKApiResult(b);getParam({Unblocked:"Номер не блокирован",Blocked:"Номер заблокирован"}[b.accountBlockStatus]||b.accountBlockStatus,a.remainders,"remainders.statuslock");getParam(b.services.length,a.remainders,"remainders.services");getParam(0,a.remainders,"remainders.services_free");getParam(0,a.remainders,"remainders.services_paid");for(c=0;c<b.services.length;++c)d=b.services[c],AnyBalance.trace("Найдена услуга "+d.name),!1===d.isSubscriptionFee?(AnyBalance.trace("Это бесплатная услуга"),
countFree=1,sumParam(countFree,a.remainders,"remainders.services_free",null,null,parseBalanceSilent,aggregate_sum)):!0===d.isSubscriptionFee?(AnyBalance.trace("Это платная услуга"),countPaid=1,sumParam(countPaid,a.remainders,"remainders.services_paid",null,null,parseBalanceSilent,aggregate_sum)):AnyBalance.trace("Неизвестный тип услуги: "+JSON.stringify(d));b=callNewLKApiToken("creditLimit");b=callNewLKApiResultAlt(b);getParam(b.currentCreditLimitValue,a.remainders,"remainders.credit")}
function mainLK(a,b){AnyBalance.trace("Мы в личном кабинете...");processInfoLK(a,b);try{processCountersLK(b)}catch(c){AnyBalance.trace("Ошибка получения остатков из кабинета. Попробуйте ещё раз позднее. Ошибка: "+c.message)}}function sleep(a){AnyBalance.trace("Sleeping "+a+" ms");AnyBalance.sleep(a)}function parseBalanceRound(a){a=parseBalance(a);isset(a)&&(a=Math.round(100*a)/100);return a}
