...
{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7","Accept-Language":"ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7","Cache-Control":"max-age=0",Connection:"keep-alive","Upgrade-Insecure-Requests":"1","User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36","If-Modified-Since":null},replaceNumber=[replaceTagsAndSpaces,/\D/g,"",/.*(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,
...
function processInfoLK(a,b){if(AnyBalance.isAvailable("balance","tariff","info"))try{var c=getLKJson(a,!0);c&&AnyBalance.trace(JSON.stringify(c));getParam(c.balance,b,"balance");getParam(c.tariff,b,"tariff");AnyBalance.isAvailable(["info.licschet","info.region","info.phone","info.fio"])&&(b.info||(b.info={}),getParam(c.license,b.info,"info.licschet"),getParam(c.region,b.info,"info.region"),getParam(c.phone,b.info,"info.phone",null,replaceNumber),getParam(c.fio,b.info,"info.fio"))}catch(d){AnyBalance.trace("Не удалось получить данные о пользователе, скорее всего, виджет временно недоступен... "+
...
addHeaders({"Content-Type":"application/json",Referer:g_baseurl+"/","X-Client-Id":"LK","X-Login":fetchNumber()}));var h=getJson(html);h.data&&h.data.transactionsByFilter&&(k=h.data.transactionsByFilter.totalPages,d=d.concat(h.data.transactionsByFilter.transactions))}catch(m){throw new AnyBalance.Error("Ошибка получения данных по общему расходу трафика: "+m.message);}while(++e<k+1);if(d&&0<d.length){AnyBalance.trace("Найдено записей по общему расходу трафика: "+d.length);for(var g=0;g<d.length;++g)e=
d[g],"Network"!==e.type||/Бесплатные интернет-ресурсы МТС/i.test(e.name)||sumParam(e.value+" "+e.unitValue,a.expenses,"expenses.traffic_used_total_mb",null,null,parseTraffic,aggregate_sum)}else AnyBalance.trace("Не удалось получить данные по общему расходу трафика")}if(isAvailable(["expenses.usedinthismonth","expenses.abonservice","expenses.refill"]))try{html=AnyBalance.requestPost("https://federation.mts.ru/graphql",JSON.stringify({operationName:"GetExpensesInfoBaseQueryInput",variables:{pageIndex:1,
...
function turnOffLoginSMSNotify(){var a=AnyBalance.requestGet("https://profile.mts.ru/account",addHeaders({Referer:"https://login.mts.ru/"})),b=getParam(a,/\/_next\/static\/([\d\S]*?)\/_buildManifest\.js/i,replaceHtmlEntities);a&&b?(a=AnyBalance.requestGet("https://profile.mts.ru/_next/data/"+b+"/account/safety.json",addHeaders({Accept:"*/*",Referer:"https://profile.mts.ru/account"})),(a=getJson(a))&&a.pageProps?a.pageProps.initialState&&a.pageProps.initialState.smsNotification&&a.pageProps.initialState.smsNotification.notifications?
(a=a.pageProps.initialState.smsNotification.notifications)&&!1!==a.lg_sms?(AnyBalance.trace("SMS уведомление о входе в кабинет включено. Выключаем..."),a.lg=!1,a.lg_sms=!1,a=AnyBalance.requestPost("https://profile.mts.ru/api",JSON.stringify({call:"patchSettingNotification",arg:a}),addHeaders({"Content-Type":"application/json",Referer:"https://profile.mts.ru/account/safety/sms-notifications"})),AnyBalance.trace(a),/successfully/i.test(a)?AnyBalance.trace("SMS уведомление о входе успешно отключено"):
AnyBalance.trace("Не удалось отключить SMS уведомление о входе")):AnyBalance.trace("SMS уведомление о входе уже отключено"):AnyBalance.trace("Не удалось получить настройки оповещения о входе"):AnyBalance.trace("Не удалось получить страницу настроек безопасности. Пропускаем проверку способа входа и оповещения о входе")):AnyBalance.trace("Не удалось получить идентификатор страницы настроек безопасности. Пропускаем проверку способа входа")};
