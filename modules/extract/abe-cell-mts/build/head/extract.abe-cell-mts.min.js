...
function loginWithPassword(){AnyBalance.trace("Entering lk...");var a=AnyBalance.getPreferences();return enterLK({login:a.login,password:a.password,baseurl:"http://lk.mts.ru",url:"http://login.mts.ru/amserver/UI/Login?service=lk&goto=http%3A%2F%2Flk.mts.ru%2F"})}
...
b=callNewLKApiResult(b);getParam({Unblocked:"Разблокировано",Blocked:"Заблокировано"}[b.accountBlockStatus]||b.accountBlockStatus,a.remainders,"remainders.statuslock");getParam(b.services.length,a.remainders,"remainders.services");getParam(0,a.remainders,"remainders.services_free");getParam(0,a.remainders,"remainders.services_paid");for(c=0;c<b.services.length;++c)d=b.services[c],AnyBalance.trace("Найдена услуга "+d.name),!1===d.isSubscriptionFee?(AnyBalance.trace("Это бесплатная услуга"),countFree=
1,sumParam(countFree,a.remainders,"remainders.services_free",null,null,parseBalanceSilent,aggregate_sum)):!0===d.isSubscriptionFee?(AnyBalance.trace("Это платная услуга"),countPaid=1,sumParam(countPaid,a.remainders,"remainders.services_paid",null,null,parseBalanceSilent,aggregate_sum)):AnyBalance.trace("Неизвестный тип услуги: "+JSON.stringify(d));b=callNewLKApiToken("creditLimit");b=callNewLKApiResultAlt(b);getParam(b.currentCreditLimitValue,a.remainders,"remainders.credit")}
function mainLK(a,b){AnyBalance.trace("Мы в личном кабинете...");for(var c=0;3>c;++c)try{if(isAnotherNumber()||isAvailableIH()){AnyBalance.trace("Попытка входа в интернет-помощник "+(c+1)+"/3");var d=followIHLink();isInOrdinary(d.html)||checkIHError(d.html,!0,!0);fetchOrdinary(d.html,d.baseurlHelper,b)}break}catch(e){if(isAnotherNumber())throw e;AnyBalance.trace("Не удалось получить данные из ип: "+e.message+"\n"+e.stack);2==c&&(b.were_errors=!0)}if(isAnotherNumber())AnyBalance.trace("Пропускаем получение данных из ЛК, если требуется информация по другому номеру");
else{processInfoLK(a,b);try{processCountersLK(b)}catch(e){AnyBalance.trace("Ошибка получения остатков из кабинета. Попробуйте ещё раз позднее. Ошибка: "+e.message)}}}function sleep(a){AnyBalance.trace("Sleeping "+a+" ms");AnyBalance.sleep(a)}function parseBalanceRound(a){a=parseBalance(a);isset(a)&&(a=Math.round(100*a)/100);return a}
