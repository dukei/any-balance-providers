...
BlockDueToInsufficiencyOfMoney:"Номер заблокирован",Blocked:"Номер заблокирован"}[b.accountBlockStatus]||b.accountBlockStatus,a.remainders,"remainders.statuslock");getParam(b.services.length,a.remainders,"remainders.services");getParam(0,a.remainders,"remainders.services_free");getParam(0,a.remainders,"remainders.services_paid");for(c=0;c<b.services.length;++c)d=b.services[c],AnyBalance.trace("Найдена услуга "+d.name),!1===d.isSubscriptionFee?(AnyBalance.trace("Это бесплатная услуга"),countFree=1,
sumParam(countFree,a.remainders,"remainders.services_free",null,null,parseBalanceSilent,aggregate_sum)):!0===d.isSubscriptionFee?(AnyBalance.trace("Это платная услуга"),countPaid=1,sumParam(countPaid,a.remainders,"remainders.services_paid",null,null,parseBalanceSilent,aggregate_sum)):AnyBalance.trace("Неизвестный тип услуги: "+JSON.stringify(d));b=callNewLKApiToken("creditLimit");b=callNewLKApiResult(b);getParam(b.currentCreditLimitValue,a.remainders,"remainders.credit");isAvailable(["expenses.usedinthismonth",
"expenses.usedinprevmonth","expenses.abonservice","expenses.refill"])&&processExpensesLK(a);isAvailable(["payments.sum","payments.date"])&&processTransactionsLK(a)}
function getCsrfToken(){AnyBalance.requestGet("https://lk.mts.ru/api/login/user-info",addHeaders({Accept:"application/json, text/plain, */*","X-Requested-With":"XMLHttpRequest"}));var a=AnyBalance.getLastResponseHeader("X-Refresh-CSRF-Token");a||AnyBalance.trace("Не удалось получить csrfToken");g_savedData.set("csrfToken",a);g_savedData.save()}
...
b.transactions[0];getParam(b.amount,a.payments,"payments.sum");getParam(b.dateTime.replace(/(\d{4})-(\d{2})-(\d{2}).*/,"$3.$2.$1"),a.payments,"payments.date",null,null,parseDate);break}else AnyBalance.trace("Не удалось получить данные последней транзакции")}
...
function turnOffLoginSMSNotify(){var a=AnyBalance.requestGet("https://profile.mts.ru/account",addHeaders({Referer:"https://login.mts.ru/"})),b=getParam(a,/<script[^>]+src="\/_next\/static\/(-?[\d\S]*?)\/_buildManifest.js" async=""><\/script>/i,replaceHtmlEntities);a&&b?(a=AnyBalance.requestGet("https://profile.mts.ru/_next/data/"+b+"/account/safety.json",addHeaders({Accept:"*/*",Referer:"https://profile.mts.ru/account"})),(a=getJson(a))&&a.pageProps?"1"!==a.pageProps.initialState.settings.authuserlevel?
(AnyBalance.trace("СМС для входа в кабинет включено. Выключаем..."),a=AnyBalance.requestPost("https://profile.mts.ru/api",JSON.stringify({call:"updateAuthUserLevel",arg:"1"}),addHeaders({"Content-Type":"application/json",Referer:"https://profile.mts.ru/account/safety/auth-level"})),a=getJson(a),AnyBalance.trace(JSON.stringify(a)),"success"!==a.result?AnyBalance.trace("Не удалось отключить СМС для входа"):AnyBalance.trace("СМС для входа успешно отключено")):AnyBalance.trace("СМС для входа уже отключено"):
AnyBalance.trace("Не удалось получить страницу настроек безопасности. Пропускаем проверку способа входа")):AnyBalance.trace("Не удалось получить идентификатор страницы настроек безопасности. Пропускаем проверку способа входа")};
