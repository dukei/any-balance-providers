...
function getPasswordBySMS(a){var b=g_baseurlLogin+"/amserver/UI/Login?service=smspassword&srcsvc=lk&goto=http%3A%2F%2Flk.ssl.mts.ru%2F",c=AnyBalance.requestGet(b,g_headers),d=getParam(c,[/<img[^>]+id="kaptchaImage"[^>]*src="data:image\/\w+;base64,([^"]+)/i,/<img[^>]+src="data:image\/\w+;base64,([^"]+)[^>]*id="kaptchaImage"/i],replaceHtmlEntities),e=AnyBalance.retrieveCode("Для получения пароля к личному кабинету по SMS символы, которые вы видите на картинке.",d,{time:3E5}),c=getParam(c,null,null,
/<form[^>]+name="Login"[^>]*>([\s\S]*?)<\/form>/i),c=createFormParams(c,function(b,c,d,k){"IDToken1"==d&&(k=a);"IDToken2"==d&&(k=e);return k}),c=AnyBalance.requestPost(b,c,addHeaders({Referer:b}));if(b=getParam(c,null,null,/var\s+passwordErr\s*=\s*'([^']*)/,replaceSlashes))throw new AnyBalance.Error(b);return AnyBalance.retrieveCode("На номер "+a+' выслан пароль к личному кабинету МТС. Введите его в поле ниже. \x3c!--#instruction:{"sms":{"number_in":"3339","regexp_in":"Пароль:\\s*(\\w+)"}}#--\x3e',
null,{time:3E5})}
