...
"Accept-Language":"en-US,en;q=0.9",Connection:"keep-alive",Referer:"https://login.mts.ru/","Upgrade-Insecure-Requests":"1","User-Agent":"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.75 Safari/537.36"},replaceNumber=[replaceTagsAndSpaces,/\D/g,"",/.*(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7 $1 $2-$3-$4"];function getParamByName(a,b){return getParam(a,null,null,new RegExp("name=[\"']"+b+'["\'][^>]*value=["\']([^"\']+)"',"i"))}
...
d.phone=""+b["mobile:phone"],d.tariff=""+b["mobile:tariff"]):(d.fio=c["profile:name"],d.phone_formatted=(""+c["mobile:phone"]).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4"),d.phone=""+c["mobile:phone"],d.tariff=""+c["mobile:tariff"]);b=callNewLKApiToken("accountInfo/mscpBalance");b=callNewLKApiResult(b);d.balance=getParam(Math.round(100*b.amount)/100);d.balance||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+a);d.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+
a);return d}
...
function processInfoLK(a,b){if(AnyBalance.isAvailable("balance","tariff","info.phone","info.fio"))try{var c=getLKJson(a,!0);AnyBalance.trace(JSON.stringify(c));getParam(c.balance,b,"balance");getParam(c.tariff,b,"tariff");AnyBalance.isAvailable("info")&&(b.info||(b.info={}),getParam(c.phone,b.info,"info.phone",null,replaceNumber),getParam(c.fio,b.info,"info.fio"))}catch(d){AnyBalance.trace("Не удалось получить данные о пользователе, скорее всего, виджет временно недоступен... "+d.message+"\n"+d.stack)}}
function parseTrafficFromKb(a){return parseTraffic(a,"kb")}function fetchNumber(){var a=AnyBalance.getPreferences();return a.phone&&a.phone!=a.login?"7"+a.phone:"7"+a.login}
function callNewLKApiToken(a){var b=AnyBalance.requestGet("https://lk.mts.ru/api/"+a,addHeaders({Accept:"application/json, text/plain, */*",Origin:"https://lk.mts.ru",Referer:"https://lk.mts.ru/","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"})),c=getJson(b);if("string"!==typeof c)throw AnyBalance.trace(b),new AnyBalance.Error("Неверный ответ API "+a);return{token:c,verb:"api/"+a}}
function callNewLKApiResult(a){for(var b=0;30>b;++b){var c=AnyBalance.requestGet("https://lk.mts.ru/api/longtask/check/"+a.token+"?for="+a.verb,addHeaders({Accept:"application/json, text/plain, */*",Origin:"https://lk.mts.ru",Referer:"https://lk.mts.ru/","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"}));AnyBalance.trace("Ожидание результата "+a.verb+" "+(b+1)+"/30");if(400<=AnyBalance.getLastStatusCode())throw AnyBalance.trace("Waiting for result error: "+c),new AnyBalance.Error(c);if(c)return b=
getJson(c),AnyBalance.trace("Получен результат "+a.verb+" от "+b.refreshDate),b.data||b;AnyBalance.sleep(2E3)}throw new AnyBalance.Error("Не удалось получить результат "+a.verb);}
function callNewLKApiResultAlt(a){for(var b=0;30>b;++b){var c=AnyBalance.requestGet("https://lk.mts.ru/api/longtask/check/"+a.token+"?for="+a.verb,addHeaders({Accept:"application/json, text/plain, */*",Origin:"https://lk.mts.ru",Referer:"https://lk.mts.ru/","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"}));AnyBalance.trace("Ожидание результата "+a.verb+" "+(b+1)+"/30");if(400<=AnyBalance.getLastStatusCode())throw AnyBalance.trace("Waiting for result error: "+c),new AnyBalance.Error(c);
if(c)return b=getJson(c),AnyBalance.trace("Получен результат "+a.verb),b;AnyBalance.sleep(2E3)}throw new AnyBalance.Error("Не удалось получить результат "+a.verb);}
function processCountersLK(a){a.remainders||(a.remainders={});var b=callNewLKApiToken("sharing/counters");b=callNewLKApiResult(b);for(var c=0;c<b.counters.length;++c){var d=b.counters[c];if(/Входящ/i.test(d.name))AnyBalance.trace("Найден счетчик "+d.name+" ("+d.packageType+"). Это посуточный дополнительный пакет. Пропускаем...");else if(AnyBalance.trace("Найден счетчик "+d.name+" ("+d.packageType+")"),"Calling"===d.packageType){AnyBalance.trace("Это минуты");var e=1;"Second"===d.unitType&&(e=60);
getParam(d.deadlineDate,a.remainders,"remainders.min_till",null,null,parseDateISO);getParam(d.usedAmount/e,a.remainders,"remainders.min_local");getParam((d.totalAmount-d.usedAmount)/e,a.remainders,"remainders.min_left")}else"Messaging"===d.packageType?(AnyBalance.trace("Это сообщения"),getParam(d.deadlineDate,a.remainders,"remainders.sms_till",null,null,parseDateISO),getParam(d.usedAmount,a.remainders,"remainders.sms_used"),getParam(d.totalAmount-d.usedAmount,a.remainders,"remainders.sms_left")):
