...
function getParamByName(a,b){return getParam(a,null,null,new RegExp("name=[\"']"+b+'["\'][^>]*value=["\']([^"\']+)"',"i"))}function isInOrdinary(a){return/<div[^>]+class="main-menu"/i.test(a)}function uniq_fast(a){for(var b={},c=[],d=a.length,e=0,k=0;k<d;k++){var h=a[k];1!==b[h]&&(b[h]=1,c[e++]=h)}return c}
...
replaceTagsAndSpaces,parseBalance)}function isLoggedIn(a){AnyBalance.getPreferences();a=AnyBalance.requestGet("https://lk.mts.ru/auth/account/is-authorized",g_headers);return"true"===a}
...
function enterLK2(a,b){AnyBalance.getPreferences();var c=b.login.replace(/.*(\d{3})(\d{3})(\d{2})(\d{2})$/i,"$1 $2-$3-$4");if(/captchaForm/i.test(a)){AnyBalance.trace("МТС затребовал капчу");var d=getElement(a,/<form[^>]+captchaForm/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удаётся найти форму для ввода капчи! Сайт изменен?");a=getParam(d,/data:image\/\w+?(?:png)?;base64,([^"]+)/i);var e=AnyBalance.retrieveCode("МТС требует ввести капчу для входа в личный кабинет, чтобы подтвердить, что вы не робот. Пожалуйста, введите символы с картинки",
a);d=createFormParams(d,function(k,h,g,f){"IDToken2"==g?f=e:"noscript"==g&&(f=void 0);return f});a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",d,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}))}if(/captchaForm/i.test(a)){d=getElement(a,/<form[^>]+captchaForm/i);if(d=getParam(d,null,null,/class="errorText"[\s\S]*?data-error="([^"]*)/i,replaceTagsAndSpaces))throw new AnyBalance.Error(d,
null,/код/i.test(d));throw new AnyBalance.Error("Не удалось войти в личный кабинет. Сайт изменен?");}d=getElement(a,/<form[^>]+loginForm/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удаётся найти форму входа! Сайт изменен?");d=createFormParams(d,function(k,h,g,f){"login"==g?f=c:"IDToken1"==g?f=b.login:"IDToken2"==g?f=b.password:"noscript"==g&&(f=void 0);return f});a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",d,addHeaders({Origin:"https://login.mts.ru",
Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}));d=getElement(a,/<form[^>]*>/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удаётся найти форму входа! Сайт изменен?");d=createFormParams(d,function(k,h,g,f){"login"==g?f=c:"IDToken1"==g?f=b.login:"IDToken2"==g?f='{"screen":{"screenWidth":1600,"screenHeight":900,"screenColourDepth":24},"userAgent":"Mozilla/5.0+(Windows+NT+10.0;+WOW64)+AppleWebKit/537.36+(KHTML,+like+Gecko)+Chrome/107.0.0.0+Safari/537.36","platform":"Win32","language":"ru","timezone":{"timezone":-180},"plugins":{"installedPlugins":"internal-pdf-viewer;internal-pdf-viewer;internal-pdf-viewer;internal-pdf-viewer;internal-pdf-viewer;"},"fonts":{"installedFonts":"cursive;monospace;serif;sans-serif;fantasy;default;Arial;Arial+Black;Arial+Narrow;Bookman+Old+Style;Bradley+Hand+ITC;Century;Century+Gothic;Comic+Sans+MS;Courier;Courier+New;Georgia;Impact;Lucida+Console;Monotype+Corsiva;Papyrus;Tahoma;Times;Times+New+Roman;Trebuchet+MS;Verdana;"},"appName":"Netscape","appCodeName":"Mozilla","appVersion":"5.0+(Windows+NT+10.0;+WOW64)+AppleWebKit/537.36+(KHTML,+like+Gecko)+Chrome/107.0.0.0+Safari/537.36","product":"Gecko","productSub":"20030107","vendor":"Google+Inc."}':
"noscript"==g&&(f=void 0);return f});a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",d,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}));if(/новый пользователь|Разблокировать/i.test(a))throw AnyBalance.trace(a),new AnyBalance.Error("Ваш номер не зарегистрирован в МТС или доступ к нему заблокирован. Пожалуйста, перейдите на страницу авторизации https://login.mts.ru/amserver/UI/Login через браузер и выполните действия по регистрации или восстановлению доступа",
null,!0);d=getElement(a,/<form[^>]+passwordForm/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удаётся найти форму входа! Сайт изменен?");d=createFormParams(d,function(k,h,g,f){"login"==g?f=c:"IDToken1"==g?f=b.login:"IDToken2"==g?f=b.password:"noscript"==g&&(f=void 0);return f});a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",d,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}));
if(/passwordForm/i.test(a)){d=getElement(a,/<form[^>]+passwordForm/i);if(d=getParam(d,null,null,/class="errorText"[\s\S]*?data-error="([^"]*)/i,replaceTagsAndSpaces))throw new AnyBalance.Error(d,null,/парол/i.test(d));throw new AnyBalance.Error("Не удалось войти в личный кабинет. Сайт изменен?");}d=getElement(a,/<form[^>]+gaForm/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удаётся найти форму входа! Сайт изменен?");d=createFormParams(d,function(k,h,g,f){"login"==g?f=c:"IDToken1"==g?
f=b.login:"IDToken2"==g?f=b.password:"noscript"==g&&(f=void 0);return f});return a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",d,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}))}
function turnOffLoginSMSNotify(a){var b=g_baseurlLogin+"/amserver/UI/Login?service=setnotify2018&ForceAuth=true";a=AnyBalance.requestGet(b,addHeaders({Referer:g_baseurl+"/settings"}));var c=getElements(a,[/<form/ig,/<input[^>]+checkbox[^>]+successfullLogin/i])[0];c?(a=createFormParams(c),a.successfullLogin?(AnyBalance.trace("СМС о входе в кабинет включено. Выключаем..."),delete a.successfullLogin,AnyBalance.requestPost(b,a,addHeaders({Referer:b})),/settings_changed/i.test(AnyBalance.getLastUrl())?
AnyBalance.trace("СМС о входе отключено, чтобы не терзало зря телефон"):AnyBalance.trace("Не удалось отключить смс о входе")):AnyBalance.trace("СМС о входе уже отключено")):AnyBalance.trace("Could not find notification form, skipping sms notification check: "+a)};
