...
"Accept-Language":"ru,en-US;q=0.8,en;q=0.6","User-Agent":"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36",Connection:"keep-alive","Upgrade-Insecure-Requests":"1","Accept-Encoding":"gzip, deflate, sdch, br"},replaceNumber=[replaceTagsAndSpaces,/\D/g,"",/.*(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7 $1 $2-$3-$4"];function getParamByName(a,b){return getParam(a,null,null,new RegExp("name=[\"']"+b+'["\'][^>]*value=["\']([^"\']+)"',"i"))}
function isInOrdinary(a){return/<div[^>]+class="main-menu"/i.test(a)}function uniq_fast(a){for(var b={},c=[],d=a.length,e=0,f=0;f<d;f++){var g=a[f];1!==b[g]&&(b[g]=1,c[e++]=g)}return c}function setCsrfCookie(a,b){b=getParam(b,null,null,/<input[^>]+name="csrfToken"[^>]*value="([^"]*)/i);a=getParam(a,null,null,/\/\/(.*?)\//);AnyBalance.setCookie(a,"csrfToken",null);AnyBalance.setCookie(a,"csrfToken",null);AnyBalance.setCookie(a,"csrfToken",b)}
...
/(?:Осталось|Остаток)(?: пакета)? (?:sms|смс):\s*(\d+)/ig,replaceTagsAndSpaces,parseBalance,aggregate_sum,!0),a=sumParam(a,b.remainders,"remainders.sms_left",/(?:Осталось|Остаток)[^\d]*(\d+)\s*(?:sms|смс)/ig,replaceTagsAndSpaces,parseBalance,aggregate_sum,!0),a=sumParam(a,b.remainders,"remainders.sms_left",/Остаток пакета[^<]*?(?:смс|sms):\s*([\d\.,]+)/ig,replaceTagsAndSpaces,parseBalance,aggregate_sum,!0),a=sumParam(a,b.remainders,"remainders.sms_left",/Осталось sms[^<]*?:\s*([\d\.,]+)/ig,replaceTagsAndSpaces,
...
"remainders.mms_left",/(?:Осталось|Остаток)(?: пакета)? (?:mms|ммс):\s*(\d+)/ig,replaceTagsAndSpaces,parseBalance,aggregate_sum),sumParam(a,b.remainders,"remainders.mms_left",/(?:Осталось|Остаток)[^\d]*(\d+)\s*(?:mms|ммс)/ig,replaceTagsAndSpaces,parseBalance,aggregate_sum),sumParam(a,b.remainders,"remainders.min_used",/Накоплено\s*([\d\.,]+)\s*мин[^\s]*/g,replaceTagsAndSpaces,parseBalance,aggregate_sum),sumParam(a,b.remainders,"remainders.traffic_left_mb",/(?:Осталось|Остаток)[^\d]*(\d+[\.,]?\d* *([kmgкмг][бb]|байт|bytes))/ig,
null,parseTraffic,aggregate_sum),sumParam(a,b.remainders,"remainders.traffic_left_mb",/Подбаланс gprs:[^\d]*(\d+[\.,]?\d*\s*([kmgкмг][бb]|байт|bytes))/ig,null,parseTraffic,aggregate_sum),sumParam(a,b.remainders,"remainders.traffic_left_till",[/Подбаланс gprs:[^<]*?[kmgкмг][бb]\s*до\s*([\s\S]*?)<\//ig,/Остаток GPRS-пакета[^<]*[мm][бb][^<]*действует до([^<]*)/ig],null,parseDate,aggregate_min),getParam(a,b.remainders,"remainders.bonus_balance",/Остаток бонуса:?\s*([\d\.,]+)\s*р/i,replaceTagsAndSpaces,
parseBalance),sumParam(a,b.remainders,"remainders.traffic_used_mb",/Использовано:?\s*(\d+\s*[кkmмгg][бb])/ig,replaceTagsAndSpaces,parseTrafficFromKb,aggregate_sum));AnyBalance.isAvailable("info")&&(b.info||(b.info={}),getParam(a,b.info,"info.licschet",/№([\s\S]*?)[:<]/,replaceTagsAndSpaces),getParam(a,b.info,"info.sim",/Номер SIM-карты:([\s\S]*?)[:<]/,replaceTagsAndSpaces));getParam(a,b,"debt",/Сумма [^<]*по неоплаченным счетам(?:[\s\S](?!<\/td|<\/p))*?([-\d\.,]+)\s+руб/i,replaceTagsAndSpaces,parseBalance);
getParam(a,b,"pay_till",/оплатить до\s*([\d\.,\/]+)/i,replaceTagsAndSpaces,parseDate);getParam(a,b,"statuslock",/<(?:p|div)[^>]+class="account-status-lock"[^>]*>([\s\S]*?)<\/(?:p|div)>/i,replaceTagsAndSpaces);getParam(a,b,"credit",/(?:Лимит|Сумма кредитного лимита)[\s\S]*?([-\d\.,]+)\s*\(?руб/i,replaceTagsAndSpaces,parseBalance);getParam(a,b,"usedinthismonth",/Израсходовано [^<]*?(?:<[^>]*>)?([\d\.,]+) \(?руб/i,replaceTagsAndSpaces,parseBalance)}
function isLoggedIn(a){return getParam(a,/(<meta[^>]*name="lkMonitorCheck")/i)||/api\/Login\/Logout/i.test(a)}function getLKJson(a,b){try{return getLKJson2(a)}catch(c){AnyBalance.trace(c.message),AnyBalance.trace("Не удалось найти Json с описанием пользователя первым способом, сайт изменен?")}try{return getLKJson1(a)}catch(c){if(b)throw c;AnyBalance.trace(c.message);AnyBalance.trace("Не удалось найти Json с описанием пользователя двумя способами, сайт изменен?");json={}}return{}}
...
function checkLoginState(a,b){var c=b&&b.baseurl||g_baseurl,d=AnyBalance.getLastUrl();if(/checkAuthStatus|дождитесь окончания процесса авторизации/i.test(a)||/waitauth/i.test(d)){a={};for(var e=20;"Success"!=a.Data&&0<e--;){a=AnyBalance.requestGet(c+"/WaitAuth/CheckAuth?_="+(new Date).getTime(),addHeaders({Referer:d}));if(404==AnyBalance.getLastStatusCode())return AnyBalance.trace("Проверка загрузки отсутствует, переходим на лк напрямую: "+c),a=AnyBalance.requestGet(c,addHeaders({Referer:AnyBalance.getLastUrl()}));
a=getJson(a);"PreSuccess"==a.Data&&(a=AnyBalance.requestGet(c+"/WaitAuth/CompleteAuth?_="+(new Date).getTime(),addHeaders({Referer:d})),a=getJson(a),AnyBalance.trace("Received PreSuccess, called CompleteAuth: "+JSON.stringify(a)));if("Success"==a.Data)break;sleep(1E3)}if("Success"!=a.Data){AnyBalance.trace("Слишком долго ждали авторизацию: "+JSON.stringify(a));if(b&&b.automatic)return AnyBalance.trace("МТС не пустил нас в ЛК после ожидания авторизации. Ладно, попробуем с логином и паролем войти."),
...
function enterLK(a){restoreLoginCookies();var b=enterMtsLK(a),b=checkLoginState(b,{automatic:!0});AnyBalance.trace("isLoggedIn(html) = "+isLoggedIn(b));if(isLoggedIn(b)){var c=function(){var a=AnyBalance.requestGet(g_baseurlLogin+"/amserver/UI/Logout",g_headers);if(isLoggedIn(a))throw AnyBalance.trace(a),new AnyBalance.Error("Не удаётся выйти из личного кабинета, чтобы зайти под правильным номером. Сайт изменен?");return a};AnyBalance.trace("Уже залогинены, проверяем, что на правильный номер...");
var d=getLKJson(b).phone;d?d!=a.login?(AnyBalance.trace("Залогинены на неправильный номер: "+d+", выходим"),b=c()):AnyBalance.trace("Залогинены на правильный номер: "+d):(AnyBalance.trace(b),AnyBalance.trace("Не удалось определить текущий номер в кабинете, сайт изменен? Попробуем выйти и зайти с логином-паролем."),b=c())}if(!isLoggedIn(b)){if(a.onlyAutomatic)throw new AnyBalance.Error("Ручной вход запрещен");b=enterMtsLK(joinObjects(a,{html:b,service:"lk",url:AnyBalance.getLastUrl()}));b=checkLoginState(b)}if(!isLoggedIn(b)){if(getParam(b,
null,null,/(auth-status=0)/i))throw new AnyBalance.Error("Неверный логин или пароль. Повторите попытку или получите новый пароль на сайте "+g_baseurl+"/.",!1,!0);AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в личный кабинет. Он изменился или проблемы на сайте.");}__setLoginSuccessful();saveLoginCookies();turnOffLoginSMSNotify(b);return b}
function loginWithPassword(){AnyBalance.trace("Entering lk...");var a=AnyBalance.getPreferences();return enterLK({login:a.login,password:a.password})}
...
function processTraffic(a){try{AnyBalance.trace("Пробуем получить трафик из ЛК по API");processTrafficLKApi(a);return}catch(b){AnyBalance.trace("Не удалось получить трафик из ЛК по API: "+b.message+"\n"+b.stack)}try{AnyBalance.trace("Пробуем получить трафик из интернет-кабинета");processTrafficInternet(a);return}catch(b){AnyBalance.trace("Не удалось получить трафик из интернет-кабинета: "+b.message+"\n"+b.stack)}a.were_errors=!0}function parseTrafficFromKb(a){return parseTraffic(a,"kb")}
function keysToLowerCase(a){function b(a){var b=typeof a;return"object"!==b||null===a||"string"===b||"number"===b||"boolean"===b?!1:!0}if(!b(a))return a;for(var c=Object.keys(a),d=c.length,e;d--;){var f=c[d],g=f===(e=f.toLowerCase());if(!g||b(a[f]))a[e]=keysToLowerCase(a[f]),g||delete a[f]}return a}
function processTrafficH2O(a,b){AnyBalance.trace("H2O profile: "+JSON.stringify(a));var c="undefined"!=typeof a.personalOptionUsage?a.personalOptionUsage.usageEndDate:a.traffic.homeNextRotateDate,d="undefined"!=typeof a.autoStep&&a.autoStep.isActive,e=d?a.autoStep.stepRemainder:a.traffic.available,d=d?a.autoStep.stepQuota-a.autoStep.stepRemainder:a.traffic.consumed;keysToLowerCase(a);b.remainders||(b.remainders={});a=b.remainders;sumParam(e+"",a,"remainders.traffic_left_mb",null,null,parseTrafficFromKb,
aggregate_sum);sumParam(d+"",a,"remainders.traffic_used_mb",null,null,parseTrafficFromKb,aggregate_sum);getParam(c,a,"remainders.traffic_left_till",null,null,parseDateISO)}
function processTrafficInternet(a){if(isAvailable("remainders.traffic_left_mb","remainders.traffic_used_mb","remainders.traffic_left_till")){var b=AnyBalance.requestGet("https://internet.mts.ru/",addHeaders({Referer:g_baseurl}));checkLoginState(b,{baseurl:"https://internet.mts.ru"});b=AnyBalance.requestGet("https://internet.mts.ru/sitesettings/H2OProfile.js",addHeaders({Referer:"https://internet.mts.ru/"}));b=getJsonObject(b,/_user:(?=\s*\{)/i);processTrafficH2O(b,a)}}
function processTrafficLKApi(a){if(isAvailable("remainders.traffic_left_mb","remainders.traffic_used_mb","remainders.traffic_left_till")){AnyBalance.trace("Пробуем получить трафик по апи из ЛК..");var b=AnyBalance.requestGet("https://oauth.mts.ru/webapi-1.4/api/counters/all",addHeaders({Referer:"https://oauth.mts.ru/wrs-4.1/gadgets/ifr?url=https://oauth.mts.ru/os/mts-my-account-payment/mts-my-account.xml&refresh=86400&lang=ru",Authorization:"Bearer sso_1.0_websso_cookie"})),c=getJson(b);AnyBalance.trace("Остатки: "+
b);b=c.counters.filter(function(a){return"internet"==a.type})[0];if(!b)throw new Anybalance.Error("Трафик отсутствует в счетчиках");a.remainders||(a.remainders={});a=a.remainders;getParam(b.value/b.ratio+b.measBase,a,"remainders.traffic_used_mb",null,null,parseTraffic);getParam((b.limit-b.value)/b.ratio+b.measBase,a,"remainders.traffic_left_mb",null,null,parseTraffic);getParam(b.expirationTime,a,"remainders.traffic_left_till",null,null,parseDateISO)}}
function processTrafficLK(a){if(isAvailable("remainders.traffic_left_mb","remainders.traffic_used_mb","remainders.traffic_left_till")){a.remainders||(a.remainders={});AnyBalance.trace("Запросим трафик...");for(var b=0;3>b;b++){AnyBalance.trace("Пробуем получить трафик, попытка: "+(b+1));var c=AnyBalance.requestGet(g_baseurl,addHeaders({Referer:g_baseurl})),c=getParam(c,null,null,/'\/miwidgettrafficavailable\/[^']*'/i,null,getJsonEval);if(!c)throw new AnyBalance.Error("Не удалось найти ссылку на трафик.");
c=joinUrl(g_baseurl,c);info=AnyBalance.requestGet(c+"&_="+(new Date).getTime(),addHeaders({"X-Requested-With":"XMLHttpRequest",Accept:"*/*",Referer:g_baseurl+"/"}));AnyBalance.trace(info);if(info){b=getJson(info);processTrafficH2O(b.H2OExtended,a);return}AnyBalance.trace("Сервер вернул ерунду, пробуем еще раз...")}throw new AnyBalance.Error("Трафик не найден!");}}
function mainLK(a,b){AnyBalance.trace("Мы в личном кабинете...");isAnotherNumber()?AnyBalance.trace("Пропускаем получение данных из ЛК, если требуется информация по другому номеру"):(processInfoLK(a,b),processBonusLK(b),processTraffic(b));for(var c=0;3>c;++c)try{if(isAnotherNumber()||isAvailableIH()){AnyBalance.trace("Попытка входа в интернет-помощник "+(c+1)+"/3");var d=followIHLink();a=d.html;isInOrdinary(a)||checkIHError(a,!0,!0);fetchOrdinary(a,d.baseurlHelper,b)}break}catch(e){if(isAnotherNumber())throw e;
AnyBalance.trace("Не удалось получить данные из ип: "+e.message+"\n"+e.stack);2==c&&(b.were_errors=!0)}}function sleep(a){AnyBalance.trace("Sleeping "+a+" ms");AnyBalance.sleep(a)}function parseBalanceRound(a){a=parseBalance(a);isset(a)&&(a=Math.round(100*a)/100);return a}
...
function turnOffLoginSMSNotify(a){var b=g_baseurlLogin+"/amserver/UI/Login?service=setnotify&ForceAuth=true";a=AnyBalance.requestGet(b,addHeaders({Referer:g_baseurl+"/settings"}));var c=getElements(a,[/<form/ig,/<input[^>]+checkbox[^>]+successfullLogin/i])[0];c?(a=createFormParams(c),a.successfullLogin?(AnyBalance.trace("СМС о входе в кабинет включено. Выключаем..."),delete a.successfullLogin,AnyBalance.requestPost(b,a,addHeaders({Referer:b})),/settings_changed/i.test(AnyBalance.getLastUrl())?AnyBalance.trace("СМС о входе отключено, чтобы не терзало зря телефон"):
AnyBalance.trace("Не удалось отключить смс о входе")):AnyBalance.trace("СМС о входе уже отключено")):AnyBalance.trace("Could not find notification form, skipping sms notification check: "+a)};
