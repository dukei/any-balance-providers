...
function getLKJson2(a){a=AnyBalance.requestGet("https://login.mts.ru/api/profile",addHeaders({Referer:g_baseurl+"/"}));var b=getJson(a),c={};c.fio=b["profile:name"];c.phone_formatted=(""+b["mobile:phone"]).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4");c.phone=""+b["mobile:phone"];c.balance=b["mobile:balance"];c.balance||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+a);c.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+a);return c}
