...
function enterLK(a){var b=enterMtsLK(a);b=checkLoginState(b,{automatic:!0});AnyBalance.trace("isLoggedIn(html) = "+isLoggedIn(b));/no-config&arg=newsession/i.test(AnyBalance.getLastUrl())&&enterLKUI(b,a);isLoggedIn(b)||(AnyBalance.trace("Требуется дологиниться"),AnyBalance.requestGet("https://lk.mts.ru/auth/account/login?goto=https://lk.mts.ru",addHeaders({Referer:"https://lk.mts.ru/"})));if(!isLoggedIn(b)){if(getParam(b,null,null,/(auth-status=0)/i))throw new AnyBalance.Error("Неверный логин или пароль. Повторите попытку или получите новый пароль на сайте "+
