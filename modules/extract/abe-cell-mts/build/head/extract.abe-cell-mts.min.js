...
replaceTagsAndSpaces,parseBalance)}function isLoggedIn(a){a=AnyBalance.getPreferences();a=AnyBalance.requestGet("https://lk.mts.ru/auth/account/is-authorized",addHeaders({"X-Requested-With":"XMLHttpRequest",Referer:"https://lk.mts.ru/","X-Login":"7"+a.login}));return"true"===a}
...
d?d!=a.login?(AnyBalance.trace("Залогинены на неправильный номер: "+d+", выходим"),b=c()):AnyBalance.trace("Залогинены на правильный номер: "+d):(AnyBalance.trace(b),AnyBalance.trace("Не удалось определить текущий номер в кабинете, сайт изменен? Попробуем выйти и зайти с логином-паролем."),b=c())}if(!isLoggedIn(b)){if(a.onlyAutomatic)throw new AnyBalance.Error("Ручной вход запрещен");b=enterMtsLK(joinObjects(a,{html:b,service:"lk",url:AnyBalance.getLastUrl()}));b=checkLoginState(b)}isLoggedIn(b)||
(AnyBalance.trace("Требуется дологиниться"),AnyBalance.requestGet("https://lk.mts.ru/auth/account/login?goto=https://lk.mts.ru",addHeaders({Referer:"https://lk.mts.ru/"})));if(!isLoggedIn(b)){if(getParam(b,null,null,/(auth-status=0)/i))throw new AnyBalance.Error("Неверный логин или пароль. Повторите попытку или получите новый пароль на сайте "+g_baseurl+"/.",!1,!0);AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в личный кабинет. Он изменился или проблемы на сайте.");}__setLoginSuccessful();
turnOffLoginSMSNotify(b);return b}
function loginWithPassword(){AnyBalance.trace("Пробуем войти в личный кабинет...");var a=AnyBalance.getPreferences();g_savedData.set("csrfToken","");g_savedData.save();b=loadProtectedPage("https://login.mts.ru/api/profile",addHeaders({referer:"https://lk.mts.ru/"}));if(/"mobile:phone"/i.test(b))AnyBalance.trace("Сессия сохранена. Входим автоматически...");else if(AnyBalance.trace("Сессия новая. Будем логиниться заново..."),clearAllCookiesExceptProtection(),/qrator/i.test(b))AnyBalance.trace("Обнаружена защита. Используем новый вход"),b=
enterLKNew({login:a.login,password:a.password,baseurl:"https://lk.mts.ru",url:"https://login.mts.ru/amserver/json/authenticate?authIndexType=service&authIndexValue=login-spa"});else{AnyBalance.trace("Защита не обнаружена. Используем старый вход");var b=enterLK({login:a.login,password:a.password,baseurl:"http://lk.mts.ru",url:"http://login.mts.ru/amserver/UI/Login?service=lk&goto=http%3A%2F%2Flk.mts.ru%2F"})}g_savedData.setCookies();g_savedData.save();return b}
...
function login(a){AnyBalance.setDefaultCharset("utf-8");var b=AnyBalance.getPreferences();g_savedData||(g_savedData=new SavedData("mts",b.login));g_savedData.restoreCookies();if(login.isLoggedIn)b=enterLK({login:b.login,onlyAutomatic:!0});else if(b.password)b=loginWithPassword();else{var c=loginWithoutPassword();b=c.html;a.password=c.password}login.isLoggedIn=!0;return b}
...
function processPayments(a,b){try{AnyBalance.trace("Получаем платежи...");var c=AnyBalance.requestGet(a+"payment-full-history.aspx",g_headers);checkIHError(c,!0);var d=getElement(c,/<form[^>]+aspnetForm[^>]*>/i),e=new Date,k=new Date(e.getFullYear()-1,e.getMonth(),e.getDate()),f=createFormParams(d,function(l,p,m,n){return/from$/i.test(m)?fmtDate(k):/to$/i.test(m)?fmtDate(e):n});setCsrfCookie(a,c);c=AnyBalance.requestPost(a+"payment-full-history.aspx",f,addHeaders({Referer:AnyBalance.getLastUrl()}));
checkIHError(c,!0);var g=getElement(c,/<div[^>]+paymentsGridium[^>]*>/i);if(!g){var h=getElement(c,/<div[^>]+panelMessage[^>]*>/i,replaceTagsAndSpaces,html_entity_decode);if(h&&/платежей не было/i.test(h)){AnyBalance.trace(h);b.payments=[];return}AnyBalance.trace(c);throw new AnyBalance.Error("Не найдена таблица платежей. Сайт изменен?");}g=replaceAll(g,[/<tfoot[^>]*>[\s\S]*?<\/tfoot>/i,""]);b.payments=[];processTable(g,b.payments,"payments.",{date:{re:/Дата/i,result_func:parseDate},sum:{re:/Сумма/i},
