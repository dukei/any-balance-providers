...
function getLKJson1(a){a=AnyBalance.requestGet("https://login.mts.ru/profile/header?ref=https%3A//lk.mts.ru/&scheme=https&style=2015v2",addHeaders({Referer:g_baseurl+"/","X-Login":fetchNumber()}));var b={};b.fio=getElement(a,/<div[^>]+b-header_lk__name[^>]*>/i,replaceTagsAndSpaces);b.phone_formatted=getElement(a,/<div[^>]+b-header_lk__phone[^>]*>/i,replaceTagsAndSpaces);b.phone=replaceAll(b.phone_formatted,[/\+7/,"",/\D/g,""]);b.balance=getElement(a,/<div[^>]+b-header_balance[^>]*>/i,replaceTagsAndSpaces,
...
null,null,parseTraffic)):AnyBalance.trace("Неизвестный счетчик: "+JSON.stringify(d))}isAvailable("remainders.cashback")&&(b=callNewLKApiToken("cashback/account"),b=callNewLKApiResult(b),getParam(b.balance,a.remainders,"remainders.cashback"));if(isAvailable("remainders.statuslock remainders.services remainders.services_free remainders.services_paid remainders.services_abon remainders.services_abon_day".split(" ")))for(b=callNewLKApiToken("services/list/active"),b=callNewLKApiResult(b),getParam({Unblocked:"Номер не блокирован",
OnlyInboundCalls:"Частичная блокировка",BlockDueToInsufficiencyOfMoney:"Номер заблокирован",Blocked:"Номер заблокирован"}[b.accountBlockStatus]||b.accountBlockStatus,a.remainders,"remainders.statuslock"),getParam(b.services.length,a.remainders,"remainders.services"),getParam(0,a.remainders,"remainders.services_free"),getParam(0,a.remainders,"remainders.services_paid"),getParam(0,a.remainders,"remainders.services_abon"),getParam(0,a.remainders,"remainders.services_abon_day"),c=0;c<b.services.length;++c)d=
b.services[c],!1===d.isSubscriptionFee||0===d.primarySubscriptionFee.value?(AnyBalance.trace("Найдена бесплатная услуга "+d.name),sumParam(1,a.remainders,"remainders.services_free",null,null,parseBalanceSilent,aggregate_sum)):!0===d.isSubscriptionFee&&0!==d.primarySubscriptionFee.value?(AnyBalance.trace("Найдена платная услуга "+d.name+": "+d.primarySubscriptionFee.quotaPeriodicity+" "+d.primarySubscriptionFee.value+" ₽"),sumParam(1,a.remainders,"remainders.services_paid",null,null,parseBalanceSilent,
aggregate_sum),"Month"!==d.primarySubscriptionFee.unitOfMeasure?sumParam(d.primarySubscriptionFee.value,a.remainders,"remainders.services_abon_day",null,null,parseBalanceSilent,aggregate_sum):sumParam(d.primarySubscriptionFee.value,a.remainders,"remainders.services_abon",null,null,parseBalanceSilent,aggregate_sum)):AnyBalance.trace("Неизвестный тип услуги: "+JSON.stringify(d));isAvailable("remainders.credit")&&(b=callNewLKApiToken("creditLimit"),b=callNewLKApiResult(b),getParam(b.currentCreditLimitValue,
a.remainders,"remainders.credit"));isAvailable(["remainders.cashback_mts","remainders.cashback_mts_pending","remainders.cashback_mts_burning","remainders.cashback_mts_burning_date","remainders.premium_state"])&&processCashbackLK(a);isAvailable("messages.total_msg","messages.unread_msg")&&processMessagesLK(a);isAvailable(["expenses.traffic_used_total_mb","expenses.usedinthismonth","expenses.usedinprevmonth","expenses.abonservice","expenses.refill"])&&processExpensesLK(a);isAvailable(["payments.sum",
"payments.date"])&&processTransactionsLK(a)}function getCsrfToken(){AnyBalance.requestGet("https://lk.mts.ru/api/login/refreshCsrfToken",addHeaders({Accept:"application/json, text/plain, */*",Referer:"https://lk.mts.ru/","X-Login":fetchNumber()}));var a=AnyBalance.getLastResponseHeader("X-Refresh-Csrf-Token");a||AnyBalance.trace("Не удалось получить csrfToken");return a}
...
addHeaders({"Content-Type":"application/json",Referer:g_baseurl+"/","X-Client-Id":"LK","X-Login":fetchNumber()}));var h=getJson(html);h.data&&h.data.transactionsByFilter&&(k=h.data.transactionsByFilter.totalPages,d=d.concat(h.data.transactionsByFilter.transactions))}catch(m){throw new AnyBalance.Error("Ошибка получения данных по общему расходу трафика: "+m.message);}while(++e<k+1);if(d&&0<d.length){AnyBalance.trace("Найдено записей по общему расходу трафика: "+d.length);for(var f=0;f<d.length;++f)e=
d[f],"Network"!==e.type||/Бесплатные интернет-ресурсы МТС/i.test(e.name)||sumParam(e.value+" "+e.unitValue,a.expenses,"expenses.traffic_used_total_mb",null,null,parseTraffic,aggregate_sum)}else AnyBalance.trace("Не удалось получить данные по общему расходу трафика")}if(isAvailable(["expenses.usedinthismonth","expenses.abonservice","expenses.refill"]))try{html=AnyBalance.requestPost("https://federation.mts.ru/graphql",JSON.stringify({operationName:"GetExpensesInfoBaseQueryInput",variables:{pageIndex:1,
filter:{dateFrom:c,dateTo:b,showFree:!1,tabType:null,categoryId:null,textSearch:"",transactionsDirection:null,cashbackType:"ALL"}},query:"query GetExpensesInfoBaseQueryInput($pageIndex: Int!, $filter: TransactionsFilterInput!) {\n  transactionsByFilter(input: {pageIndex: $pageIndex, filter: $filter}) {\n    totalInfo {\n      currency\n      incomeAmount\n      outcomeAmount\n      cashback {\n        incomeAmount\n        outcomeAmount\n        __typename\n      }\n      period {\n        dateFrom\n        dateTo\n        __typename\n      }\n      __typename\n    }\n    categories {\n      alias\n      value\n      name\n      color\n      amount\n      percentage\n      __typename\n    }\n    totalPages\n    transactions {\n      name\n      type\n      subtitle\n      dateTime\n      unitValue\n      value\n      direction\n      amount\n      icon\n      darkIcon\n      description\n      productType\n      globalCode\n      actionLabel\n      hints\n      cashbackInfo {\n        amount\n        direction\n        __typename\n      }\n      chargePeriod {\n        dateFrom\n        dateTo\n        __typename\n      }\n      __typename\n    }\n    __typename\n  }\n}"}),
addHeaders({"Content-Type":"application/json",Referer:g_baseurl+"/","X-Client-Id":"LK","X-Login":fetchNumber()}));h=getJson(html);AnyBalance.trace(JSON.stringify(h));var g=h.data;if(g.transactionsByFilter&&g.transactionsByFilter.categories&&0<g.transactionsByFilter.categories.length)for(f=0;g.transactionsByFilter.categories&&f<g.transactionsByFilter.categories.length;++f){var l=g.transactionsByFilter.categories[f];AnyBalance.trace('Найдена категория расходов "'+l.name+'": '+l.amount+" ₽");sumParam(l.amount,
a.expenses,"expenses.abonservice",null,null,parseBalanceSilent,aggregate_sum)}else AnyBalance.trace("Не удалось получить данные по категориям расходов за этот месяц");g.transactionsByFilter&&g.transactionsByFilter.totalInfo?(getParam(g.transactionsByFilter.totalInfo.incomeAmount,a.expenses,"expenses.refill"),getParam(g.transactionsByFilter.totalInfo.outcomeAmount,a.expenses,"expenses.usedinthismonth")):AnyBalance.trace("Не удалось получить сводные данные по расходам за этот месяц")}catch(m){AnyBalance.trace("Ошибка получения данных по расходам за этот месяц: "+
m.message)}if(isAvailable("expenses.usedinprevmonth")){b=new Date;f=new Date(b.getFullYear(),b.getMonth(),0);c=f.getFullYear()+"-"+n2(f.getMonth()+1)+"-01T00:00:00+03:00";f=f.getFullYear()+"-"+n2(f.getMonth()+1)+"-"+n2(f.getDate())+"T23:59:59+03:00";try{html=AnyBalance.requestPost("https://federation.mts.ru/graphql",JSON.stringify({operationName:"GetExpensesInfoBaseQueryInput",variables:{pageIndex:1,filter:{dateFrom:c,dateTo:f,showFree:!1,tabType:null,categoryId:null,textSearch:"",transactionsDirection:null,
cashbackType:"ALL"}},query:"query GetExpensesInfoBaseQueryInput($pageIndex: Int!, $filter: TransactionsFilterInput!) {\n  transactionsByFilter(input: {pageIndex: $pageIndex, filter: $filter}) {\n    totalInfo {\n      currency\n      incomeAmount\n      outcomeAmount\n      cashback {\n        incomeAmount\n        outcomeAmount\n        __typename\n      }\n      period {\n        dateFrom\n        dateTo\n        __typename\n      }\n      __typename\n    }\n    categories {\n      alias\n      value\n      name\n      color\n      amount\n      percentage\n      __typename\n    }\n    totalPages\n    transactions {\n      name\n      type\n      subtitle\n      dateTime\n      unitValue\n      value\n      direction\n      amount\n      icon\n      darkIcon\n      description\n      productType\n      globalCode\n      actionLabel\n      hints\n      cashbackInfo {\n        amount\n        direction\n        __typename\n      }\n      chargePeriod {\n        dateFrom\n        dateTo\n        __typename\n      }\n      __typename\n    }\n    __typename\n  }\n}"}),
addHeaders({"Content-Type":"application/json",Referer:g_baseurl+"/","X-Client-Id":"LK","X-Login":fetchNumber()})),h=getJson(html),AnyBalance.trace(JSON.stringify(h)),g=h.data,g.transactionsByFilter&&g.transactionsByFilter.totalInfo?(getParam(g.transactionsByFilter.totalInfo.incomeAmount,a.expenses,"expenses.refillinprevmonth"),getParam(g.transactionsByFilter.totalInfo.outcomeAmount,a.expenses,"expenses.usedinprevmonth")):AnyBalance.trace("Не удалось получить сводные данные по расходам за прошлый месяц")}catch(m){AnyBalance.trace("Ошибка получения данных по расходам за прошлый месяц: "+
m.message)}}}
...
}}processInfoLK(a,b);try{processCountersLK(b)}catch(h){AnyBalance.trace("Ошибка получения остатков из кабинета. Попробуйте ещё раз позднее. Ошибка: "+h.message),b.were_errors=!0}}function sleep(a){AnyBalance.trace("Sleeping "+a+" ms");AnyBalance.sleep(a)}function parseBalanceRound(a){a=parseBalance(a);isset(a)&&(a=Math.round(100*a)/100);return a}
...
checkIHError(c,!0);var f=getElement(c,/<div[^>]+paymentsGridium[^>]*>/i);if(!f){var g=getElement(c,/<div[^>]+panelMessage[^>]*>/i,replaceTagsAndSpaces,html_entity_decode);if(g&&/платежей не было/i.test(g)){AnyBalance.trace(g);b.payments=[];return}AnyBalance.trace(c);throw new AnyBalance.Error("Не найдена таблица платежей. Сайт изменен?");}f=replaceAll(f,[/<tfoot[^>]*>[\s\S]*?<\/tfoot>/i,""]);b.payments=[];processTable(f,b.payments,"payments.",{date:{re:/Дата/i,result_func:parseDate},sum:{re:/Сумма/i},
descr:{re:/Тип платежа/i,result_func:html_entity_decode}})}catch(l){AnyBalance.trace("Не удалось получить платежи: "+l.message+"\nStack: "+l.stack)}}
function turnOffLoginSMSNotify(){var a=AnyBalance.requestGet("https://profile.mts.ru/account",addHeaders({Referer:"https://login.mts.ru/"})),b=getParam(a,/\/_next\/static\/([\d\S]*?)\/_buildManifest\.js/i,replaceHtmlEntities);a&&b?(a=AnyBalance.requestGet("https://profile.mts.ru/_next/data/"+b+"/account/safety.json",addHeaders({Accept:"*/*",Referer:"https://profile.mts.ru/account"})),(b=getJson(a))&&b.pageProps?(b.pageProps.initialState&&b.pageProps.initialState.settings&&b.pageProps.initialState.settings.authuserlevel?
"1"!==b.pageProps.initialState.settings.authuserlevel?(AnyBalance.trace("SMS подтверждение для входа в кабинет включено. Выключаем..."),a=AnyBalance.requestPost("https://profile.mts.ru/api",JSON.stringify({call:"updateAuthUserLevel",arg:"1"}),addHeaders({"Content-Type":"application/json",Referer:"https://profile.mts.ru/account/safety/auth-level"})),a=getJson(a),AnyBalance.trace(JSON.stringify(a)),"success"!==a.result?AnyBalance.trace("Не удалось отключить SMS подтверждение для входа"):AnyBalance.trace("SMS подтверждение для входа успешно отключено")):
AnyBalance.trace("SMS подтверждение для входа уже отключено"):AnyBalance.trace("Не удалось получить настройки способа входа"),b.pageProps.initialState&&b.pageProps.initialState.smsNotification&&b.pageProps.initialState.smsNotification.notifications?(a=b.pageProps.initialState.smsNotification.notifications)&&!1!==a.lg_sms?(AnyBalance.trace("SMS уведомление о входе в кабинет включено. Выключаем..."),a.lg=!1,a.lg_sms=!1,a=AnyBalance.requestPost("https://profile.mts.ru/api",JSON.stringify({call:"patchSettingNotification",
arg:a}),addHeaders({"Content-Type":"application/json",Referer:"https://profile.mts.ru/account/safety/sms-notifications"})),AnyBalance.trace(a),/successfully/i.test(a)?AnyBalance.trace("SMS уведомление о входе успешно отключено"):AnyBalance.trace("Не удалось отключить SMS уведомление о входе")):AnyBalance.trace("SMS уведомление о входе уже отключено"):AnyBalance.trace("Не удалось получить настройки оповещения о входе")):AnyBalance.trace("Не удалось получить страницу настроек безопасности. Пропускаем проверку способа входа и оповещения о входе")):
