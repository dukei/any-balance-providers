var region_aliases={eao:"primorye",dv:"primorye"},regionsOrdinary={auto:"https://ihelper.mts.ru/selfcare/",center:"https://ihelper.mts.ru/selfcare/",primorye:"https://ihelper.dv.mts.ru/selfcare/",nnov:"https://ihelper.nnov.mts.ru/selfcare/",nw:"https://ihelper.nw.mts.ru/selfcare/",sib:"https://ihelper.sib.mts.ru/selfcare/",ural:"https://ihelper.nnov.mts.ru/selfcare/",ug:"https://ihelper.ug.mts.ru/selfcare/"},g_baseurl="https://lk.mts.ru",g_baseurlLogin="http://login.mts.ru",g_savedData,g_headers=
{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","Accept-Language":"ru-RU,ru;q=0.9",Connection:"keep-alive","Upgrade-Insecure-Requests":"1","User-Agent":"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36","If-Modified-Since":null},replaceNumber=[replaceTagsAndSpaces,/\D/g,"",/.*(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7 $1 $2-$3-$4"];
function getParamByName(a,b){return getParam(a,null,null,new RegExp("name=[\"']"+b+'["\'][^>]*value=["\']([^"\']+)"',"i"))}function isInOrdinary(a){return/<div[^>]+class="main-menu"/i.test(a)}function uniq_fast(a){for(var b={},c=[],d=a.length,e=0,l=0;l<d;l++){var h=a[l];1!==b[h]&&(b[h]=1,c[e++]=h)}return c}
...
replaceTagsAndSpaces,parseBalance)}function isLoggedIn(a){a=AnyBalance.getPreferences();a=AnyBalance.requestGet("https://lk.mts.ru/auth/account/is-authorized",addHeaders({Referer:"https://lk.mts.ru/","X-Login":"7"+a.login,"X-Requested-With":"XMLHttpRequest"}));return"true"===a}
function getLKJson(a,b){try{return getLKJsonNew(a)}catch(c){AnyBalance.trace(c.message),AnyBalance.trace("Не удалось найти Json с описанием пользователя первым способом, сайт изменен?")}try{return getLKJson2(a)}catch(c){AnyBalance.trace(c.message),AnyBalance.trace("Не удалось найти Json с описанием пользователя вторым способом, сайт изменен?")}try{return getLKJson1(a)}catch(c){if(b)throw c;AnyBalance.trace(c.message);AnyBalance.trace("Не удалось найти Json с описанием пользователя тремя способами, сайт изменен?");
json={}}return{}}
function getLKJsonNew(a){var b=AnyBalance.getPreferences();a=AnyBalance.requestGet("https://lk.mts.ru/auth/profile/full/legacy",addHeaders({Accept:"application/json, text/plain, */*",Referer:"https://lk.mts.ru/","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"}));var c=getJson(a),d={};isAnotherNumber()?(b=c["profile:slaves"]["profile:slaves:accounts"]["id="+b.phone+",ou=user,o=users,ou=services,dc=amroot"],d.fio=b["profile:name"],d.phone_formatted=(""+b["mobile:phone"]).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4"),
d.phone=""+b["mobile:phone"],d.license=""+b["mobile:terminal:id"],d.tariff=""+b["mobile:tariff"]):(d.fio=c["profile:name"],d.phone_formatted=(""+c["mobile:phone"]).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4"),d.phone=""+c["mobile:phone"],d.license=""+c["mobile:terminal:id"],d.tariff=""+c["mobile:tariff"]);b=callNewLKApiToken("accountInfo/mscpBalance");b=callNewLKApiResult(b);d.balance=getParam(Math.round(100*b.amount)/100);d.balance||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+
a);d.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+a);return d}
function getLKJson2(a){a=AnyBalance.requestGet("http://login.mts.ru/api/profile",addHeaders({Referer:g_baseurl+"/"}));var b=getJson(a),c={};c.fio=b["profile:name"];c.phone_formatted=(""+b["mobile:phone"]).replace(/(\d\d\d)(\d\d\d)(\d\d)(\d\d)$/,"+7($1)$2-$3-$4");c.phone=""+b["mobile:phone"];c.balance=Math.round(100*b["mobile:balance"])/100;c.tariff=""+b["mobile:tariff"];c.balance||AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+a);c.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+
a);return c}
function getLKJson1(a){a=AnyBalance.requestGet("https://login.mts.ru/profile/header?ref=https%3A//lk.mts.ru/&scheme=https&style=2015v2",addHeaders({Referer:g_baseurl+"/"}));var b={};b.fio=getElement(a,/<div[^>]+b-header_lk__name[^>]*>/i,replaceTagsAndSpaces);b.phone_formatted=getElement(a,/<div[^>]+b-header_lk__phone[^>]*>/i,replaceTagsAndSpaces);b.phone=replaceAll(b.phone_formatted,[/\+7/,"",/\D/g,""]);b.balance=getElement(a,/<div[^>]+b-header_balance[^>]*>/i,replaceTagsAndSpaces,parseBalance);b.balance||
AnyBalance.trace("Нулевой баланс! Возможно, что-то не так! "+a);b.phone||AnyBalance.trace("Не удаётся получить информацию о текущем пользователе. Сайт изменен?\n"+a);return b}function isAnotherNumber(){var a=AnyBalance.getPreferences();return a.phone&&a.phone!=a.login}
...
function enterLK(a){var b=enterMtsLK(a);b=checkLoginState(b,{automatic:!0});AnyBalance.trace("isLoggedIn(html) = "+isLoggedIn(b));if(isLoggedIn(b)){var c=function(){var e=AnyBalance.requestGet(g_baseurlLogin+"/amserver/UI/Logout",g_headers);if(isLoggedIn(e))throw AnyBalance.trace(e),new AnyBalance.Error("Не удаётся выйти из личного кабинета, чтобы зайти под правильным номером. Сайт изменен?");return e};AnyBalance.trace("Уже залогинены, проверяем, что на правильный номер...");var d=getLKJson(b).phone;
d?d!=a.login?(AnyBalance.trace("Залогинены на неправильный номер: "+d+", выходим"),b=c()):AnyBalance.trace("Залогинены на правильный номер: "+d):(AnyBalance.trace(b),AnyBalance.trace("Не удалось определить текущий номер в кабинете, сайт изменен? Попробуем выйти и зайти с логином-паролем."),b=c())}if(!isLoggedIn(b)){if(a.onlyAutomatic)throw new AnyBalance.Error("Ручной вход запрещен");b=enterMtsLK(joinObjects(a,{html:b,service:"lk",url:AnyBalance.getLastUrl()}));b=checkLoginState(b)}/no-config&arg=newsession/i.test(AnyBalance.getLastUrl())&&
enterLK2(b,a);isLoggedIn(b)||(AnyBalance.trace("Требуется дологиниться"),AnyBalance.requestGet("https://lk.mts.ru/auth/account/login?goto=https://lk.mts.ru",addHeaders({Referer:"https://lk.mts.ru/"})));if(!isLoggedIn(b)){if(getParam(b,null,null,/(auth-status=0)/i))throw new AnyBalance.Error("Неверный логин или пароль. Повторите попытку или получите новый пароль на сайте "+g_baseurl+"/.",!1,!0);AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в личный кабинет. Он изменился или проблемы на сайте.");
}__setLoginSuccessful();turnOffLoginSMSNotify();return b}
function loginWithPassword(){AnyBalance.trace("Пробуем войти в личный кабинет...");var a=AnyBalance.getPreferences();g_savedData||(g_savedData=new SavedData("mts",a.login));g_savedData.restoreCookies();g_savedData.set("csrfToken","");g_savedData.save();b=loadProtectedPage("https://profile.mts.ru/account",addHeaders({referer:"https://lk.mts.ru/"}));if(/\/account\/safety/i.test(b))AnyBalance.trace("Сессия сохранена. Входим автоматически...");else{AnyBalance.trace("Сессия новая. Будем логиниться заново...");
clearAllCookiesExceptProtection();var b=enterLK({login:a.login,password:a.password,baseurl:"http://lk.mts.ru",url:"http://login.mts.ru/amserver/UI/Login?service=lk&goto=http%3A%2F%2Flk.mts.ru%2F"})}g_savedData.setCookies();g_savedData.save();return b}
...
function processInfoLK(a,b){if(AnyBalance.isAvailable("balance","tariff","info.licschet","info.phone","info.fio"))try{var c=getLKJson(a,!0);AnyBalance.trace(JSON.stringify(c));getParam(c.balance,b,"balance");getParam(c.tariff,b,"tariff");AnyBalance.isAvailable("info")&&(b.info||(b.info={}),getParam(c.license,b.info,"info.licschet"),getParam(c.phone,b.info,"info.phone",null,replaceNumber),getParam(c.fio,b.info,"info.fio"))}catch(d){AnyBalance.trace("Не удалось получить данные о пользователе, скорее всего, виджет временно недоступен... "+
d.message+"\n"+d.stack)}}function parseTrafficFromKb(a){return parseTraffic(a,"kb")}function fetchNumber(){var a=AnyBalance.getPreferences();return a.phone&&a.phone!=a.login?"7"+a.phone:"7"+a.login}
function callNewLKApiToken(a,b){var c="GET",d="";if(b){var e=g_savedData.get("csrfToken");e||(getCsrfToken(),e=g_savedData.get("csrfToken"));d=JSON.stringify(b);c="POST";b=addHeaders({Accept:"application/json, text/plain, */*","Content-Type":"application/json","X-CSRF-TOKEN":e,"X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"})}else b=addHeaders({Accept:"application/json, text/plain, */*","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"});c=AnyBalance.requestPost("https://lk.mts.ru/api/"+
a,d,b,{HTTP_METHOD:c});d=getJson(c);if("string"!==typeof d)throw AnyBalance.trace(c),new AnyBalance.Error("Неверный ответ API "+a);return{token:d,verb:"api/"+a}}
function callNewLKApiResult(a){for(var b=0;30>b;++b){var c=AnyBalance.requestGet("https://lk.mts.ru/api/longtask/check/"+a.token+"?for="+a.verb,addHeaders({Accept:"application/json, text/plain, */*","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"}));AnyBalance.trace("Ожидание результата "+a.verb+" "+(b+1)+"/30");if(400<=AnyBalance.getLastStatusCode())throw AnyBalance.trace("Waiting for result error: "+c),new AnyBalance.Error(c);if(c)return b=getJson(c),AnyBalance.trace("Получен результат "+
a.verb+" от "+b.refreshDate),b.data||b;AnyBalance.sleep(2E3)}throw new AnyBalance.Error("Не удалось получить результат "+a.verb);}
function callNewLKApiData(a,b){var c="GET",d="";if(b){var e=g_savedData.get("csrfToken");e||(getCsrfToken(),e=g_savedData.get("csrfToken"));d=JSON.stringify(b);c="POST";b=addHeaders({Accept:"application/json, text/plain, */*","Content-Type":"application/json","X-CSRF-TOKEN":e,"X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"})}else b=addHeaders({Accept:"application/json, text/plain, */*","X-Login":fetchNumber(),"X-Requested-With":"XMLHttpRequest"});c=AnyBalance.requestPost("https://lk.mts.ru/api/"+
a,d,b,{HTTP_METHOD:c});c=getJson(c);AnyBalance.trace("Получен результат api/"+a+" от "+c.refreshDate);return c.data||c}
function processCountersLK(a){a.remainders||(a.remainders={});var b=callNewLKApiToken("sharing/counters");b=callNewLKApiResult(b);if(b.counters&&3>b.counters.length){var c=0;do AnyBalance.trace("Получены не все остатки. Повторная попытка "+(c+1)+"/5"),AnyBalance.sleep(2E3),b=callNewLKApiToken("sharing/counters"),b=callNewLKApiResult(b);while(3>b.counters.length&&5>++c)}for(c=0;c<b.counters.length;++c){var d=b.counters[c];if(/Входящ/i.test(d.name))AnyBalance.trace("Найден счетчик "+d.name+" ("+d.packageType+
"). Это посуточный дополнительный пакет. Пропускаем...");else if(AnyBalance.trace("Найден счетчик "+d.name+" ("+d.packageType+")"),"Calling"===d.packageType){AnyBalance.trace("Это минуты");var e=1;"Second"===d.unitType&&(e=60);getParam(d.deadlineDate,a.remainders,"remainders.min_till",null,null,parseDateISO);getParam(d.usedAmount/e,a.remainders,"remainders.min_local");getParam((d.totalAmount-d.usedAmount)/e,a.remainders,"remainders.min_left")}else"Messaging"===d.packageType?(AnyBalance.trace("Это сообщения"),
getParam(d.deadlineDate,a.remainders,"remainders.sms_till",null,null,parseDateISO),getParam(d.usedAmount,a.remainders,"remainders.sms_used"),getParam(d.totalAmount-d.usedAmount,a.remainders,"remainders.sms_left")):"Internet"===d.packageType?(AnyBalance.trace("Это трафик"),getParam(d.deadlineDate,a.remainders,"remainders.traffic_left_till",null,null,parseDateISO),getParam(d.usedAmount+d.unitType,a.remainders,"remainders.traffic_used_mb",null,null,parseTraffic),getParam(d.usedByAcceptors+d.unitType,
a.remainders,"remainders.traffic_used_by_acceptors_mb",null,null,parseTraffic),getParam(d.totalAmount-d.usedAmount+d.unitType,a.remainders,"remainders.traffic_left_mb",null,null,parseTraffic)):AnyBalance.trace("Неизвестный счетчик: "+JSON.stringify(d))}b=callNewLKApiToken("cashback/account");b=callNewLKApiResult(b);getParam(b.balance,a.remainders,"remainders.cashback");b=callNewLKApiToken("services/list/active");b=callNewLKApiResult(b);getParam({Unblocked:"Номер не блокирован",OnlyInboundCalls:"Частичная блокировка",
BlockDueToInsufficiencyOfMoney:"Номер заблокирован",Blocked:"Номер заблокирован"}[b.accountBlockStatus]||b.accountBlockStatus,a.remainders,"remainders.statuslock");getParam(b.services.length,a.remainders,"remainders.services");getParam(0,a.remainders,"remainders.services_free");getParam(0,a.remainders,"remainders.services_paid");getParam(0,a.remainders,"remainders.services_abon");for(c=0;c<b.services.length;++c)if(d=b.services[c],AnyBalance.trace("Найдена услуга "+d.name),!1===d.isSubscriptionFee)AnyBalance.trace("Это бесплатная услуга"),
sumParam(1,a.remainders,"remainders.services_free",null,null,parseBalanceSilent,aggregate_sum);else if(!0===d.isSubscriptionFee){AnyBalance.trace("Это платная услуга");e=new Date;sumParam(1,a.remainders,"remainders.services_paid",null,null,parseBalanceSilent,aggregate_sum);if("Month"!==d.primarySubscriptionFee.unitOfMeasure)(new Date(e.getFullYear(),e.getMonth()+1,0)).getDate();else var l=1;AnyBalance.trace("Платная услуга "+d.name+": "+d.primarySubscriptionFee.quotaPeriodicity+" "+d.primarySubscriptionFee.value+
" ₽");sumParam(d.primarySubscriptionFee.value*l,a.remainders,"remainders.services_abon",null,null,parseBalanceSilent,aggregate_sum)}else AnyBalance.trace("Неизвестный тип услуги: "+JSON.stringify(d));b=callNewLKApiToken("creditLimit");b=callNewLKApiResult(b);getParam(b.currentCreditLimitValue,a.remainders,"remainders.credit");isAvailable(["expenses.usedinthismonth","expenses.usedinprevmonth","expenses.abonservice","expenses.refill"])&&processExpensesLK(a);isAvailable(["payments.sum","payments.date"])&&
processTransactionsLK(a)}function getCsrfToken(){AnyBalance.requestGet("https://lk.mts.ru/api/login/user-info",addHeaders({Accept:"application/json, text/plain, */*","X-Requested-With":"XMLHttpRequest"}));var a=AnyBalance.getLastResponseHeader("X-Refresh-CSRF-Token");a||AnyBalance.trace("Не удалось получить csrfToken");g_savedData.set("csrfToken",a);g_savedData.save()}
function processExpensesLK(a){a.expenses||(a.expenses={});var b=new Date,c=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-01T00:00:00+03:00";b=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-"+n2(b.getDate())+"T23:59:59+03:00";c=callNewLKApiToken("billHistory/summary",{dateFrom:c,dateTo:b,transactionFilter:{pageNumber:0,filterTypes:{},category:null}});c=callNewLKApiResult(c);getParam(c.billingTotal,a.expenses,"expenses.usedinthismonth");if(c.categories&&0<c.categories.length)for(b=0;c.categories&&b<c.categories.length;++b){var d=
c.categories[b];AnyBalance.trace('Найдена категория "'+d.name+'" ');/Абонентская плата/i.test(d.name)?getParam(d.totalAmount,a.expenses,"expenses.abonservice"):/Пополнения/i.test(d.name)&&getParam(d.totalAmount,a.expenses,"expenses.refill")}else AnyBalance.trace("Не удалось получить данные по расходам");b=new Date;b=new Date(b.getFullYear(),b.getMonth(),0);c=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-01T00:00:00+03:00";b=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-"+n2(b.getDate())+"T23:59:59+03:00";c=
callNewLKApiToken("billHistory/summary",{dateFrom:c,dateTo:b,transactionFilter:{pageNumber:0,filterTypes:{},category:null}});c=callNewLKApiResult(c);getParam(c.billingTotal,a.expenses,"expenses.usedinprevmonth")}
function processTransactionsLK(a){a.payments||(a.payments={});var b=new Date,c=new Date(b.getFullYear(),b.getMonth()-6,b.getDate());c=c.getFullYear()+"-"+n2(c.getMonth()+1)+"-"+n2(c.getDate())+"T00:00:00+03:00";b=b.getFullYear()+"-"+n2(b.getMonth()+1)+"-"+n2(b.getDate())+"T23:59:59+03:00";b=callNewLKApiData("billHistory/transactionPage",{dateFrom:c,dateTo:b,transactionFilter:{category:"refill",pageNumber:0,types:["Incomes"]}});if(b.transactions&&0<b.transactions.length)for(;b.transactions&&0<b.transactions.length;){b=
b.transactions[0];getParam(b.amount,a.payments,"payments.sum");getParam(b.dateTime.replace(/(\d{4})-(\d{2})-(\d{2}).*/,"$3.$2.$1"),a.payments,"payments.date",null,null,parseDate);getParam(b.label,a.payments,"payments.descr");break}else AnyBalance.trace("Не удалось получить данные последней транзакции")}
function mainLK(a,b){var c=AnyBalance.getPreferences();AnyBalance.trace("Мы в личном кабинете...");if(isAnotherNumber()){AnyBalance.trace("Пробуем переключиться на номер "+c.phone);a=AnyBalance.requestGet("https://lk.mts.ru/auth/profile/full/legacy",g_headers);var d=getJson(a)["profile:slaves"]["profile:slaves:accounts"]["id="+c.phone+",ou=user,o=users,ou=services,dc=amroot"];if(!d)throw new AnyBalance.Error("Не удалось переключиться на номер "+c.phone+". Вероятно, он не принадлежит логину "+c.login);
if(d=d.login)a=AnyBalance.requestGet(d,g_headers),a=getParam(a,null,null,/antiForgeryToken = "([^"]*)/i),a=AnyBalance.requestPost(d,null,addHeaders({Accept:"application/json, text/plain, */*",Origin:"https://lk.mts.ru",Referer:"https://lk.mts.ru/","X-CSRF-TOKEN":a,"X-Login":"7"+c.login,"X-Requested-With":"XMLHttpRequest"}));else throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось переключиться на другой номер. Сайт изменен?");}processInfoLK(a,b);try{processCountersLK(b)}catch(e){AnyBalance.trace("Ошибка получения остатков из кабинета. Попробуйте ещё раз позднее. Ошибка: "+
e.message),b.were_errors=!0}}function sleep(a){AnyBalance.trace("Sleeping "+a+" ms");AnyBalance.sleep(a)}function parseBalanceRound(a){a=parseBalance(a);isset(a)&&(a=Math.round(100*a)/100);return a}
...
function processPayments(a,b){try{AnyBalance.trace("Получаем платежи...");var c=AnyBalance.requestGet(a+"payment-full-history.aspx",g_headers);checkIHError(c,!0);var d=getElement(c,/<form[^>]+aspnetForm[^>]*>/i),e=new Date,l=new Date(e.getFullYear()-1,e.getMonth(),e.getDate()),h=createFormParams(d,function(g,p,m,n){return/from$/i.test(m)?fmtDate(l):/to$/i.test(m)?fmtDate(e):n});setCsrfCookie(a,c);c=AnyBalance.requestPost(a+"payment-full-history.aspx",h,addHeaders({Referer:AnyBalance.getLastUrl()}));
checkIHError(c,!0);var k=getElement(c,/<div[^>]+paymentsGridium[^>]*>/i);if(!k){var f=getElement(c,/<div[^>]+panelMessage[^>]*>/i,replaceTagsAndSpaces,html_entity_decode);if(f&&/платежей не было/i.test(f)){AnyBalance.trace(f);b.payments=[];return}AnyBalance.trace(c);throw new AnyBalance.Error("Не найдена таблица платежей. Сайт изменен?");}k=replaceAll(k,[/<tfoot[^>]*>[\s\S]*?<\/tfoot>/i,""]);b.payments=[];processTable(k,b.payments,"payments.",{date:{re:/Дата/i,result_func:parseDate},sum:{re:/Сумма/i},
descr:{re:/Тип платежа/i,result_func:html_entity_decode}})}catch(g){AnyBalance.trace("Не удалось получить платежи: "+g.message+"\nStack: "+g.stack)}}
function enterLK2(a,b){AnyBalance.getPreferences();var c=b.login.replace(/.*(\d{3})(\d{3})(\d{2})(\d{2})$/i,"$1 $2-$3-$4");if(/captchaForm/i.test(a)){AnyBalance.trace("МТС затребовал капчу");var d=getElement(a,/<form[^>]+captchaForm[^>]*>/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось найти форму для ввода капчи. Сайт изменен?");a=getParam(d,/data:image\/\w+?(?:png)?;base64,([^"]+)/i);var e=AnyBalance.retrieveCode("МТС требует ввести капчу для входа в личный кабинет, чтобы подтвердить, что вы не робот. Пожалуйста, введите символы с картинки",
a);a=createFormParams(d,function(h,k,f,g){"IDToken2"==f?g=e:"noscript"==f&&(g=void 0);return g});a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",a,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}));if(!a||400<AnyBalance.getLastStatusCode())throw new AnyBalance.Error("Личный кабинет МТС временно недоступен. Попробуйте ещё раз позже");if(/captchaForm/i.test(a)){d=getElement(a,/<form[^>]+captchaForm[^>]*>/i);
if(d=getParam(d,null,null,/class="errorText"[\s\S]*?data-error="*>?([\s\S]*?)[">]*?<\/p>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(d,null,/код/i.test(d));AnyBalance.trace(a);throw new AnyBalance.Error("Не удалось войти в личный кабинет. Сайт изменен?");}}if(/loginForm/i.test(a)){d=getElement(a,/<form[^>]+loginForm[^>]*>/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось найти форму для ввода номера. Сайт изменен?");a=createFormParams(d,function(h,k,f,g){"login"==f?g=c:"IDToken1"==
f?g=b.login:"IDToken2"==f?g=b.password:"noscript"==f&&(g=void 0);return g});a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",a,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}))}d=getElement(a,/<form[^>]*>/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось найти форму для ввода devicePrint. Сайт изменен?");a=createFormParams(d,function(h,k,f,g){"login"==f?g=c:"IDToken1"==
f?g=b.login:"IDToken2"==f?g='{"screen":{"screenWidth":1600,"screenHeight":900,"screenColourDepth":24},"userAgent":"Mozilla/5.0+(Windows+NT+10.0;+WOW64)+AppleWebKit/537.36+(KHTML,+like+Gecko)+Chrome/107.0.0.0+Safari/537.36","platform":"Win32","language":"ru","timezone":{"timezone":-180},"plugins":{"installedPlugins":"internal-pdf-viewer;internal-pdf-viewer;internal-pdf-viewer;internal-pdf-viewer;internal-pdf-viewer;"},"fonts":{"installedFonts":"cursive;monospace;serif;sans-serif;fantasy;default;Arial;Arial+Black;Arial+Narrow;Bookman+Old+Style;Bradley+Hand+ITC;Century;Century+Gothic;Comic+Sans+MS;Courier;Courier+New;Georgia;Impact;Lucida+Console;Monotype+Corsiva;Papyrus;Tahoma;Times;Times+New+Roman;Trebuchet+MS;Verdana;"},"appName":"Netscape","appCodeName":"Mozilla","appVersion":"5.0+(Windows+NT+10.0;+WOW64)+AppleWebKit/537.36+(KHTML,+like+Gecko)+Chrome/107.0.0.0+Safari/537.36","product":"Gecko","productSub":"20030107","vendor":"Google+Inc."}':
...
null,!0);if(/passwordForm/i.test(a)){d=getElement(a,/<form[^>]+passwordForm[^>]*>/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось найти форму для ввода пароля. Сайт изменен?");a=createFormParams(d,function(h,k,f,g){"login"==f?g=c:"IDToken1"==f?g=b.login:"IDToken2"==f?g=b.password:"noscript"==f&&(g=void 0);return g});a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",a,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}));
if(/passwordForm/i.test(a)){d=getElement(a,/<form[^>]+passwordForm[^>]*>/i);if(d=getParam(d,null,null,/class="errorText"[\s\S]*?data-error="*>?([\s\S]*?)[">]*?<\/p>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(d,null,/парол/i.test(d));AnyBalance.trace(a);throw new AnyBalance.Error("Не удалось войти в личный кабинет. Сайт изменен?");}}if(/codeCheckForm/i.test(a)){AnyBalance.trace("МТС запросил проверку с помощью кода из SMS");var l=AnyBalance.retrieveCode("Пожалуйста, введите код подтверждения, высланный на номер +7 "+
c+'.\n\nЕсли вы не хотите постоянно вводить SMS-пароли при входе, выберите способ входа "Пароль" в настройках безопасности вашего личного кабинета МТС',null,{inputType:"number",time:3E5});d=getElement(a,/<form[^>]+codeCheckForm[^>]*>/i);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось найти форму для ввода кода из СМС. Сайт изменен?");a=createFormParams(d,function(h,k,f,g){"IDToken1"==f?g=l:"noscript"==f&&(g=void 0);return g});a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",
a,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}));if(/codeCheckForm/i.test(a)){d=getElement(a,/<form[^>]+codeCheckForm[^>]*>/i);if(d=getParam(d,null,null,/class[^>]+errorText"[\s\S]*?oneLine"*>?([\s\S]*?)[">]*?<\/span>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(d,null,/код/i.test(d));AnyBalance.trace(a);throw new AnyBalance.Error("Не удалось войти в личный кабинет. Сайт изменен?");}}d=getElement(a,/<form[^>]+gaForm[^>]*>/i);
if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось найти форму входа. Сайт изменен?");a=createFormParams(d,function(h,k,f,g){"login"==f?g=c:"IDToken1"==f?g=b.login:"IDToken2"==f?g=b.password:"noscript"==f&&(g=void 0);return g});return a=AnyBalance.requestPost("https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession",a,addHeaders({Origin:"https://login.mts.ru",Referer:"https://login.mts.ru/amserver/UI/Login?no-config&arg=newsession"}))}
function turnOffLoginSMSNotify(){var a=AnyBalance.requestGet("https://profile.mts.ru/account",addHeaders({Referer:"https://login.mts.ru/"})),b=getParam(a,/\/_next\/static\/([\d\S]*?)\/_buildManifest\.js/i,replaceHtmlEntities);a&&b?(a=AnyBalance.requestGet("https://profile.mts.ru/_next/data/"+b+"/account/safety.json",addHeaders({Accept:"*/*",Referer:"https://profile.mts.ru/account"})),(a=getJson(a))&&a.pageProps?"1"!==a.pageProps.initialState.settings.authuserlevel?(AnyBalance.trace("СМС для входа в кабинет включено. Выключаем..."),
a=AnyBalance.requestPost("https://profile.mts.ru/api",JSON.stringify({call:"updateAuthUserLevel",arg:"1"}),addHeaders({"Content-Type":"application/json",Referer:"https://profile.mts.ru/account/safety/auth-level"})),a=getJson(a),AnyBalance.trace(JSON.stringify(a)),"success"!==a.result?AnyBalance.trace("Не удалось отключить СМС для входа"):AnyBalance.trace("СМС для входа успешно отключено")):AnyBalance.trace("СМС для входа уже отключено"):AnyBalance.trace("Не удалось получить страницу настроек безопасности. Пропускаем проверку способа входа")):
