var g_headers={Connection:"keep-alive","Tele2-User-Agent":'"mytele2-app/3.17.0"; "unknown"; "Android/9"; "Build/12998710"',"X-API-Version":"1","User-Agent":"okhttp/4.2.0"},baseurl="https://api.tele2.ru/";
...
function loginByAccessToken(){var a=AnyBalance.getPreferences();if(a.login!==AnyBalance.getData("login"))return!1;g_headers.Authorization="Bearer "+AnyBalance.getData("ac");try{callApi("https://sso.tele2.ru/auth/realms/tele2-b2c/protocol/openid-connect/userinfo")}catch(b){return AnyBalance.trace("Не удалось войти по access token: "+b.message),!1}AnyBalance.trace("Вошли по access token");return!0}
...
d)}}}
function getDiscount(a,b){var c=b.uom;AnyBalance.trace("Найден дискаунт: "+b.service.name+" ("+c+")");if(0!==b.limit)if(getParam(b.endDay,a,"remainders.endDate",null,null,parseDateISO),/min/i.test(c))sumParam(Math.round(100*b.remain)/100,a,"remainders.min_left",null,null,null,aggregate_sum),sumParam(Math.round(100*(b.limit-b.remain)/100),a,"remainders.min_used",null,null,null,aggregate_sum),sumParam(b.endDay||void 0,a,"remainders.min_till",null,null,parseDateISO,aggregate_min);else if(/[кмгkmg][bб]/i.test(c)){var c=parseTraffic(b.remain+
b.uom),d=parseTraffic(b.limit+b.uom);sumParam(c,a,"remainders.traffic_left",null,null,null,aggregate_sum);sumParam(d-c,a,"remainders.traffic_used",null,null,null,aggregate_sum);sumParam(b.endDay||void 0,a,"remainders.traffic_till",null,null,parseDateISO,aggregate_min)}else/pcs/i.test(c)?(sumParam(b.remain,a,"remainders.sms_left",null,null,null,aggregate_sum),sumParam(b.limit-b.remain,a,"remainders.sms_used",null,null,null,aggregate_sum),sumParam(b.endDay||void 0,a,"remainders.sms_till",null,null,
parseDateISO,aggregate_min)):AnyBalance.trace("Неизвестный дискаунт: "+JSON.stringify(b))}
function processPayments(a){if(AnyBalance.isAvailable("payments")){AnyBalance.trace("Searching for payments");var b=getSubscriberId();try{var c=AnyBalance.requestGet(baseurl+"api/subscribers/"+b+"/payments?fromDate="+getFormattedDate({format:"YYYY-MM-DD",offsetMonth:3})+"T00%3A00%3A00%2B03%3A00&toDate="+getFormattedDate({format:"YYYY-MM-DD"})+"T23%3A59%3A59%2B03%3A00",g_headers),d=getJson(c)}catch(f){}if(d&&d.data)for(AnyBalance.trace("History json: "+JSON.stringify(d)),a.payments=[],b=0;b<d.data.length;++b){var c=
d.data[b],e={};getParam(c.sum.amount,e,"payments.sum",null,null,parseBalanceSilent);getParam(c.type,e,"payments.descr");getParam(c.payDate,e,"payments.date",null,null,parseDateISO);a.payments.push(e)}else AnyBalance.trace(c),AnyBalance.trace("Не удалось получить последние платежи, может их просто нет?")}}
function processInfo(a){if(AnyBalance.isAvailable("info")){var b=getSubscriberId();a=a.info={};var c=AnyBalance.requestGet(baseurl+"api/subscribers/"+b+"/profile",addHeaders({Accept:"*/*","X-Requested-With":"XMLHttpRequest",Referer:baseurl})),c=getJson(c);getParam(c.data.fullName,a,"info.fio",/<div[^>]+class="user-name"[^>]*>([\s\S]*?)(?:<\/div>|<a)/i,replaceTagsAndSpaces,capitalFirstLetters);getParam(b,a,"info.mphone");getParam(c.data.address.city+" "+c.data.address.street+" "+c.data.address.house,
a,"info.address");getParam(c.data.email||void 0,a,"info.email")}};
