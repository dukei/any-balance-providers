...
function processRemainders(a){if(AnyBalance.isAvailable("remainders")){AnyBalance.trace("Получаем остатки услуг");try{a.remainders||(a.remainders={});var b=getSubscriberId();AnyBalance.trace("Searching for resources left");var c=AnyBalance.requestGet(baseurl+"api/subscribers/"+b+"/rests",addHeaders({Accept:"*/*","X-Requested-With":"XMLHttpRequest",Referer:baseurl}));AnyBalance.trace("Got discounts: "+c);json=JSON.parse(c);getParam({active:"Номер не блокирован",blocked:"Номер заблокирован"}[json.data.tariffStatus]||
json.data.tariffStatus,a.remainders,"remainders.statuslock");for(b=0;b<json.data.rests.length;++b)getDiscount(a.remainders,json.data.rests[b])}catch(d){AnyBalance.trace("Не удалось получить данные об остатках пакетов и услуг, попробуйте позже "+d)}}}
function getDiscount(a,b){var c=b.uom;AnyBalance.trace("Найден дискаунт: "+b.service.name+" ("+c+")");if(0!==b.limit)if(getParam(b.endDay,a,"remainders.endDate",null,null,parseDateISO),/min/i.test(c))sumParam(Math.round(100*b.remain)/100,a,"remainders.min_left",null,null,null,aggregate_sum),sumParam(Math.round(100*b.limit/100),a,"remainders.min_total",null,null,null,aggregate_sum),sumParam(Math.round(100*(b.limit-b.remain)/100),a,"remainders.min_used",null,null,null,aggregate_sum),sumParam(b.endDay||
void 0,a,"remainders.min_till",null,null,parseDateISO,aggregate_min);else if(/[кмгkmg][bб]/i.test(c)){c=parseTraffic(b.remain+b.uom);var d=parseTraffic(b.limit+b.uom);sumParam(c,a,"remainders.traffic_left",null,null,null,aggregate_sum);sumParam(d,a,"remainders.traffic_total",null,null,null,aggregate_sum);sumParam(d-c,a,"remainders.traffic_used",null,null,null,aggregate_sum);sumParam(b.endDay||void 0,a,"remainders.traffic_till",null,null,parseDateISO,aggregate_min)}else/pcs/i.test(c)?(sumParam(b.remain,
a,"remainders.sms_left",null,null,null,aggregate_sum),sumParam(b.limit,a,"remainders.sms_total",null,null,null,aggregate_sum),sumParam(b.limit-b.remain,a,"remainders.sms_used",null,null,null,aggregate_sum),sumParam(b.endDay||void 0,a,"remainders.sms_till",null,null,parseDateISO,aggregate_min)):AnyBalance.trace("Неизвестный дискаунт: "+JSON.stringify(b))}
