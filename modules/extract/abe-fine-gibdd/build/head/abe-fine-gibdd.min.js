...
", "+b.password);a=AnyBalance.requestPost("http://www.gibdd.ru/proxy/check/fines/2.0/client.php",a,addHeaders({"X-Requested-With":"XMLHttpRequest",Referer:"http://www.gibdd.ru/check/fines/","Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",Connection:"keep-alive"}));return getGibddJson(a)}
function parseFines(b,a){if(200!=a.status){if(a.error)throw new AnyBalance.Error(a.error);if(1==a.status)throw new AnyBalance.Error("Цифры с картинки введены не верно!");AnyBalance.trace(JSON.stringify(a));throw new AnyBalance.Error("Не удалось получить данные по штрафам, сайт изменен?");}if(!a.request||"0"!=a.request.error)throw new AnyBalance.Error("Указанное Вами свидетельство о регистрации транспортного средства не соответствует государственным регистрационным знакам или более недействительно. Вероятно, Вами допущена ошибка при заполнении полей запроса.");
AnyBalance.trace("Штрафов: "+a.request.count);if(0<a.request.count){b.fines=[];for(var c=0;c<a.request.count;c++){var d=a.request.data[c],e={__id:d.id,__name:d.NumPost};__shouldProcess("fines",e)&&(getParam(d.Summa+"",e,"fines.summ",null,null,parseBalance),getParam(d.SummaDiscount+"",e,"fines.summDiscount",null,null,parseBalance),getParam(d.DateDecis+"",e,"fines.date",null,replaceTagsAndSpaces,parseDateGibdd),getParam(d.DateDiscount+"",e,"fines.dateDiscount",null,replaceTagsAndSpaces,parseDateGibdd),
getParam(d.DatePost+"",e,"fines.datePost",null,replaceTagsAndSpaces,parseDateGibdd),getParam(d.KoAPcode,e,"fines.koap",null,replaceTagsAndSpaces),getParam(d.KoAPtext.toUpperCase().substring(0,1)+d.KoAPtext.toLowerCase().substring(1),e,"fines.descr",null,replaceTagsAndSpaces),getParam(d.NumPost,e,"fines.postanovlenie",null,replaceTagsAndSpaces),getParam(a.request.cacheDiv[d.Division],e,"fines.podrazdel",null,replaceTagsAndSpaces));b.fines.push(e);sumParam(d.Summa+"",b,"balance",null,null,parseBalance,
aggregate_sum)}getParam(a.request.count+"",b,"count",null,null,parseBalance)}else b.descr="Неуплаченных штрафов в федеральной информационной системе ГИБДД по указанным данным не найдено.",b.count=0}function parseDateGibdd(b){var a=/(\d{4})\D(\d{2})\D(\d{2})(?:\D(\d{1,2}):(\d{1,2}):(\d{1,2}))?/.exec(b);if(a){var a=new Date(a[1],a[2]-1,+a[3],a[4]||0,a[5]||0,a[6]||0),c=a.getTime();AnyBalance.trace("Parsing date "+a+" from value: "+b);return c}AnyBalance.trace("Failed to parse date from value: "+b)};
