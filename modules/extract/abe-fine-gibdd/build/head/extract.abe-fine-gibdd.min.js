var g_headers={Accept:"application/json, text/javascript, */*; q=0.01","Accept-Language":"ru,en;q=0.8",Connection:"keep-alive",Origin:"http://www.gibdd.ru","User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4298.0 Safari/537.36"};
...
var a=/(\D{0,1}\d+\D{2})(\d{2,3})/i.exec(b.login);if(!a)throw new AnyBalance.Error("Номер должен быть в формате а123вс190 либо 1234ав199, буквы русские.");var d=solveRecaptcha("Пожалуйста, докажите, что вы не робот",AnyBalance.getLastUrl(),JSON.stringify({SITEKEY:"6Lc66nwUAAAAANZvAnT-OK4f4D_xkdzw5MLtAYFL",TYPE:"V3",ACTION:"check_fines",USERAGENT:g_headers["User-Agent"]}));a=[["regnum",a[1].toUpperCase()],["regreg",a[2]],["stsnum",b.password.toUpperCase()],["captchaWord",""],["reCaptchaToken",d]];
AnyBalance.trace("Пробуем запросить информацию с данными: "+b.login+", "+b.password);b=AnyBalance.requestPost("https://xn--b1afk4ade.xn--90adear.xn--p1ai/proxy/check/fines",a,addHeaders({"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",Referer:"https://xn--90adear.xn--p1ai/check/fines/",Connection:"keep-alive"}));return getGibddJson(b)}
function parseFines(b,a){var d=a.code||a.status;if(200!=d){if(b=a.error||a.message)throw new AnyBalance.Error(b,null,404==d);AnyBalance.trace(JSON.stringify(a));throw new AnyBalance.Error("Не удалось получить данные по штрафам, сайт изменен?");}if(!a.data)throw AnyBalance.trace(JSON.stringify(a)),new AnyBalance.Error("Вероятно, Вами допущена ошибка при заполнении полей запроса.");AnyBalance.trace("Штрафов: "+a.data.length);if(0<a.data.length){b.fines=[];for(d=0;d<a.data.length;d++){var c=a.data[d],
...
e,"fines.descr"),getParam(c.NumPost,e,"fines.postanovlenie"),getParam(a.divisions[c.Division].name,e,"fines.podrazdel"));b.fines.push(e);sumParam(c.Summa+"",b,"balance",null,null,parseBalance,aggregate_sum)}getParam(a.data.length,b,"count")}else b.descr="Штрафов нет.",b.count=0}
function parseDateGibdd(b){var a=/(\d{4})\D(\d{2})\D(\d{2})(?:\D(\d{1,2}):(\d{1,2}):(\d{1,2}))?/.exec(b);if(a){a=new Date(a[1],a[2]-1,+a[3],a[4]||0,a[5]||0,a[6]||0);var d=a.getTime();AnyBalance.trace("Parsing date "+a+" from value: "+b);return d}AnyBalance.trace("Failed to parse date from value: "+b)};
