...
function processProfile(a,c){isAvailable("profile")&&(a=AnyBalance.requestGet(baseurl+"main?main=priv&replace=profile_view&item=0",g_headers),c.profile={},getParam(a,c.profile,"profile.fio",/ФИО((?:[\s\S]*?<\/div[^>]*>){2})/i,replaceTagsAndSpaces),getParam(a,c.profile,"profile.birthday",/Дата рождения((?:[\s\S]*?<\/div[^>]*>){2})/i,replaceTagsAndSpaces),getParam(a,c.profile,"profile.phone",/Телефон((?:[\s\S]*?<\/div[^>]*>){2})/i,replaceTagsAndSpaces),getParam(a,c.profile,"profile.adress",/Адрес регистрации((?:[\s\S]*?<\/div[^>]*>){2})/i,
...
function requestGetWicketAction(a,c,b){var d=getParam(a,null,null,c);if(!d)throw AnyBalance.trace(a),new AnyBalance.Error("Не нашли wicketId "+c.source);a=findWicketActions(a);d=findExactWickeAction(a,d);return b?AnyBalance.requestPost(baseurl+d+"&_="+(new Date).getTime(),b,addHeaders(g_Xml_Headers)):AnyBalance.requestGet(baseurl+d+"&_="+(new Date).getTime(),addHeaders(g_Xml_Headers))}
...
f=getParam(b[d],null,null,/"card-name"[^>]*>([\s\S]*?)<\//i,replaceTagsAndSpaces),e={__id:e,__name:f+" "+e};__shouldProcess("cards",e)&&processCard(b[d],e,a);c.cards.push(e)}}}
...
function processCredit(a,c,b){getParam(a,c,"credits.balance",/class="sum"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces,parseBalance);getParam(a,c,["credits.currency","credits"],/class="sum"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces,parseCurrency);getParam(a,c,"credits.minpay_till",/К оплате([\s\S]*?)<\//i,replaceTagsAndSpaces,parseDate);getParam(a,c,"credits.minpay",/К оплате[\s\S]*?class="sum"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces,parseBalance);b=requestGetWicketAction(b,/<div[^>]+class="account inner[^>]+id="(id[^"]+)"/i);
getParam(b,c,"credits.debt_main",/Общая задолженность(?:[^>]*>){3}([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.principal_debt",/Сумма основного долга(?:[^>]*>){3}([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.pct_sum",/Cумма начисленных процентов(?:[^>]*>){3}([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.debt_expired",/Сумма просроченного основного долга(?:[^>]*>){3}([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,
parseBalance);getParam(b,c,"credits.pct_expired",/Сумма просроченных процентов(?:[^>]*>){3}([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.penalty",/Штрафы(?:[^>]*>){3}([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.minpay_main_debt",/К оплате[\s\S]*?Сумма основного долга([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.minpay_pct",/К оплате[\s\S]*?Сумма процентов([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);
getParam(b,c,"credits.minpay_others",/К оплате[\s\S]*?Другие комиссии([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.minpay_penalty",/К оплате[\s\S]*?Пеня за([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.minpay_pct_expired",/К оплате[\s\S]*?Сумма просроченных процентов([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(b,c,"credits.minpay_debt_expried",/К оплате[\s\S]*?Сумма просроченного основного долга([\s\S]*?)<\/span>/i,
replaceTagsAndSpaces,parseBalance);"undefined"!=typeof processCreditSchedule&&processCreditSchedule(b,c)};
