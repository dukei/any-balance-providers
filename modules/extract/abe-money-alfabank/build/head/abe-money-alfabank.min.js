...
function login(b){AnyBalance.setOptions({cookiePolicy:"rfc2109"});AnyBalance.setDefaultCharset("utf-8");var c=AnyBalance.getPreferences();if(g_mainHtml)return g_mainHtml;checkEmpty(c.login,"Введите логин!");checkEmpty(c.password,"Введите пароль!");return processClick(b)}var g_mainHtmlIs20;function isClick20(){isset(g_mainHtmlIs20)||(g_mainHtmlIs20=/\(C2skin\)/.test(g_mainHtml));return g_mainHtmlIs20}
function processClick(b){function c(a,b,c,e){return"pt1:password_key"==c?(a=getParam(d,null,null,/<span[^>]+id="[^>]*_(?:token|sms)"(?:[^>](?!display\s*:\s*none))*>([\s\S]*?)(?:<\/span>|<div)/i,replaceTagsAndSpaces),AnyBalance.trace("Затребован код на вход: "+a),AnyBalance.retrieveCode((a||"Для входа в интернет-банк требуется одноразовый код. Введите его в поле ниже.")+"\n\nЧтобы не вводить код всегда при входе в интернет-банк, вы можете отключить требование одноразового кода на вход в настройках Альфа-Клик. Это безопасно, отменяет только подтверждение входа. Подтверждение транзакций всё равно будет требоваться обязательно.",
null,{time:18E4,inputType:"number"})):e}var a=AnyBalance.getPreferences(),d=AnyBalance.requestGet(g_baseurl+"/ALFAIBSR/",g_headers),d=AnyBalance.requestPost(g_baseurl+"/AlfaSign/security",{username:a.login,password:a.password.substr(0,16)},g_headers),d=AnyBalance.requestPost(g_baseurl+"/oam/server/auth_cred_submit",{username:a.login,password:a.password.substr(0,16)},g_headers);if(!/"_afrLoop",\s*"(\d+)"/i.test(d)){if(a=getParam(d,null,null,/<div[^>]+class="[^"]*\bred"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(a,
null,/Неверный логин или пароль/i.test(a));if(a=getParam(d,null,null,/(Неверный логин или пароль)/i))throw new AnyBalance.Error(a,null,!0);if(b=getParam(AnyBalance.getLastUrl(),null,null,/p_error_code=([^&]*)/i,null,decodeURIComponent)){var g=AnyBalance.requestGet(g_baseurl+"/SLAlfaSignFront10/errors?code="+b+"&type="+getParam(b,null,null,/[^\-]*/),g_headers),e=getJson(g);"SUCCESS"!=e.result?AnyBalance.trace("Не удалось получить ошибку: "+g):a=e.payload[b];if(a)throw new AnyBalance.Error(a,null,/логин или пароль/i.test(a));
}AnyBalance.trace(d);throw new AnyBalance.Error("Не удалось зайти в интернет-банк. Сайт изменен?");}d=clickGoToUrl(d,{isHtml:!0,sessionRequired:!0});g=AnyBalance.getLastUrl();if(/faces\/main\/changePassword|faces\/routeChangePwd\/changePassword/i.test(d))throw new AnyBalance.Error("Вам необходимо сменить старый пароль. Зайдите в Альфа-Клик через браузер, поменяйте пароль, затем введите новый пароль в настройки провайдера.",null,!0);if(/pt1:password_key/i.test(d)){var e=getParam(d,null,null,/window.name='([^']*)/i,
replaceSlashes)||null,f=getNextPage(d,"pt1:next_button",[["event","%EVENT%"],["event.%EVENT%",g_some_action],["oracle.adf.view.rich.PPR_FORCED","true"]],{paramsFunc:c,withScripts:!0});if(d=f.html){if(a=getParam(f.scripts,null,null,/AdfFacesMessage.TYPE_ERROR,[^,]*,('[^']*')/i,[/^/,"return "],safeEval))throw new AnyBalance.Error(a);AnyBalance.trace(d+f.scripts);AnyBalance.trace("Не удалось зайти в Альфа-Клик после ввода одноразового пароля. Попробуйте отключить запрос одноразового пароля в настройках своего Альфа-Клик.")}d=
clickGoToUrl(g.replace(/_afrWindowId=null/,"_afrWindowId="+e))}if(!/images\/close.png/i.test(d)&&!/&#1042;&#1099;&#1093;&#1086;&#1076;/i.test(d))throw AnyBalance.trace(d),new AnyBalance.Error("Не удалось войти в Альфа-Клик. Сайт изменен?");g_mainHtml=d;if(isClick20())AnyBalance.trace("Определен Альфа-Клик 2.0");else if(AnyBalance.trace("Определен Альфа-Клик 1.0"),!b||!b.allowClick10)throw new AnyBalance.Error("К сожалению, Альфа-Клик 1.0 не поддерживается. Зайдите в Альфа-Клик через браузер и переключите ваш Альфа-Клик на версию 2.0");
__setLoginSuccessful();return d}
function clickGoToUrl(b,c){c||(c={});var a=c.isHtml?b:AnyBalance.requestGet(b,g_headers),d=getParam(a,null,null,/"_afrLoop",\s*"(\d+)"/i);if(!d)throw new AnyBalance.Error("Не удаётся найти параметр для входа: _afrLoop. Сайт изменен?");a=getParam(a,null,null,/var\s+sess\s*=\s*"([^"]*)/i,replaceSlashes);if(c.sessionRequired&&!a)throw new AnyBalance.Error("Не удаётся найти параметр для входа: sess. Сайт изменен?");a=AnyBalance.requestGet(g_baseurl+"/ALFAIBSR/"+a+"?_afrLoop="+d+"&_afrWindowMode=0&_afrWindowId=null",
g_headers);d=getParam(a,null,null,/<div[^>]+id="ct"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces);AnyBalance.trace("CSRF token found: "+d);g_csrfToken=d;var g=getParam(a,null,null,/location\.href\s*=\s*'(\/ALFA[^']*)/,replaceSlashes),a=AnyBalance.requestGet(g_baseurl+g,g_headers),d=getParam(a,null,null,/"_afrLoop",\s*"(\d+)"/i);if(!d)throw new AnyBalance.Error("Не удаётся найти очередной параметр для входа: _afrLoop. Сайт изменен?");return a=AnyBalance.requestGet(g_baseurl+g+"&_afrLoop="+d+"&_afrWindowMode=0&_afrWindowId=null",
...
function processCards2(b,c){if(AnyBalance.isAvailable("cards")){AnyBalance.trace("Получаем информацию о картах");b=getMainPageOrModule2(b,"card");var a=getElements(b,[/<div[^>]+class="[^"]*AFFlow[^>]*>/ig,/c2_cardlist_Card/i]);if(!a.length)if(/Удобство пластиковых карт:/i.test(b))AnyBalance.trace("Нет ни одной карты");else{AnyBalance.trace("Не удалось найти карты\n"+b);return}c.cards=[];AnyBalance.trace("Найдено карт: "+a.length);for(var d=0;d<a.length;++d){var g=a[d],e=getParam(g,null,null,/<a[^>]+c2_cardlist_Card[^>]*>([\s\S]*?)<\/a>/i,
replaceTagsAndSpaces),f=getParam(g,null,null,/<a[^>]+style="[^"]*display:\s*inline;[^>]*>([\s\S]*?)<\/a>/i,replaceTagsAndSpaces),e={__id:e,__name:f+" ("+e+")"};__shouldProcess("cards",e)&&processCard2(b,g,e);c.cards.push(e)}}}
function processCard2(b,c,a){c=getParam(c,null,null,/<a[^>]+id="([^"]*)"[^>]*c2_cardlist_Card/i,replaceHtmlEntities);b=getNextPage(b,c,[["event","%EVENT%"],["event.%EVENT%",g_some_action],["oracle.adf.view.rich.PPR_FORCED","true"]]);getParam(b,a,"cards.balance",/&#1041;&#1072;&#1083;&#1072;&#1085;&#1089; &#1089;&#1095;&#(?:1077|1105);&#1090;&#1072;[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"cards.currency cards.balance cards.topay cards.debt cards.minpay cards.penalty cards.late cards.overdraft cards.limit".split(" "),
/&#1041;&#1072;&#1083;&#1072;&#1085;&#1089; &#1089;&#1095;&#1077;&#1090;&#1072;[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces,parseCurrency);getParam(b,a,"cards.till",/&#1057;&#1088;&#1086;&#1082; &#1076;&#1077;&#1081;&#1089;&#1090;&#1074;&#1080;&#1103;[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces,parseDate);getParam(b,a,"cards.num",/&#1053;&#1086;&#1084;&#1077;&#1088;<[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces);getParam(b,a,["cards.type","cards.credit"],/&#1058;&#1080;&#1087;<[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,
replaceTagsAndSpaces);AnyBalance.isAvailable("cards.credit")&&(a.credit=/кредитн/i.test(""+a.type));getParam(b,a,"cards.acctype",/&#1053;&#1072;&#1079;&#1074;&#1072;&#1085;&#1080;; &#1089;&#1095;&#(?:1077|1105);&#1090;&#1072;<[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces);getParam(b,a,"cards.userName",/&#1056;&#1072;&#1076;&#1099; &#1042;&#1072;&#1089; &#1074;&#1080;&#1076;&#1077;&#1090;&#1100;,([^<(]*)/i,replaceTagsAndSpaces);getParam(b,a,"cards.status",/&#1057;&#1090;&#1072;&#1090;&#1091;&#1089;<[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,
replaceTagsAndSpaces);getParam(b,a,"cards.accnum",/&#1053;&#1086;&#1084;&#1077;&#1088; &#1089;&#1095;&#(?:1077|1105);&#1090;&#1072;<[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces);AnyBalance.isAvailable("cards.transactions")&&processCardTransactions(b,a)}
...
a,"accounts.dateStart",/&#1044;&#1072;&#1090;&#1072; &#1086;&#1090;&#1082;&#1088;&#1099;&#1090;&#1080;&#1103; &#1089;&#1095;&#1105;&#1090;&#1072;[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces,parseDate),getParam(b,a,"accounts.blocked",/&#1047;&#1072;&#1073;&#1083;&#1086;&#1082;&#1080;&#1088;&#1086;&#1074;&#1072;&#1085;&#1086; &#1089;&#1088;&#1077;&#1076;&#1089;&#1090;&#1074;[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces,parseBalance),AnyBalance.isAvailable("accounts.transactions")&&
...
function getNextPage(b,c,a,d){var g=getParam(b,null,null,/<form[^>]*name="f1"[^>]*>([\s\S]*?)<\/form>/i),e=getParam(b,null,null,/<form[^>]*name="f1"[^>]*action="(\/ALFAIBSR[^"]*)/i,replaceHtmlEntities);if(!e)throw new AnyBalance.Error("Не удаётся найти форму ввода команды. Сайт изменен?");d||(d={});g=createFormParams(g,function(a,b,c,e){if(!/::input$/.test(c))return d.paramsFunc&&(e=d.paramsFunc(a,b,c,e)),e},!0);c=createParams(joinArrays(g,a),c);e=AnyBalance.requestPost(g_baseurl+e,c,addHeaders(d.headers||
{"Adf-Ads-Page-Id":g_pageID,"Adf-Rich-Message":"true","Csrf-Token":g_csrfToken}),{options:d.requestOptions});if(!/<\?Adf-Rich-Response-Type\s*\?>/i.test(e))return e;var f;d.withScripts&&(f=create_aggregate_join("\n"),f=sumParam(e,null,null,/<script>([\s\S]*?)<\/script>/ig,null,parseScriptTag,f));c=sumParam(e,null,null,/<fragment>\s*<!\[CDATA\[([\s\S]*?)\]\]>\s*<\/fragment>/ig);for(a=0;a<c.length;a++){var g=c[a],h=getParam(g,null,null,/^\s*<[^>]+[\s"]id="([^"]*)/i,replaceHtmlEntities);if("afr::DocWrapper"==
...
(d+1));var g=getNextPage(g,e,[["event","%EVENT%"],["event.%EVENT%",g_some_action],["oracle.adf.view.rich.PPR_FORCED","true"]]),e=getParam(g,null,null,/\d{20}/i),f=getParam(a[d],null,null,null,replaceTagsAndSpaces),e={__id:e,__name:f};__shouldProcess("credits",e)&&processCredit2(g,e);c.credits.push(e)}}else AnyBalance.trace("Не удаётся найти ни одного кредита!")}}
...
(getParam(f,c,"credits.payment_day"),getParam(f,c,"credits.minpay_till",null,null,parseNDay)):/Уже внесено/i.test(h)?getParam(f,c,["credits.minpay_paid","credits.minpay_left"],null,null,parseBalance):AnyBalance.trace("Неизвестный параметр: "+h+": "+f+"\n"+e)}}getParam(b,c,"credits.minpay_left",/<span[^>]+ACCreditsLargeHeaderFont[^>]*>([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);AnyBalance.isAvailable("credits.transactions")&&processCreditTransactions(b,c)}
function processInfo2(b,c){if(AnyBalance.isAvailable("info")){AnyBalance.trace("Получаем информацию о пользователе");c=c.info={};getParam(b,c,"info.mphone",/<div[^>]*>([^<]*)<div[^>]+id="[^"]*mymobilePopup/i,replaceTagsAndSpaces);b=clickFollowUrl("https://settings.alfabank.ru/settings/profile");var a=getJsonObject(b,/window.initialState\s*=\s*JSON.parse\s*\(\s*'/);if(!a)throw AnyBalance.trace("Не удалось получить json с именем: "+b),new AnyBalance.Error("Не удалось получить json с именем, сайт изменен?");
var d=create_aggregate_join(" ");sumParam(jspath1(a,"$.profileInfo.customer.lastName"),c,"info.fio",null,null,null,d);sumParam(jspath1(a,"$.profileInfo.customer.firstName"),c,"info.fio",null,null,null,d);sumParam(jspath1(a,"$.profileInfo.customer.middleName"),c,"info.fio",null,null,null,d);getParam(jspath1(a,"$.profileInfo.customer.email"),c,"info.email");if(AnyBalance.isAvailable("info.mphone","info.rphone","info.wphone","info.hphone","info.login","info.login2","info.check","info.mobile")){b=AnyBalance.requestGet("https://settings.alfabank.ru/settings/api/load?_="+
(new Date).getTime(),g_headers);a=getJson(b);getParam(jspath1(a,"$[*].contacts.clickPhone"),c,"info.mphone");getParam(jspath1(a,"$[*].contacts.registrationPhone"),c,"info.rphone");getParam(jspath1(a,"$[*].contacts.workPhone"),c,"info.wphone");getParam(jspath1(a,"$[*].contacts.homePhone"),c,"info.hphone");getParam(jspath1(a,'$[*].channels[?(@.id=="C2")].login'),c,"info.login");getParam(jspath1(a,'$[*].channels[?(@.id=="C2")].alternativeLogin'),c,"info.login2");if((d=jspath(a,"$[*].cards[?(@.alfaCheck.enabled==true)]"))&&
AnyBalance.isAvailable("info.check")){for(var g=[],e=0;e<d.length;e++){var f=d[e],h={};getParam(jspath1(f,"$.maskedNumber"),h,"info.check.card");getParam(jspath1(f,"$.alfaCheck.phone"),h,"info.check.phone");g.push(h)}c.check=g}getParam(!jspath1(a,'$[*].channels[?(@.id=="M2")].isBlocked'),c,"info.mobile")}}}
