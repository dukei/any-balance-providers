...
function processCards2(a,c){if(AnyBalance.isAvailable("cards")){AnyBalance.trace("Получаем информацию о картах");a=getMainPageOrModule2(a,"card");var b=getElement(a,/<table[^>]+card-list/i),f;b&&(f=getElements(b,/<tr/ig));if(!f||!f.length)if(/Удобство пластиковых карт:/i.test(a))AnyBalance.trace("Нет ни одной карты");else{AnyBalance.trace("Не удалось найти карты\n"+a);return}c.cards=[];AnyBalance.trace("Найдено карт: "+f.length);for(b=0;b<f.length;++b){var g=f[b],d=getParam(g,/\*{4}([\s\S]*?)<\/a>/i,
replaceTagsAndSpaces),e=getParam(g,/<a[^>]+style="[^"]*display:\s*inline;[^>]*>([\s\S]*?)<\/a>/i,replaceTagsAndSpaces),d={__id:d,__name:e+" ("+d+")"};__shouldProcess("cards",d)&&processCard2(a,g,d);c.cards.push(d)}}}
