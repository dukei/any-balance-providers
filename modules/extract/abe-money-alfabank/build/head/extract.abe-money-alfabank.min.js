...
function processClick(a){function c(a,b,c,d){return"pt1:password_key"==c?(a=getParam(e,null,null,/<span[^>]+id="[^>]*_(?:token|sms)"(?:[^>](?!display\s*:\s*none))*>([\s\S]*?)(?:<\/span>|<div)/i,replaceTagsAndSpaces),AnyBalance.trace("Затребован код на вход: "+a),AnyBalance.retrieveCode((a||"Для входа в интернет-банк требуется одноразовый код. Введите его в поле ниже.")+"\n\nЧтобы не вводить код всегда при входе в интернет-банк, вы можете отключить требование одноразового кода на вход в настройках Альфа-Клик. Это безопасно, отменяет только подтверждение входа. Подтверждение транзакций всё равно будет требоваться обязательно.",
null,{time:18E4,inputType:"number"})):d}var b=AnyBalance.getPreferences(),e=AnyBalance.requestGet(g_baseurl+"/ALFAIBSR/",g_headers),g=getJsonObject(e,/window.initialState\s*=/).passport.authorization.requestId,e=AnyBalance.requestPost(g_baseurl+"/oam/server/auth_cred_submit",{username:b.login,password:b.password,request_id:g},g_headers);if(!/"_afrLoop",\s*"(\d+)"/i.test(e)){if(b=getParam(e,null,null,/<div[^>]+class="[^"]*\bred"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(b,
null,/Неверный логин или пароль/i.test(b));if(b=getParam(e,null,null,/(Неверный логин или пароль)/i))throw new AnyBalance.Error(b,null,!0);if(a=getParam(AnyBalance.getLastUrl(),null,null,/p_error_code=([^&]*)/i,null,decodeURIComponent)){var g=AnyBalance.requestGet(g_baseurl+"/SLAlfaSignFront10/errors?code="+a+"&type="+getParam(a,null,null,/[^\-]*/),g_headers),d=getJson(g);"SUCCESS"!=d.result?AnyBalance.trace("Не удалось получить ошибку: "+g):b=d.payload[a];if(b)throw new AnyBalance.Error(b,null,/логин или пароль/i.test(b));
}AnyBalance.trace(e);throw new AnyBalance.Error("Не удалось зайти в интернет-банк. Сайт изменен?");}e=clickGoToUrl(e,{isHtml:!0,sessionRequired:!0});g=AnyBalance.getLastUrl();if(/faces\/main\/changePassword|faces\/routeChangePwd\/changePassword/i.test(e))throw new AnyBalance.Error("Вам необходимо сменить старый пароль. Зайдите в Альфа-Клик через браузер, поменяйте пароль, затем введите новый пароль в настройки провайдера.",null,!0);if(/pt1:password_key/i.test(e)){var d=getParam(e,null,null,/window.name='([^']*)/i,
replaceSlashes)||null,f=getNextPage(e,"pt1:next_button",[["event","%EVENT%"],["event.%EVENT%",g_some_action],["oracle.adf.view.rich.PPR_FORCED","true"]],{paramsFunc:c,withScripts:!0});if(e=f.html){if(b=getParam(f.scripts,null,null,/AdfFacesMessage.TYPE_ERROR,[^,]*,('[^']*')/i,[/^/,"return "],safeEval))throw new AnyBalance.Error(b);AnyBalance.trace(e+f.scripts);AnyBalance.trace("Не удалось зайти в Альфа-Клик после ввода одноразового пароля. Попробуйте отключить запрос одноразового пароля в настройках своего Альфа-Клик.")}e=
clickGoToUrl(g.replace(/_afrWindowId=null/,"_afrWindowId="+d))}if(!/images\/close.png/i.test(e)&&!/&#1042;&#1099;&#1093;&#1086;&#1076;/i.test(e))throw AnyBalance.trace(e),new AnyBalance.Error("Не удалось войти в Альфа-Клик. Сайт изменен?");g_mainHtml=e;if(isClick20())AnyBalance.trace("Определен Альфа-Клик 2.0");else if(AnyBalance.trace("Определен Альфа-Клик 1.0"),!a||!a.allowClick10)throw new AnyBalance.Error("К сожалению, Альфа-Клик 1.0 не поддерживается. Зайдите в Альфа-Клик через браузер и переключите ваш Альфа-Клик на версию 2.0");
__setLoginSuccessful();return e}
