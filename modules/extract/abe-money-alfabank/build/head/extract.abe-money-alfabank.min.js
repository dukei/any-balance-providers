...
b=AB.createFormParams(c,function(a,b,c,f){return"otp_param"==c?AnyBalance.retrieveCode("Введите код подтверждения входа из СМС",null,{inputType:"number"}):f});b=AnyBalance.requestPost(g_baseurl+"/oam/server/auth_cred_submit",b,g_headers)}if(!/"_afrLoop",\s*"(\d+)"/i.test(b)){if(a=getElement(b,/<div[^>]+class="[^"]*\b(?:red|notification__message)\b/i,replaceTagsAndSpaces))throw new AnyBalance.Error(a,null,/парол/i.test(a));if(a=getParam(b,null,null,/(Неверный логин или пароль)/i))throw new AnyBalance.Error(a,
null,!0);if(c=getParam(AnyBalance.getLastUrl(),null,null,/p_error_code=([^&]*)/i,null,decodeURIComponent)){var f=AnyBalance.requestGet(g_baseurl+"/SLAlfaSignFront10/errors?code="+c+"&type="+getParam(c,null,null,/[^\-]*/),g_headers),g=getJson(f);"SUCCESS"!=g.result?AnyBalance.trace("Не удалось получить ошибку: "+f):a=g.payload[c];if(a)throw new AnyBalance.Error(a,null,/логин или пароль/i.test(a));}AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в интернет-банк. Сайт изменен?");}b=
...
function clickFollowUrl(a){for(var c=5;--c;){var b=AnyBalance.requestGet(a,g_headers),f=getParam(b,null,null,/"_afrLoop",\s*"(\d+)"/i);if(!f)break;a=AnyBalance.getLastUrl();b=AnyBalance.requestGet(a+"&_afrLoop="+f+"&_afrWindowMode=0&_afrWindowId=null",g_headers);if(a=getParam(b,null,null,/<div[^>]+id="ct"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces))AnyBalance.trace("CSRF token found: "+a),g_csrfToken=a;a=sumParam(b,null,null,/paramToCookie\s*\(\s*['"]([^'"]*)/ig);for(f=0;f<a.length;++f)paramToCookie(a[f]);
...
function processCards2(a,c){if(AnyBalance.isAvailable("cards")){AnyBalance.trace("Получаем информацию о картах");a=getMainPageOrModule2(a,"card");var b=getElements(a,[/<div[^>]+class="[^"]*AFFlow[^>]*>/ig,/c2_cardlist_Card/i]);if(!b.length)if(/Удобство пластиковых карт:/i.test(a))AnyBalance.trace("Нет ни одной карты");else{AnyBalance.trace("Не удалось найти карты\n"+a);return}c.cards=[];AnyBalance.trace("Найдено карт: "+b.length);for(var f=0;f<b.length;++f){var g=b[f],d=getParam(g,null,null,/<a[^>]+c2_cardlist_Card[^>]*>([\s\S]*?)<\/a>/i,
replaceTagsAndSpaces),e=getParam(g,null,null,/<a[^>]+style="[^"]*display:\s*inline;[^>]*>([\s\S]*?)<\/a>/i,replaceTagsAndSpaces),d={__id:d,__name:e+" ("+d+")"};__shouldProcess("cards",d)&&processCard2(a,g,d);c.cards.push(d)}}}
function processCard2(a,c,b){c=getParam(c,null,null,/<a[^>]+id="([^"]*)"[^>]*c2_cardlist_Card/i,replaceHtmlEntities);a=getNextPage(a,c,[["event","%EVENT%"],["event.%EVENT%",g_some_action],["oracle.adf.view.rich.PPR_FORCED","true"]]);getParam(a,b,"cards.balance",/&#1041;&#1072;&#1083;&#1072;&#1085;&#1089; &#1089;&#1095;&#(?:1077|1105);&#1090;&#1072;[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces,parseBalance);getParam(a,b,"cards.currency cards.balance cards.topay cards.debt cards.minpay cards.penalty cards.late cards.overdraft cards.limit".split(" "),
...
function processAccounts2(a,c){if(AnyBalance.isAvailable("accounts")){AnyBalance.trace("Получаем информацию о счетах");a=getMainPageOrModule2(a,"acc");var b=getElements(a,/<table[^>]+class="accounts_table"[^>]*>/ig);if(b.length){AnyBalance.trace("Найдено счетов: "+b.length);c.accounts=[];for(var f=0;f<b.length;f++){var g=b[f],d=getParam(g,null,null,/(?:[\s\S]*?<td[^>]*>){2}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces),e=getParam(g,null,null,/(?:[\s\S]*?<td[^>]*>){1}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces),
d={__id:d,__name:e+" ("+d.substr(-4)+")"};__shouldProcess("accounts",d)&&processAccount2(a,g,d);c.accounts.push(d)}}else AnyBalance.trace(a),AnyBalance.trace("Не удалось найти ни одного счета")}}function n2(a){return 10>a?"0"+a:""+a}
...
processAccountTransactions(a,b))}function createParams(a,c){for(var b={},f=0;f<a.length;++f)a[f]&&(b[a[f][0].replace(/%EVENT%/g,c)]=a[f][1].replace(/%EVENT%/g,c));return b}
...
function getNextPage(a,c,b,f){var g=getParam(a,null,null,/<form[^>]*name="f1"[^>]*>([\s\S]*?)<\/form>/i),d=getParam(a,null,null,/<form[^>]*name="f1"[^>]*action="(\/ALFAIBSR[^"]*)/i,replaceHtmlEntities);if(!d)throw new AnyBalance.Error("Не удаётся найти форму ввода команды. Сайт изменен?");f||(f={});g=createFormParams(g,function(a,b,c,d){if(!/::input$/.test(c))return f.paramsFunc&&(d=f.paramsFunc(a,b,c,d)),d},!0);c=createParams(joinArrays(g,b),c);d=AnyBalance.requestPost(g_baseurl+d,c,addHeaders(f.headers||
{"Adf-Ads-Page-Id":g_pageID,"Adf-Rich-Message":"true","Csrf-Token":g_csrfToken}),{options:f.requestOptions});if(!/<\?Adf-Rich-Response-Type\s*\?>/i.test(d))return d;var e;f.withScripts&&(e=create_aggregate_join("\n"),e=sumParam(d,null,null,/<script>([\s\S]*?)<\/script>/ig,null,parseScriptTag,e));c=sumParam(d,null,null,/<fragment>\s*<!\[CDATA\[([\s\S]*?)\]\]>\s*<\/fragment>/ig);for(b=0;b<c.length;b++){var g=c[b],h=getParam(g,null,null,/^\s*<[^>]+[\s"]id="([^"]*)/i,replaceHtmlEntities);if("afr::DocWrapper"==
h){a=g;break}h=getElement(a,new RegExp('<\\w+[^>]*[\\s"]id="'+h+'"[^>]*>',"i"));a=a.replace(h,g)}/<div[^>]+id="pt1:menu:ATP2_r1"[^>]*>/i.test(a)&&(g_mainHtml=a);d=getParam(d,null,null,/AdfPage.PAGE._setPageId\('([^']*)/i,replaceSlashes);isset(d)&&(g_pageID=d);return f.withScripts?{html:a,scripts:e}:a}
function processCredits2(a,c){if(AnyBalance.isAvailable("credits")){AnyBalance.trace("Получаем информацию о кредитах");a=getMainPageOrModule2(a,"crd");var b=getElements(a,/<a[^>]+id="[^"]*::disAcr"[^>]*>/ig);if(b){AnyBalance.trace("Кредитов в системе: "+b.length);c.credits=[];for(var f=0;f<b.length;++f){var g=a;if(0<f){AnyBalance.trace("Запрос кредита №"+(f+1));var d=getParam(b[f],null,null,/<a[^>]+id="([^"]*)::disAcr"/i,replaceHtmlEntities),g=getNextPage(a,d,[["event","%EVENT%"],["event.%EVENT%",
...
function processCredit2(a,c){for(var b=getElements(a,/<table[^>]+pglCreditDetails(?:Label)?List[^>]*/ig),f=!1,g=0;g<b.length;g++){var d=b[g],e=getElements(d,/<td[^>]*>/ig);if(!(2>e.length)){var h=getParam(e[e.length-2],null,null,null,replaceTagsAndSpaces),e=getParam(e[e.length-1],null,null,null,replaceTagsAndSpaces);/Для полного погашения|Доступно для трат/i.test(h)?(getParam(e,c,"credits.balance",null,null,parseBalance),getParam(e,c,["credits.currency","credits.balance"],null,null,parseCurrency)):
/Собственных средств/i.test(h)?getParam(e,c,"credits.own",null,null,parseBalance):/Задолженность|Остаток задолженности|Для полного погашения/i.test(h)?getParam(e,c,"credits.debt",null,null,parseBalance):/сумма основного долга/i.test(h)&&!f?getParam(e,c,"credits.debt_main",null,null,parseBalance):/сумма начисленных процентов/i.test(h)&&!f?getParam(e,c,"credits.debt_pct",null,null,parseBalance):/Процентная ставка/i.test(h)?getParam(e,c,"credits.pct",null,null,parseBalance):/Установленный лимит|Начальная сумма кредита/i.test(h)?
getParam(e,c,"credits.limit",null,null,parseBalance):/Неподтверждённые операции/i.test(h)?getParam(e,c,"credits.blocked",null,null,parseBalance):/Льготный период/i.test(h)?getParam(e,c,"credits.gracepay",null,null,parseBalance):/Дата начала льготного периода/i.test(h)?getParam(e,c,"credits.grace_start",null,null,parseDateWord):/Дата окончания льготного периода/i.test(h)?getParam(e,c,"credits.gracepay_till",null,null,parseDateWord):/дата платежа|Оплатить до/i.test(h)?getParam(e,c,"credits.pay_till",
null,null,parseDateWord):/Ближайший плат[её]ж|Минимальный плат[её]ж|Ежемесячный плат[её]ж/i.test(h)?(f=!0,getParam(e,c,["credits.minpay","credits.minpay_left"],null,null,parseBalance)):"основной долг"==h&&f?getParam(e,c,"credits.minpay_main",null,null,parseBalance):"проценты"==h&&f?getParam(e,c,"credits.minpay_pct",null,null,parseBalance):/Дата выдачи/i.test(h)?getParam(e,c,"credits.date_start",null,null,parseDateWord):/Срок кредита/i.test(h)?getParam(e,c,"credits.period"):/День платежа/i.test(h)?
(getParam(e,c,"credits.payment_day"),getParam(e,c,"credits.minpay_till",null,null,parseNDay)):/Уже внесено/i.test(h)?getParam(e,c,["credits.minpay_paid","credits.minpay_left"],null,null,parseBalance):AnyBalance.trace("Неизвестный параметр: "+h+": "+e+"\n"+d)}}getParam(a,c,"credits.minpay_left",/<span[^>]+ACCreditsLargeHeaderFont[^>]*>([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);AnyBalance.isAvailable("credits.transactions")&&processCreditTransactions(a,c)}
...
AnyBalance.isAvailable("info.check")){for(var f=[],g=0;g<a.length;g++){var d=a[g],e={};getParam(jspath1(d,"$.maskedNumber"),e,"info.check.card");getParam(jspath1(d,"$.alfaCheck.phone"),e,"info.check.phone");f.push(e)}c.check=f}getParam(!jspath1(b,'$[*].channels[?(@.id=="M2")].isBlocked'),c,"info.mobile")}}}
function processDeposits2(a,c){if(AnyBalance.isAvailable("deposits")){AnyBalance.trace("Получаем информацию о депозитах");a=getMainPageOrModule2(a,"dep");var b=sumParam(a,null,null,/<table[^>]+(?:[\s\S](?!<\/?table))*?>\d{20}<[\s\S]*?<\/table>/ig);if(!b.length)if(/Депозиты в банке, как и накопительные/i.test(a))AnyBalance.trace("Нет ни одного депозита");else{AnyBalance.trace("Не удалось найти депозиты: "+a);return}AnyBalance.trace("Найдено депозитов: "+b.length);c.deposits=[];for(var f=0;f<b.length;f++){var g=
b[f],d=getParam(g,null,null,/(?:[\s\S]*?<td[^>]*>){1}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces),e=getParam(g,null,null,/(?:[\s\S]*?<td[^>]*>){2}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces),h=getParam(g,null,null,/(?:[\s\S]*?<td[^>]*>){4}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces,parseCurrency),d={__id:e,__name:d+" "+h+" ("+e.substr(-4)+")"};h||AnyBalance.trace("Не удалось получить баланс и валюту для депозита "+d.__name+": "+g);__shouldProcess("deposits",d)&&processDeposit2(a,g,d);c.deposits.push(d)}}}
...
c,[["event","%EVENT%"],["event.%EVENT%",g_some_action],["oracle.adf.view.rich.PPR_FORCED","true"]]);c=getElements(a,/<tr[^>]+id="pt\d+:plam\d+[^>]*>/ig);for(var f=0;f<c.length;f++){var g=c[f],d=getParam(g,null,null,/(?:[\s\S]*?<td[^>]*>){1}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces),e=getParam(g,null,null,/(?:[\s\S]*?<td[^>]*>){2}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces);/Экспресс-счёт/i.test(d)?getParam(e,b,"deposits.expressnum"):/Сумма на дату открытия/i.test(d)?getParam(e,b,"deposits.balance_start",
null,null,parseBalance):/Сумма выплаченных процентов/i.test(d)?getParam(e,b,"deposits.pct_sum",null,null,parseBalance):/Дата открытия/i.test(d)?getParam(e,b,"deposits.date_start",null,null,parseDateWord):/Срок депозита/i.test(d)?getParam(e,b,"deposits.period",null,null,parseBalance):/Процентная ставка$/i.test(d)?getParam(e,b,"deposits.pct",null,null,parseBalance):/Процентная ставка с учетом капитализации/i.test(d)?getParam(e,b,"deposits.pct_cap",null,null,parseBalance):/Возможность пополнения/i.test(d)?
getParam(e,b,"deposits.topup",null,null,parseBool):/Крайний срок пополнения/i.test(d)?getParam(e,b,"deposits.topup_till",null,null,parseDateWord):/Минимальная сумма пополнения/i.test(d)?getParam(e,b,"deposits.topup_min",null,null,parseBalance):/Возможность частичного снятия/i.test(d)?getParam(e,b,"deposits.withdraw",null,null,parseBool):/Сумма неснижаемого остатка/i.test(d)?getParam(e,b,"deposits.balance_min",null,null,parseBalance):/Условие начисления процентов/i.test(d)?getParam(e,b,"deposits.pct_conditions"):
/Автоматическая пролонгация/i.test(d)?getParam(e,b,"deposits.prolongate",null,null,parseBool):/Текущий баланс|Депозитный счёт|Дата окончания/i.test(d)||AnyBalance.trace("Неизвестный параметр "+d+": "+e+"\n"+g)}}AnyBalance.isAvailable("deposits.transactions")&&processDepositTransactions(a,b)};
