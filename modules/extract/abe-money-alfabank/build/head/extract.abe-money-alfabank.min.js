...
{"Adf-Ads-Page-Id":g_pageID,"Adf-Rich-Message":"true","Csrf-Token":g_csrfToken}),{options:e.requestOptions});if(!/<\?Adf-Rich-Response-Type\s*\?>/i.test(d))return d;if(e.withScripts){var f=create_aggregate_join("\n");f=sumParam(d,null,null,/<script>([\s\S]*?)<\/script>/ig,null,parseScriptTag,f)}c=sumParam(d,null,null,/<fragment>\s*<!\[CDATA\[([\s\S]*?)\]\]>\s*<\/fragment>/ig);for(b=0;b<c.length;b++){g=c[b];var h=getParam(g,null,null,/^\s*<[^>]+[\s"]id="([^"]*)/i,replaceHtmlEntities);if("afr::DocWrapper"==
...
(e+1));g=getNextPage(g,d,[["event","%EVENT%"],["event.%EVENT%",g_some_action],["oracle.adf.view.rich.PPR_FORCED","true"]]);d=getParam(g,null,null,/\d{20}/i);var f=getParam(b[e],null,null,null,replaceTagsAndSpaces);d={__id:d,__name:f};__shouldProcess("credits",d)&&processCredit2(g,d);c.credits.push(d)}}else AnyBalance.trace("Не удаётся найти ни одного кредита!")}}
...
function turnOffLoginOtp(){try{var a=AnyBalance.requestGet("https://settings.alfabank.ru/settings/logins-passwords",g_headers),c=AnyBalance.getLastUrl();a=AnyBalance.requestGet("https://settings.alfabank.ru/settings/api/load?_="+(new Date).getTime(),addHeaders({Referer:c,"X-Requested-With":"XMLHttpRequest"}));for(var b=getJson(a),e=0;e<b.length;++e)if(b[e].channels){b=b[e];break}var g=!1;for(e=0;b.channels&&e<b.channels.length;++e){var d=b.channels[e];if("C2"===d.id){g=!0;d.hasAdditionalLoginPassword?
