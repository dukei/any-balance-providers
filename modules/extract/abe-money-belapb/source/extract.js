...
function processCards(info, result) {
	...
	AnyBalance.trace('Найдено потенциальных карт: ' + cards.length);
	if(cards.length == 0 || (cards.length == 1 && /Карты отсутствуют/i.test(html))){
        if(/Карты отсутствуют/i.test(html)){
            AnyBalance.trace('Активные карты отсутствуют');
            result.cards = [];
        }else {
...
function processDeposits(html, result) {
	...
	if(!table) {
		...
		}else if(/У Вас нет открытых счетов/i.test(html)){
			AnyBalance.trace('У вас нет счетов и депозитов');
		}else{
	    	AnyBalance.trace(html);
