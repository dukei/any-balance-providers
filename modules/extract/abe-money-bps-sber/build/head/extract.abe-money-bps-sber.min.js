...
function apiCall(b,a,c){var d=g_headers;"string"===typeof a&&(d=addHeaders({"Content-Type":"application/json; charset=UTF-8"}));b=AnyBalance.requestPost(baseurl+b,a,c?addHeaders(d,c):d,{HTTP_METHOD:a?"POST":"GET"});if(!b)throw AnyBalance.trace(b),new AnyBalance.Error("Не удалось получить ответ от сервера.");a=getJson(b);if(a.error||a.errorInfo&&+a.errorInfo.errorCode){if("new_device"===AnyBalance.getLastResponseHeader("sbol-status"))return a;c=a.error_description||a.errorInfo&&a.errorInfo.errorDescription;
AnyBalance.trace(b);throw new AnyBalance.Error(c,null,/unauthorized/i.test(a.error+c));}return a}
function login(){var b=AnyBalance.getPreferences();AnyBalance.setDefaultCharset("utf-8");checkEmpty(b.login,"Введите логин!");checkEmpty(b.password,"Введите пароль!");if(g_headers.Authorization&&/Bearer/i.test(g_headers.Authorization))AnyBalance.trace("Already logged in");else{g_headers["X-Sbol-Id"]="96892da4c0d22530";g_headers.Authorization="Basic "+Base64.encode(b.login+":"+b.password);var a=apiCall("oauth/token",{client_id:b.login,client_secret:b.password,grant_type:"client_credentials"},{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"});
if(a.error)if("new_device"===AnyBalance.getLastResponseHeader("sbol-status")){AnyBalance.trace("Требуется подтверждение устройства");a=AnyBalance.getLastResponseHeader("sbol-udid");apiCall("rest/registration/prepareDevice",JSON.stringify({udId:a}));var c=AnyBalance.retrieveCode("Пожалуйста, подтвердите разрешение AnyBalance входить в интернет-банк БПС-Сбербанк, введя код из SMS",null,{inputType:"number",time:3E5});apiCall("rest/registration/trustedDevice",JSON.stringify({smsCode:c,udId:a}));a=apiCall("oauth/token",
{client_id:b.login,client_secret:b.password,grant_type:"client_credentials"},{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"})}else throw AnyBalance.trace(JSON.stringify(a)),new AnyBalance.Error("Неизвестная ошибка входа. Сайт изменен?");g_headers.Authorization="Bearer "+a.access_token;__setLoginSuccessful()}}function getProducts(){getProducts.products||(getProducts.products=apiCall("rest/client/contracts"));return getProducts.products}
...
function processCard(b,a,c){AnyBalance.trace("Обработка карты "+a.__name);getParam(b.name,a,"cards.type");getParam(b.status,a,"cards.status");getParam(b.monthEnd+"/"+b.yearEnd,a,"cards.till",null,replaceTagsAndSpaces,parseDate);getParam(c.currencyName,a,["cards.currency","cards.balance"]);AnyBalance.isAvailable("cards.balance")&&(c=apiCall("rest/client/balance",JSON.stringify({cardExpire:b.yearEnd+"-"+b.monthEnd+"-15",cardId:b.cardId,currency:c.currencyCode})),getParam(c.amount,a,"cards.balance"));
isAvailable("cards.transactions")&&processCardTransactions(b,a)}function processDeposits(b,a){AnyBalance.isAvailable("deposits")}function processDeposit(b,a,c){AnyBalance.trace("Обработка депозита "+a.__name);isAvailable("deposits.transactions")&&processDepositTransactions(b,a)}function processCredits(b,a){if(AnyBalance.isAvailable("credits"))throw AnyBalance.Error("Ошибка получения кредитов. Сайт изменен?");}
