...
function login(){var a=AnyBalance.getPreferences();AnyBalance.setDefaultCharset("utf-8");checkEmpty(a.login,"Введите логин!");checkEmpty(a.password,"Введите пароль!");g_baseparams={LOGIN:a.login,PASSWORD:a.password,DEVICEID:hex_md5(a.login).substr(0,16),VERSION:"v2.35"};var a=makeRequest("login",g_baseparams),b=getElement(a,/<(?:\w+:)?(?:faultcode|errorcode)>/i,replaceTagsAndSpaces);if("0"!=b){var c=getElement(a,/<(?:\w+:)?(?:faultstring|errortext)>/i,replaceTagsAndSpaces);if(c)throw new AnyBalance.Error(c,
null,/парол|не зарегистрирован/i.test(c));AnyBalance.trace(a);throw new AnyBalance.Error("Не удалось зайти в интернет-банк (ошибка "+b+"). Сайт изменен?");}__setLoginSuccessful();return a}function getProducts(){getProducts.products||(getProducts.products=makeRequest("accounts",g_baseparams));return getProducts.products}
function processAccounts(a,b){if(AnyBalance.isAvailable("accounts")){a=getProducts();var c=getElement(a,/<cardAccounts[^>\/]*>/i);if(!c)if(/<cardAccounts\s*\/>/i.test(a))c="";else{AnyBalance.trace(a);AnyBalance.trace("Не удалось найти счета");return}c=getElements(c,/<cardAccount\b[^>]*>/ig);AnyBalance.trace("Найдено "+c.length+" счетов");b.accounts=[];for(a=0;a<c.length;++a){var d=c[a],e=getElement(d,/<accountId[^>]*>/i,replaceTagsAndSpaces),g=getElement(d,/<additionalAccountId[^>]*>/i,replaceTagsAndSpaces),
f=getElement(d,/<depoAccountInfo[^>]*>/i),f=getElement(f,/<name[^>]*>/i,replaceTagsAndSpaces),h=f+" "+g.substr(-4),e={__id:e,__name:h,num:g,name:f};__shouldProcess("accounts",e)&&processAccount(d,e);b.accounts.push(e)}}}
...
function processCards(a,b){if(AnyBalance.isAvailable("cards")){a=getProducts();var c=getElement(a,/<cardAccounts[^>\/]*>/i);if(!c)if(/<cardAccounts\s*\/>/i.test(a))c="";else{AnyBalance.trace(a);AnyBalance.trace("Не удалось найти карточные счета");return}c=getElements(c,/<cardAccount\b[^>]*>/ig);AnyBalance.trace("Найдено "+c.length+" карточных счетов");b.cards=[];for(a=0;a<c.length;++a){var d=c[a],e=getElement(d,/<accountId[^>]*>/i,replaceTagsAndSpaces),g=getElement(d,/<additionalAccountId[^>]*>/i,
replaceTagsAndSpaces),f=getElements(d,/<card\b[^>]*>/ig);AnyBalance.trace("Счет "+g+" содержит "+f.length+" карт");for(var h=0;h<f.length;++h){var k=f[h],l=getElement(k,/<cardHash[^>]*>/i,replaceTagsAndSpaces),n=getElement(k,/<maskedCard[^>]*>/i,replaceTagsAndSpaces),m=getElement(k,/<personalizedName[^>]*>/i,replaceTagsAndSpaces);m||(m=getParam(k,null,null,/<cardType[^>]*name="([^"]*)/i,replaceHtmlEntities));var p=m+" "+n.substr(-4),l={__id:l,__name:p,num:n,accid:e,accnum:g,name:m};__shouldProcess("cards",
l)&&processCard(k,l,d);b.cards.push(l)}}}}
...
function processCredits(a,b){if(AnyBalance.isAvailable("credits")){a=getProducts();var c=getElement(a,/<credits[^>\/]*>/i);if(!c)if(/<credits\s*\/>/i.test(a))c="";else{AnyBalance.trace(a);AnyBalance.trace("Не удалось найти кредиты");return}c=getElements(c,/<credit[^>]*>/ig);AnyBalance.trace("Найдено "+c.length+" кредитов");b.credits=[];for(a=0;a<c.length;++a){var d=c[a],e=getElement(d,/<contract_id[^>]*>/i,replaceTagsAndSpaces),g=getElement(d,/<contractNumber[^>]*>/i,replaceTagsAndSpaces),f=getElement(d,
/<contract_name[^>]*>/i,replaceTagsAndSpaces),h=f+" "+g.substr(-4),e={__id:e,__name:h,num:g,name:f};__shouldProcess("credits",e)&&processCredit(d,e);b.credits.push(e)}}}
