var g_headers={Accept:"application/json; charset=UTF-8","X-Sbol-OS":"android","X-Sbol-Version":"3.7.1","X-Sbol-Id":"",Connection:"Keep-Alive","User-Agent":"okhttp/3.6.0","Accept-Encoding":"gzip",Origin:null},baseurl="https://digital.bps-sberbank.by/SBOLServer/";
...
function processCards(b,a){if(AnyBalance.isAvailable("cards")){var c=getProducts();b=c.contracts.filter(function(k){return k.cardList&&k.cardList.length});if(b.length)for(AnyBalance.trace("Найдено "+b.length+" карточных счетов"),a.cards=[],c=0;c<b.length;++c){var d=b[c];AnyBalance.trace("Счет "+d.account+" содержит "+d.cardList.length+" карт");for(var e=0;e<d.cardList.length;++e){var f=d.cardList[e],g=f.cardId,h=f.panCode,l=f.name,m=f.name+" "+h.substr(-4);g={__id:g,__name:m,num:h,accnum:d.account,
name:l,accid:d.contractId};__shouldProcess("cards",g)&&processCard(f,g,d);a.cards.push(g)}}else AnyBalance.trace(JSON.stringify(c)),AnyBalance.trace("Не удалось найти карточные счета")}}
...
processCreditTransactions(b,a)}function processInfo(b,a){a=a.info={};b=apiCall("rest/user/updateProfile",JSON.stringify({}));var c={};sumParam(b.lastName,c,"__n",null,null,null,create_aggregate_join(" "));sumParam(b.firstName,c,"__n",null,null,null,create_aggregate_join(" "));sumParam(b.middleName,c,"__n",null,null,null,create_aggregate_join(" "));getParam(c.__n,a,"info.fio",null,null,capitalFirstLetters);getParam(b.passport,a,"info.passport");getParam(b.birthday,a,"info.birthday")};
