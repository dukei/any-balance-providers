var g_headers={Accept:"application/json; charset=UTF-8","X-Sbol-OS":"android","X-Sbol-Version":"1.0.5","X-Sbol-Id":"",Connection:"Keep-Alive","User-Agent":"okhttp/3.6.0"},baseurl="https://digital.bps-sberbank.by/SBOLServer/";
...
function login(){var a=AnyBalance.getPreferences();AnyBalance.setDefaultCharset("utf-8");checkEmpty(a.login,"Введите логин!");checkEmpty(a.password,"Введите пароль!");g_headers.Authorization&&/Bearer/i.test(g_headers.Authorization)?AnyBalance.trace("Already logged in"):(g_headers["X-Sbol-Id"]=hex_md5(a.login).substr(0,16),g_headers.Authorization="Basic "+Base64.encode(a.login+":"+a.password),a=apiCall("oauth/token",{client_id:a.login,client_secret:a.password,grant_type:"client_credentials"}),g_headers.Authorization=
"Bearer "+a.access_token,__setLoginSuccessful())}function getProducts(){getProducts.products||(getProducts.products=apiCall("rest/client/contracts"));return getProducts.products}
function processAccounts(a,b){if(AnyBalance.isAvailable("accounts")){var c=getProducts();if(a=c.contracts)for(AnyBalance.trace("Найдено "+a.length+" счетов"),b.accounts=[],c=0;c<a.length;++c){var d=a[c],e=d.contractId,f=d.account,g=d.name,h=g+" "+f.substr(-4),e={__id:e,__name:h,num:f,name:g};__shouldProcess("accounts",e)&&processAccount(d,e);b.accounts.push(e)}else AnyBalance.trace("Не удалось найти счета "+JSON.stringify(c))}}
...
function processCard(a,b,c){AnyBalance.trace("Обработка карты "+b.__name);getParam(a.name,b,"cards.type");getParam(a.status,b,"cards.status");getParam(a.monthEnd+"/"+a.yearEnd,b,"cards.till",null,replaceTagsAndSpaces,parseDate);getParam(c.currencyName,b,["cards.currency","cards.balance"]);getParam(c.amount,b,"cards.balance");isAvailable("cards.transactions")&&processCardTransactions(a,b)}function processDeposits(a,b){AnyBalance.isAvailable("deposits")}
function processDeposit(a,b,c){AnyBalance.trace("Обработка депозита "+b.__name);isAvailable("deposits.transactions")&&processDepositTransactions(a,b)}function processCredits(a,b){if(AnyBalance.isAvailable("credits"))throw AnyBalance.Error("Ошибка получения кредитов. Сайт изменен?");}
