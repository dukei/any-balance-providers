var g_headers={Accept:"application/json; charset=UTF-8","X-Sbol-OS":"android","X-Sbol-Version":"1.2.1","X-Sbol-Id":"",Connection:"Keep-Alive","User-Agent":"okhttp/3.6.0","Accept-Encoding":"gzip",Origin:null},baseurl="https://digital.bps-sberbank.by/SBOLServer/";
...
function processCard(b,a,c){AnyBalance.trace("Обработка карты "+a.__name);getParam(b.name,a,"cards.type");getParam(b.status,a,"cards.status");getParam(b.monthEnd+"/"+b.yearEnd,a,"cards.till",null,replaceTagsAndSpaces,parseDate);getParam(c.currencyName,a,["cards.currency","cards.balance"]);getParam(c.amount,a,"cards.balance");isAvailable("cards.transactions")&&processCardTransactions(b,a)}function processDeposits(b,a){AnyBalance.isAvailable("deposits")}
function processDeposit(b,a,c){AnyBalance.trace("Обработка депозита "+a.__name);isAvailable("deposits.transactions")&&processDepositTransactions(b,a)}function processCredits(b,a){if(AnyBalance.isAvailable("credits"))throw AnyBalance.Error("Ошибка получения кредитов. Сайт изменен?");}
