...
return a}
function processProfile(b,a){AnyBalance.trace("Получаем данные профиля");try{var c=AnyBalance.requestPost(baseurl+"REST/welcome/welcomeMsgContent?JFP_TOKEN="+getJFPToken(b),"",addHeaders(jsonHeaders,{Referer:baseurl+"JSO/signon/uname/HomePage.do?SYNC_TOKEN="+getToken(b)+"&JFP_TOKEN="+getJFPToken(b)})),d=getJson(c);a.profile={};getParam(d.USERNAME,a.profile,"info.fio",null,replaceTagsAndSpaces,capitalFirstLetters);getParam(d.EMAIL,a.profile,"info.email",null,replaceTagsAndSpaces);getParam(d.MOBILENUMBER,a.profile,
"info.mphone",null,replaceTagsAndSpaces);getParam(d.COMPANYNAMEPHRASE,a.profile,"info.COMPANYNAMEPHRASE",null,replaceTagsAndSpaces);getParam(d.COMPANYNAME,a.profile,"info.COMPANYNAME",null,replaceTagsAndSpaces);AnyBalance.trace("Получили данные профиля")}catch(e){}}var smsAlreadyEntered=!1,count=3;
