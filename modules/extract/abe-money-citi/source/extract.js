    ...
    if(!JFP_TOKEN) {
		...
		getElements(html, /<script[^>]*>/ig, [/^<script[^>]*>|<\/script>$/ig, ''] , function(str) {
			if(/XXX_Extra/.test(str)) {
				...
				var document = '';
				safeEval(str, 'jQuery,document', [jQueryLocal,document]);
			}
			return str;
		});
		...
		if (!/выйти/i.test(html)) {
			var error = getParam(html, null, null, /<span class="[^"]*error[^>]*>([\s\S]*?)<\//i, replaceTagsAndSpaces);
			if (error)
		...
		// if(select){ //Необходимо выбрать профиль
			...
			// var value = getParam(select, null, null, re, replaceHtmlEntities);
			// if(!value)
