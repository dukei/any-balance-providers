var g_headers={Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8","User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.143 Safari/537.36","Accept-Language":"ru,en-US;q=0.8,en;q=0.6"},baseurl="https://online.vostbank.ru/app/",g_Xml_Headers={Accept:"application/xml, text/xml, */*; q=0.01","X-Requested-With":"XMLHttpRequest","Wicket-Ajax":"true","Wicket-Ajax-BaseURL":".",Referer:baseurl+"main?main=priv"};
function login(a,b){AnyBalance.setDefaultCharset("utf-8");checkEmpty(a.login,"Введите логин!");checkEmpty(a.password,"Введите пароль!");b=AnyBalance.requestGet(baseurl+"main?main=priv",g_headers);if(!b||400<AnyBalance.getLastStatusCode())throw AnyBalance.trace(b),new AnyBalance.Error("Ошибка при подключении к сайту провайдера! Попробуйте обновить данные позже.");if(!/logout/i.test(b)){var c=findWicketActions(b),e=[],d=getParam(b,null,null,/<input[^>]*name="password"[^>]*id="([^"]+)/i),f=getParam(b,
null,null,/<input[^>]*name="login"[^>]*id="([^"]+)/i),g=getParam(b,null,null,/<a[^>]*class="button[^>]*id="([^"]+)/i);c.forEach(function(a,b,c){a=getJson(a);b=(a.u||"").replace(/^.\/main/,"main").replace(/;jsessionid[^?]+/i,"");switch(a.c){case f:e[0]=b;break;case d:e[1]=b;break;case g:e[2]=b}});a=[{login:a.login},{password:a.password}];a[2]=joinObjects(a[0],a[1]);for(c=0;c<e.length;c++)b=AnyBalance.requestPost(baseurl+e[c],a[c],addHeaders(g_Xml_Headers));!/logout/i.test(b)&&/<input[^>]+name="otp"/i.test(b)&&
(AnyBalance.trace("Требуется ввести смс код"),a=getParam(b,/На мобильный телефон[\s\S]*?<\/div>/i,replaceTagsAndSpaces),a=AnyBalance.retrieveCode(a?a+" Введите смс-пароль.":"Введите одноразовый пароль для входа, высланный вам в смс",null,{inputType:"number",time:54E4}),requestGetWicketAction(b,/<input[^>]+name="otp"[^>]*id="([^"]*)/i,{otp:a}),c=getElement(b,/<form[^>]+login-panel/i),c=createFormParams(c),c.otp=a,c.submitOtp="1",b=requestGetWicketAction(b,/<a[^>]+\bbutton\b[^>]*id="([^"]*)/i,c));if(!/logout/i.test(b)){(a=
getElement(b,/<div[^>]+feedbackPanelERROR/i,AB.replaceTagsAndSpaces))||(a=getElement(b,/<[^>]+class="warning"/i,replaceTagsAndSpaces));if(a)throw new AnyBalance.Error(a,null,/парол/i.test(a)&&!/одноразов/i.test(a));AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в личный кабинет. Сайт изменен?");}__setLoginSuccessful()}return b}
...
f.message)}b.credits=[];for(var e=0;e<c.length;++e){var d=getParam(c[e],null,null,/<span[^>]*class=['"]index['"](?:[^>]*>){2}([\s\S]*?)<\/div>/i,replaceTagsAndSpaces),d={__id:d,__name:d};__shouldProcess("credits",d)&&processCredit(c[e],d,a);b.credits.push(d)}}}
