...
function processAccountTransactions(a,d){var c=getParam(a,null,null,/div[^>]+class="more-operations[\s\S]*?href="([^"]*)/i,null,html_entity_decode);if(!c)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось найти ссылку на выписку, сайт изменен?");a=AnyBalance.requestGet(g_baseurl+"/priv/"+c);getParam(a,d,"accounts.minpay",/<td[^>]*>Оплатить до([^<]*)/g,replaceTagsAndSpaces,parseBalance);getParam(a,d,"accounts.minpay_till",/<td[^>]*>Оплатить до[\s\S]*?<td[^>]*>([\s\S]*?)<\/td>/g,replaceTagsAndSpaces,
