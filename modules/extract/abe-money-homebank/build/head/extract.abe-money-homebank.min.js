...
getFingerprint(),d["x-recaptcha"]=solveRecaptcha("Пожалуйста, докажите, что вы не робот.","https://homebank.kz/",JSON.stringify({SITEKEY:"6LeaksYUAAAAAKSdZ3zOupIZj7TGYC67DFlL_MI4",TYPE:"V3",ACTION:"login"}))),e=a);a=AnyBalance.requestPost(c+b,e,d,{HTTP_METHOD:a?"POST":"GET"});if(400<=AnyBalance.getLastStatusCode()){AnyBalance.trace(a);if((a=getJson(a))&&a.UserMessage)throw new AnyBalance.Error(a.UserMessage,null,/(Вы не зарегистрированы|неверный логин)/i.test(a.UserMessage));throw new AnyBalance.Error("Ошибка вызова API "+
b+": "+AnyBalance.getLastStatusCode());}return a=getJson(a)}
