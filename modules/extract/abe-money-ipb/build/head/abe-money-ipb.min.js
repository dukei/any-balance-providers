...
function processCard(b,a){AnyBalance.trace("Обработка карты "+a.__name);getParam(b,a,"cards.balance",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<span[^>]*>){1}([\s\S]*?)<\//i,null,parseBalance);getParam(b,a,"cards.limit",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<td[^>]*>){4}([\s\S]*?)<\//i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"cards.available",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<td[^>]*>){1}([\s\S]*?)<\//i,replaceTagsAndSpaces,parseBalance);getParam(b,a,["cards.currency","cards.balance","cards.limit","cards.available"],
/<span[^>]+id="ctl00_ContentPlaceHolder1_listDocs_ctl02_lblCurrency"[^>]*>([\s\S]*?)<\/span>/i,[replaceTagsAndSpaces,/(.*?)/,"0$1"],parseCurrency);getParam(b,a,"cards.status",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<td[^>]*>){3}([\s\S]*?)<\//i,replaceTagsAndSpaces);getParam(b,a,"cards.till",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<td[^>]*>){2}([\s\S]*?)<\//i,replaceTagsAndSpaces,parseDate)}
function processDeposits(b,a){if(AnyBalance.isAvailable("deposits")){b=AnyBalance.requestGet(baseurl+"deposits.aspx",g_headers);var c=getParam(b,null,null,/<table[^>]+class="GridTable"[^>]*>[^]*?<\/div>/i);if(c){c=getElements(c,/<tr[^>]+class="normal[^>]+>/ig);AnyBalance.trace("Найдено депозитов: "+c.length);a.deposits=[];for(var f=0;f<c.length;++f){var e=c[f],d=getParam(e,null,null,/<tr[^>]*>(?:[\s\S]*?<td[^>]*>){3}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces),g=getParam(e,null,null,/<tr[^>]*>(?:[\s\S]*?<td[^>]*>){4}([\s\S]*?)<\/td>/i,
