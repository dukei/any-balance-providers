...
"Login/login.aspx",a,addHeaders({Referer:baseurl+"Login/login.aspx",Origin:baseurl})));if(/<input[^>]+name="txtCode"/i.test(a)){AnyBalance.trace("Потребовался ввод кода.");var c=getElement(a,/<p[^>]*msgSMSCode[^>]*/i,replaceTagsAndSpaces);getElement(a,/<form[^>]+name="login"[^>]*>/i);a=createFormParams(a,function(a,b,d,g){return"txtCode"==d?AnyBalance.retrieveCode((c||"Пожалуйста, введите код из SMS для входа в интернет-банк.")+'\n\nЧтобы каждый раз не вводить код, вы можете отключить его в своём интернет банке: меню "Настройки системы"/"Настройки информирования"/"Информирование об операциях в системе", затем снять галочку "Запрашивать SMS-код подтверждения при входе". Это безопасно, код подтверждения всё равно будет требоваться для всех операций.',
null,{inputType:"number",time:18E4}):g});a=AnyBalance.requestPost(baseurl+"Login/login.aspx",a,addHeaders({Referer:baseurl+"secure/login.aspx"}))}if(!/sExit/i.test(a)){if(b=getParam(a,null,null,[/<span[^>]+id="lerrortext"[^>]*>([\s\S]*?)<\/span>/i,/<div[^>]+panel2[^>]*>[\s\S]*?<span[^>]+lError[^>]*>([\s\S]*?)<\/span>/i],replaceTagsAndSpaces))throw new AnyBalance.Error(b,null,/(Пароль неверен|не зарегистрирован|Срок действия пароля абонента истек)/i.test(b));AnyBalance.trace(a);throw new AnyBalance.Error("Не удалось зайти в интернет-банк. Сайт изменен?");
}__setLoginSuccessful();return a}function getParamByName(b,a){return getParam(b,null,null,new RegExp("name=[\"']"+a+'["\'][^>]*value=["\']([^"\']+)"',"i"))}
function processCards(b,a){if(AnyBalance.isAvailable("cards")){b=AnyBalance.requestGet(baseurl+"cards.aspx",g_headers);var c=getElement(b,/<table[^>]+class="GridTable"[^>]*>/i),c=getElements(c,/<tr[^>]+class="normal[^>]*>/ig);if(c.length){AnyBalance.trace("Найдено карт: "+c.length);a.cards=[];for(var f=0;f<c.length;++f){var e=c[f],d=getParam(e,null,null,/<tr[^>]*>(?:[\s\S]*?<td[^>]*>){3}([\s\S]*?)<\/td>/i),g=getParam(e,null,null,/<tr[^>]*>(?:[\s\S]*?<td[^>]*>){4}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces),
h=getParam(e,null,null,/<span[^>]+id="ctl00_ContentPlaceHolder1_listDocs_ctl02_lDescr"[^>]*>([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,null),d={__id:d,__name:h,num:g};__shouldProcess("cards",d)&&processCard(e,d);a.cards.push(d)}}else AnyBalance.trace(b),AnyBalance.trace("Не удалось найти карты.")}}
function processCard(b,a){AnyBalance.trace("Обработка карты "+a.__name);getParam(b,a,"cards.limit",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<td[^>]*>){4}([\s\S]*?)<\//i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"cards.available",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<td[^>]*>){1}([\s\S]*?)<\//i,replaceTagsAndSpaces,parseBalance);getParam(b,a,["cards.currency","cards.balance","cards.limit","cards.available"],/<span[^>]+id="ctl00_ContentPlaceHolder1_listDocs_ctl02_lblCurrency"[^>]*>([\s\S]*?)<\/span>/i,[replaceTagsAndSpaces,
/(.*?)/,"0$1"],parseCurrency);getParam(b,a,"cards.status",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<td[^>]*>){3}([\s\S]*?)<\//i,replaceTagsAndSpaces);getParam(b,a,"cards.till",/(?:[\s\S]*?<\/table>){2}(?:[^]*?<td[^>]*>){2}([\s\S]*?)<\//i,replaceTagsAndSpaces,parseDate)}
function processDeposits(b,a){if(AnyBalance.isAvailable("deposits")){b=AnyBalance.requestGet(baseurl+"deposits.aspx",g_headers);var c=getElement(b,/<table[^>]+class="GridTable"[^>]*>/i),c=getElements(c,/<tr[^>]+class="normal[^>]*>/ig);if(c.length){AnyBalance.trace("Найдено депозитов: "+c.length);a.deposits=[];for(var f=0;f<c.length;++f){var e=c[f],d=getParam(e,null,null,/<tr[^>]*>(?:[\s\S]*?<td[^>]*>){3}([\s\S]*?)<\/td>/i,replaceTagsAndSpaces),g=getParam(e,null,null,/<tr[^>]*>(?:[\s\S]*?<td[^>]*>){4}([\s\S]*?)<\/td>/i,
replaceTagsAndSpaces),h=getParam(e,null,null,/<span[^>]+id="ctl00_ContentPlaceHolder1_listDocs_ctl02_lDescr"[^>]*>([\s\S]*?)<\/span>/i,replaceTagsAndSpaces),d={__id:d,__name:h,num:g};__shouldProcess("deposits",d)&&processDeposit(e,d);a.deposits.push(d)}}else AnyBalance.trace(b),AnyBalance.trace("Не удалось найти депозиты.")}}
