...
if(/logout/i.test(a))AnyBalance.trace("Уже залогинены, используем существующую сессию"),a=AnyBalance.requestGet(baseurl+"/bank",g_headers);else{var b=getElement(a,/<form[^>]+LoginForm/i);if(!b)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось получить форму авторизации. Сайт изменен?");a=AB.createFormParams(b,function(a,b,d,e){return"Login"==d?c.login:"Password"==d?c.password:e});a=AnyBalance.requestPost(baseurl+"/entrance/SignIn",a,addHeaders({Referer:baseurl+"/entrance"}));if(b=getParam(a,
null,null,/<div[^>]+class="entrance__inputErrorMessage[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(b,null,/Пароль введен неверно/i.test(b));if(/SMSCodeForm/i.test(a)){AnyBalance.trace("Отправляем запрос на сервер для получения SMS-кода");var b=AnyBalance.requestPost(baseurl+"/entrance/SendEntranceOtp",null,addHeaders({"X-Requested-With":"XMLHttpRequest",Accept:"application/json, text/javascript, */*; q=0.01",Referer:baseurl+"/entrance/SignIn"})),d=getJson(b);if("OK"!==
d.status)throw AnyBalance.trace(b),AnyBalance.trace("Сервер должен был отправить SMS с кодом, но вернул ошибку"),new AnyBalance.Error("Не удалось отправить SMS с кодом. Сайт изменен?");var e=AnyBalance.retrieveCode("Вам отправлено SMS-сообщение с кодом для входа в личный кабинет Каспи банка. Введите код из SMS",null,{inputType:"number",minLength:4,maxLength:4,time:3E5}),b=AnyBalance.requestPost(baseurl+"/entrance/CheckEntranceOtp",{otp:e},addHeaders({"X-Requested-With":"XMLHttpRequest",Accept:"application/json, text/javascript, */*; q=0.01",
"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",Referer:baseurl+"/entrance/SignIn"})),d=getJson(b);if("INVALID_OTP"===d.status)throw AnyBalance.trace("Введен некорректный код из SMS"),new AnyBalance.Error("Вы ввели некорректный код из SMS. Попробуйще ещё раз.");if("OK"!==d.status)throw AnyBalance.trace(b),AnyBalance.trace("При проверке кода из SMS произошла неизвестная ошибка."),new AnyBalance.Error("Не удалось проверить код из SMS. Сайт изменен?");b=getElement(a,/<form[^>]+SMSCodeForm/i);
if(!b)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось получить форму ввода кода из SMS. Сайт изменен?");a=AB.createFormParams(b,function(a,b,d,h){return"Otp"==d?e:"Login"==d?c.login:h});a=AnyBalance.requestPost(baseurl+"/entrance/SignIn",a,addHeaders({Referer:baseurl+"/entrance/SignIn"}))}if(!/logout/i.test(a))throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось зайти в интернет-банк. Сайт изменен?");a=AnyBalance.requestGet(baseurl+"/bank",g_headers);AnyBalance.setData&&(AnyBalance.setData("kaspi-tag",
AnyBalance.getCookie("kaspi-tag")),AnyBalance.saveData())}__setLoginSuccessful();return a}
