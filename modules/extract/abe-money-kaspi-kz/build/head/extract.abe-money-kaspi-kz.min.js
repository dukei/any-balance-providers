...
function processCards(a,b){if(AnyBalance.isAvailable("cards")){a=AnyBalance.requestGet(baseurl+"/index.aspx?action=bank",g_headers);var c=AB.getElement(a,/<div[^>]+id="item_creditcards"[^>]*>/i);if(c)for(a=AB.getElements(c,/<div[^>]+credicardInfoWrapper[^>]*>/ig),a||(AnyBalance.trace(c),AnyBalance.trace("Не удалось найти карты. Сайт изменён?")),AnyBalance.trace("Найдено карт: "+a.length),b.cards=[],c=0;c<a.length;++c){var d=a[c],e=AB.getParam(d,null,null,/<div[^>]+title--creditcard(?:[^>]*>){3}([\s\S]*?)<span/i,
AB.replaceTagsAndSpaces),f=AB.getParam(d,null,null,/<div[^>]+title--creditcard(?:[^>]*>){3}([\s\S]*?)<span/i,AB.replaceTagsAndSpaces),g=AB.getParam(d,null,null,/<div[^>]+title--creditcard(?:[^>]*>){3}([\s\S]*?)<span/i,AB.replaceTagsAndSpaces),e={__id:e,__name:g,num:f};__shouldProcess("cards",e)&&processCard(d,e);b.cards.push(e)}else/У вас нет карт/i.test(a)?(AnyBalance.trace("У вас нет карт"),b.cards=[]):(AnyBalance.trace(a),AnyBalance.trace("Не удалось найти таблицу с картами."))}}
function processCard(a,b){AnyBalance.trace("Обработка карты "+b.__name);AB.getParam(a,b,"cards.balance",/<span[^>]+"e-limit__info__amount"[^>]*>[\s\S]*?<div/i,AB.replaceTagsAndSpaces,AB.parseBalanceSilent);AB.getParam(a,b,["cards.currency","cards.balance"],/<span[^>]+"e-limit__info__amount"[^>]*>([\s\S]*?)<div/i,AB.replaceTagsAndSpaces,AB.parseCurrency);AB.getParam(a,b,"cards.debt",/<span[^>]+"e-limit__subtitle e-debt__title"[^>]*>[\s\S]*?<div/i,AB.replaceTagsAndSpaces,AB.parseBalanceSilent);AB.getParam(a,
b,"cards.pay_till",/<span[^>]+"e-transfer__day"[^>]*>[\s\S]*?<div/i,AB.replaceTagsAndSpaces,AB.parseDateWord);var c=AB.getParam(a,null,null,/<a[^>]+href="([^"]*)"[^>]*>[\s\S]*?Информация о карте/i,[/#\d*/,""]);c?(c=AnyBalance.requestGet(baseurl+c,g_headers),AB.getParam(c,b,"cards.agreement",/Номер договора:(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces),AB.getParam(c,b,"cards.limit",/Кредитный лимит:(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces,AB.parseBalance),AB.getParam(c,b,"cards.status",/Статус:(?:[^>]*>){2}([^<]*)/i,
AB.replaceTagsAndSpaces),AB.getParam(c,b,"cards.num",/<div[^>]+class="card_number"[^>]*>([^<]*)/i,AB.replaceTagsAndSpaces),AB.getParam(c,b,"cards.type",/<div[^>]+class="card_holder"[^>]*>([^<]*)/i,AB.replaceTagsAndSpaces),AB.getParam(c,b,"cards.till",/<div[^>]+class="card_valid_thru"[^>]*>([^<]*)/i,AB.replaceTagsAndSpaces,AB.parseDateWord)):(AnyBalance.trace(a),AnyBalance.trace("Не удалось найты ссылку на страницу с информацией о карте. Сайт изменён?"));isAvailable("cards.transactions")&&processCardTransactions(a,
b)}
