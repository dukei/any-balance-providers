...
function processDeposits(c,a){if(AnyBalance.isAvailable("deposits")){var b=getParam(c,null,null,/<a[^>]+class="myProductsMenu__link[^>]+href="([^>]*?)"[^>]*>\s*Депозиты/i);c=AnyBalance.requestGet(baseurl+b,g_headers);b=AB.getElements(c,/<a[^>]+myProductsSubMenu__link[^>]*\/bank\/Deposit\/\d+"[^>]*>/ig);b.length||(AnyBalance.trace(c),AnyBalance.trace("Не удалось найти депозиты. Сайт изменён?"));AnyBalance.trace("Найдено депозитов: "+b.length);a.deposits=[];for(var d=0;d<b.length;d++)if(c=AB.getParam(b[d],
null,null,/<a[^>]+href="([^"]*)"[^>]*>/i)){c=AnyBalance.requestGet(joinUrl(baseurl,c+"/About"),g_headers);var e=AB.getParam(c,null,null,/Номер счета Депозита:(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces),f=AB.getParam(c,null,null,/Номер счета Депозита:(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces),g=AB.getParam(c,null,null,/Название(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces),e={__id:e,__name:g,num:f};__shouldProcess("deposits",e)&&processDeposit(b[d],e,c);a.deposits.push(e)}else AnyBalance.trace("Не удалось найти ссылку на страницу с информацией о депозите. Сайт изменён?")}}
