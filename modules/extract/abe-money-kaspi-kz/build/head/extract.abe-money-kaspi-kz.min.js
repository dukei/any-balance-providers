...
function login(){var c=AnyBalance.getPreferences();AnyBalance.setDefaultCharset("utf-8");if(AnyBalance.getData){var a=AnyBalance.getData("kaspi-tag");a&&AnyBalance.setCookie("kaspi.kz","kaspi-tag",a)}checkEmpty(c.login,"Введите логин!");checkEmpty(c.password,"Введите пароль!");a=AnyBalance.requestGet(baseurl+"/entrance",g_headers);if(!a||400<AnyBalance.getLastStatusCode())throw AnyBalance.trace(a),new AnyBalance.Error("Ошибка при подключении к сайту интернет-банка! Попробуйте обновить данные позже.");
if(/logout/i.test(a))AnyBalance.trace("Уже залогинены, используем существующую сессию"),a=AnyBalance.requestGet(baseurl+"/bank",g_headers);else{var b=getElement(a,/<form[^>]+LoginForm/i);if(!b)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось получить форму авторизации. Сайт изменен?");a=AB.createFormParams(b,function(a,b,d,e){return"Login"==d?c.login:"Password"==d?c.password:e});a=AnyBalance.requestPost(baseurl+"/SignIn",a,addHeaders({Referer:baseurl+"/entrance"}));if(b=getParam(a,null,
null,/<div[^>]+class="entrance__inputErrorMessage[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(b,null,/Пароль введен неверно/i.test(b));if(/SMSCodeForm/i.test(a)){AnyBalance.trace("Отправляем запрос на сервер для получения SMS-кода");var b=AnyBalance.requestPost(baseurl+"/SendEntranceOtp",null,addHeaders({"X-Requested-With":"XMLHttpRequest",Accept:"application/json, text/javascript, */*; q=0.01",Referer:baseurl+"/SignIn"})),d=getJson(b);if("OK"!==d.status)throw AnyBalance.trace(b),
AnyBalance.trace("Сервер должен был отправить SMS с кодом, но вернул ошибку"),new AnyBalance.Error("Не удалось отправить SMS с кодом. Сайт изменен?");var e=AnyBalance.retrieveCode("Вам отправлено SMS-сообщение с кодом для входа в личный кабинет Каспи банка. Введите код из SMS",null,{inputType:"number",minLength:4,maxLength:4,time:3E5}),b=AnyBalance.requestPost(baseurl+"/CheckEntranceOtp",{otp:e},addHeaders({"X-Requested-With":"XMLHttpRequest",Accept:"application/json, text/javascript, */*; q=0.01",
"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",Referer:baseurl+"/SignIn"})),d=getJson(b);if("INVALID_OTP"===d.status)throw AnyBalance.trace("Введен некорректный код из SMS"),new AnyBalance.Error("Вы ввели некорректный код из SMS. Попробуйще ещё раз.");if("OK"!==d.status)throw AnyBalance.trace(b),AnyBalance.trace("При проверке кода из SMS произошла неизвестная ошибка."),new AnyBalance.Error("Не удалось проверить код из SMS. Сайт изменен?");b=getElement(a,/<form[^>]+SMSCodeForm/i);
if(!b)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось получить форму ввода кода из SMS. Сайт изменен?");a=AB.createFormParams(b,function(a,b,d,h){return"Otp"==d?e:"Login"==d?c.login:h});a=AnyBalance.requestPost(baseurl+"/SignIn",a,addHeaders({Referer:baseurl+"/SignIn"}))}if(!/logout/i.test(a))throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось зайти в интернет-банк. Сайт изменен?");AnyBalance.setData&&(AnyBalance.setData("kaspi-tag",AnyBalance.getCookie("kaspi-tag")),AnyBalance.saveData())}__setLoginSuccessful();
return a}
function processCards(c,a){if(AnyBalance.isAvailable("cards")){var b=[],d=getParam(c,null,null,/<a[^>]+class="myProductsMenu__link[^>]+href="([^>]*?)"[^>]*>\s*Kaspi Gold/i);if(d){AnyBalance.trace("Нашли карту Kaspi Gold, получаем базовую информацию.");c=AnyBalance.requestGet(baseurl+d,g_headers);var e=AB.getParam(c,null,null,/<div[^>]+bankProducts__item--gold[\s\S]*?bankProducts__title[^>]*?>([\s\S]*?)<div/i,AB.replaceTagsAndSpaces);b.push({url:d,type:"gold",title:e,html:c})}if(b.length)for(AnyBalance.trace("Найдено карт: "+b.length),
a.cards=[],c=0;c<b.length;++c)d=b[c],e={__id:d.title,__name:d.title,num:d.title},__shouldProcess("cards",e)&&processCard(d,e),a.cards.push(e);else AnyBalance.trace(c),AnyBalance.trace("У вас нет карт.")}}
...
function processDeposits(c,a){if(AnyBalance.isAvailable("deposits")){var b=getParam(c,null,null,/<a[^>]+class="myProductsMenu__link[^>]+href="([^>]*?)"[^>]*>\s*Депозиты/i);c=AnyBalance.requestGet(baseurl+b,g_headers);b=AB.getElements(c,/<a[^>]+myProductsSubMenu__link[^>]*>/ig);/\/History$/i.test(AnyBalance.getLastUrl())&&(AnyBalance.trace("Депозит, видимо, только один. Оказались на его странице"),b=['<a href="'+AnyBalance.getLastUrl().replace(/\/History$/i,"")+'">']);b.length||(AnyBalance.trace(c),
AnyBalance.trace("Не удалось найти депозиты. Сайт изменён?"));AnyBalance.trace("Найдено депозитов: "+b.length);a.deposits=[];for(var d=0;d<b.length;d++)if(c=AB.getParam(b[d],null,null,/<a[^>]+href="([^"]*)"[^>]*>/i)){c=AnyBalance.requestGet(joinUrl(baseurl,c+"/About"),g_headers);var e=AB.getParam(c,null,null,/Номер счета Депозита:(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces),f=AB.getParam(c,null,null,/Номер счета Депозита:(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces),g=AB.getParam(c,null,null,
/Название(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces),e={__id:e,__name:g,num:f};__shouldProcess("deposits",e)&&processDeposit(b[d],e,c);a.deposits.push(e)}else AnyBalance.trace("Не удалось найти ссылку на страницу с информацией о депозите. Сайт изменён?")}}
function processDeposit(c,a,b){AB.getParam(b,a,"deposits.balance",/Накоплено(?:[^>]*>){2}([\s\S]*?)<\/div>/i,AB.replaceTagsAndSpaces,AB.parseBalance);AB.getParam(b,a,"deposits.agreement",/Номер договора(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces);AB.getParam(b,a,"deposits.date_start",/Вы открыли вклад(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces,AB.parseDate);AB.getParam(b,a,"deposits.pct",/Эффективная ставка(?:[^>]*>){2}([^<]*)/i,AB.replaceTagsAndSpaces,AB.parseBalance);AB.getParam(b,a,"deposits.withdraw",
/Доступно для снятия(?:[\s\S]*?<span[^>]*>)([^<]*)/i,AB.replaceTagsAndSpaces,AB.parseBalance);AB.getParam(b,a,"deposits.currency",/Валюта Депозита(?:[^>]*>){2}([^<]*)/i,[AB.replaceTagsAndSpaces,/(.*?)/i,"0$1"],AB.parseCurrency);AB.getParam(b,a,"deposits.dvk_status",/<div[^>]+class="text dvk_status"(?:[^>]*>){4}([^<]*)/i,AB.replaceTagsAndSpaces);AB.getParam(b,a,"deposits.dvk_num",/<div[^>]+class="dvkcard_number"[^>]*>([^<]*)/i,AB.replaceTagsAndSpaces);AB.getParam(b,a,"deposits.dvk_till",/<div[^>]+class="card_valid_thru"[^>]*>([^<]*)/i,
AB.replaceTagsAndSpaces,AB.parseDate)}function processCredits(c,a){throw new AnyBalance.Error("Получение информации о кретидах ещё не реализовано. Обратитесь к разработчику программы.");}
