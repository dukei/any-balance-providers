function sleep(b){AnyBalance.trace("Calling hw sleep");AnyBalance.sleep(b)}function encryptPass(b,c){if(c){var a;c=c.split(",");for(var d="";""!=b;)a=b.substr(0,1),a=a.charCodeAt(0),255<a&&(a-=848),7622==a&&(a=185),b=1<b.length?b.substr(1,b.length):"",""!=d&&(d+=";"),d+=c[a];return d}return b}var g_headers={"Accept-Language":"ru, en",BSSHTTPRequest:1,"User-Agent":"Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.1 (KHTML, like Gecko) Chrome/21.0.1180.60 Safari/537.1"},g_baseurl;
...
f=getElement(e,/<table[^>]+id="SCROLLER"[^>]*>/i);if(!f){var g=getParam(e,null,null,/<BSS_ERROR>([\s\S]*?)<\/BSS_ERROR>/i,replaceTagsAndSpaces);if(g&&(AnyBalance.trace("Не получили транзакции с ошибкой: "+g),g=getParam(g,null,null,/Выписка будет доступна через\s*(\d+)\s*сек/i,null,parseBalance))){AnyBalance.trace("Ждем "+g+" сек...");sleep(1E3*g);continue}}if(!f){AnyBalance.trace("Похоже, транзакций по счету "+c.__id+" нет...\n"+e);return}break}while(1);b=getElements(f,/<(?:thead|tbody)[^>]*>/ig);
if(!/<thead/i.test(""+b[0]))throw AnyBalance.trace(e),new AnyBalance.Error("Не удаётся найти заголовок таблицы транзакций счета. Сайт изменен?");if(!/<tbody/i.test(""+b[1]))throw AnyBalance.trace(e),new AnyBalance.Error("Не удаётся найти тело таблицы транзакций счета. Сайт изменен?");c.transactions=[];e=getColsAccountTransactions(b[0]);b=getElements(b[1],/<tr[^>]*>/ig);for(a=0;a<b.length;++a)d=b[a],/colspan/i.test(d)||(d=getElements(d,/<td[^>]*>/ig),f={},isset(e.date)&&getParam(d[e.date],f,"accounts.transactions.date",
null,replaceTagsAndSpaces,parseDate),isset(e.sum)&&getParam(d[e.sum],f,"accounts.transactions.sum",null,replaceTagsAndSpaces,parseBalance),isset(e.opnum)&&getParam(d[e.opnum],f,"credits.transactions.opnum",null,replaceTagsAndSpaces),isset(e.corrnum)&&getParam(d[e.corrnum],f,"credits.transactions.corrnum",null,replaceTagsAndSpaces),isset(e.corrname)&&getParam(d[e.corrname],f,"credits.transactions.corrname",null,replaceTagsAndSpaces),isset(e.descr)&&getParam(d[e.descr],f,"credits.transactions.descr",
null,replaceTagsAndSpaces),c.transactions.push(f))}};
