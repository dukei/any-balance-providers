...
function requestJson(b,c,d){var a=[],e;for(e in b)a.push(encodeURIComponent(e)+"="+encodeURIComponent(b[e]));a.push(encodeURIComponent("cookie")+"="+encodeURIComponent(g_session));a.push(encodeURIComponent("simSN")+"="+encodeURIComponent(g_simsn));a.push(encodeURIComponent("imei")+"="+encodeURIComponent(g_login_hash));a.push(encodeURIComponent("appkey")+"="+encodeURIComponent(g_appKey));a.push(encodeURIComponent("language")+"=ru");a.push(encodeURIComponent("lon")+"=0.0");a.push(encodeURIComponent("ireal")+
"="+encodeURIComponent(g_imei));a.push(encodeURIComponent("device")+"="+encodeURIComponent("SM-G900F|samsung"));a.push(encodeURIComponent("version")+"="+encodeURIComponent("5.20.02"));a.push(encodeURIComponent("versionOS")+"="+encodeURIComponent("5.0"));a.push(encodeURIComponent("lat")+"="+encodeURIComponent("0.0"));b=g_baseurl+"iapi2/"+c+"?"+a.join("&");c=AnyBalance.requestGet(b,g_headers);clearAllCookies();c=getJson(c);if("ok"!=c.st&&(AnyBalance.trace("Error getting "+b+": "+JSON.stringify(c)),
d))throw new AnyBalance.Error(d+": "+(c.err||"").replace(/:\s[^&]&/,": "),null,/Неверный логин или пароль/i.test(c.err));return c}
function login(b,c){b=AnyBalance.getPreferences();AnyBalance.setDefaultCharset("utf-8");checkEmpty(b.login,"Введите логин!");checkEmpty(b.password,"Введите пароль!");var d="+"+b.login.replace(/\D+/g,"");if(g_session){AnyBalance.trace("Найдена активная сессия, проверим её.");var a=requestJson({},"props_full");"ok"!=a.st?(AnyBalance.trace("Сессия испорчена, будем заново авторизовываться"),g_session=""):AnyBalance.trace("Сессия действует, используем её.")}if(g_session)AnyBalance.trace();else{a=hex_md5(d);
AnyBalance.trace(a+" "+typeof a);g_login_hash=getParam(a,null,null,null,[/([\s\S]{8})([\s\S]{4})([\s\S]{4})([\s\S]{4})([\s\S]{12})/,"$1-$2-$3-$4-$5"]);AnyBalance.trace("Login imei param is: "+g_login_hash);g_imei=generateImei(d,g_imei);g_simsn=generateSimSN(d,g_simsn);a=requestJson({},"props_full");if(/phone not linked to imei/i.test(a.err)){AnyBalance.trace("Устройство нужно привязать.");requestJson({},"unlink_phone");a=requestJson({login:d},"auth_phone","Не удалось начать процесс привязки");d=a.id;
a=requestJson({id:d,pass:b.password},"auth_pass","Не удалось зайти с паролем");for(b=0;"redirect"!=a.nextCmd;)if(a=performNextCmd(a.nextCmd,d),5<++b)throw AnyBalance.trace(JSON.stringify(a)),new AnyBalance.Error("Слишком много подтверждений на вход. Сайт изменен?");g_session=a.cookie;AnyBalance.trace("Успешно привязали устройство: "+JSON.stringify(a))}else if(/CODE104: need auth/i.test(a.err))AnyBalance.trace("Похоже что устройство уже привязано."),a=requestJson({pass:b.password,bank:""},"chpass",
"Не удалось авторизоваться, проверьте логин и пароль."),g_session=a.cookie;else throw AnyBalance.trace(JSON.stringify(a)),new AnyBalance.Error(a.err);__setLoginSuccessful()}if(!g_session)throw new AnyBalance.Error("Не удалось получить токен авторизации!");requestJson({},"banks");a=requestJson({},"props_full","Банк не отдал данные");AnyBalance.isAvailable("info.mphone")&&(c.info={},getParam(a.phone,c.info,"info.mphone"));return a}
