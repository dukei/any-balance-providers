...
AnyBalance.trace(b+" "+typeof b);g_login_hash=getParam(b,null,null,null,[/([\s\S]{8})([\s\S]{4})([\s\S]{4})([\s\S]{4})([\s\S]{12})/,"$1-$2-$3-$4-$5"]);AnyBalance.trace("Login imei param is: "+g_login_hash);g_imei=generateImei(a.login,g_imei);g_simsn=generateSimSN(a.login,g_simsn);b=requestJson({registration_id:g_registrationId},"props_full");if(/phone not linked to imei/i.test(b.err)){AnyBalance.trace("Устройство нужно привязать.");requestJson({},"unlink_phone");b=requestJson({login:a.login},"auth_phone",
"Не удалось начать процесс привязки");b=b.id;requestJson({id:b,pass:a.password},"auth_pass","Не удалось зайти с паролем");var d=AnyBalance.retrieveCode("Пожалуйста, введите код из смс, для привязки данного устройства.",null,{time:3E5}),b=requestJson({id:b,otp:d},"auth_otp","Не удалось привязать устройство");g_session=b.cookie;AnyBalance.trace("Успешно привязали устройство.")}else AnyBalance.trace("Похоже что устройство уже привязано."),b=requestJson({pass:a.password,bank:""},"chpass","Не удалось авторизоваться, проверьте логин и пароль."),
g_session=b.cookie;__setLoginSuccessful()}if(!g_session)throw new AnyBalance.Error("Не удалось получить токен авторизации!");requestJson({},"banks");b=requestJson({registration_id:g_registrationId},"props_full");AnyBalance.isAvailable("info.mphone")&&(c.info={},getParam(b.phone,c.info,"info.mphone"));return b}
