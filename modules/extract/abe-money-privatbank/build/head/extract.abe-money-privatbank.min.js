...
pass:b.password},"auth_pass","Не удалось зайти с паролем"),a=AnyBalance.retrieveCode("Пожалуйста, введите код из смс, для привязки данного устройства.",null,{time:3E5}),a=requestJson({id:d,otp:a},"auth_otp","Не удалось привязать устройство"),g_session=a.cookie,AnyBalance.trace("Успешно привязали устройство: "+JSON.stringify(a))):(AnyBalance.trace("Похоже что устройство уже привязано."),a=requestJson({pass:b.password,bank:""},"chpass","Не удалось авторизоваться, проверьте логин и пароль."),g_session=
a.cookie),__setLoginSuccessful());if(!g_session)throw new AnyBalance.Error("Не удалось получить токен авторизации!");requestJson({},"banks");a=requestJson({},"props_full");AnyBalance.isAvailable("info.mphone")&&(c.info={},getParam(a.phone,c.info,"info.mphone"));return a}
