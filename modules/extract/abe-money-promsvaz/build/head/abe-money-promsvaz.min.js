...
function requestAndCheckForErrors(b,a,c,d,f){b=/POST/.test(b)?AnyBalance.requestPost(a,c,d):AnyBalance.requestGet(a,d);if(f&&!/pageRedirect/i.test(b))throw AnyBalance.trace(b),new AnyBalance.Error("Не удаётся войти в интернет банк (внутренняя ошибка сайта)");if(/KeyAuth/i.test(b))throw new AnyBalance.Error("Для входа в интернет-банк требуются одноразовые пароли. Зайдите в интернет-банк с компьютера и отключите в Настройках требование одноразовых паролей при входе. Это безопасно, для операций по переводу денег пароли всё равно будут требоваться.");
if(/UpdateContactInfo.aspx/i.test(b))throw new AnyBalance.Error("Для входа в интернет-банк требуются обновить контактную информацию. Зайдите в интернет-банк с компьютера и следуйте инструкциям.");f&&(f=getParam(b,null,null,/pageRedirect\|\|([^\|]*)/i,replaceTagsAndSpaces,decodeURIComponent),AnyBalance.trace("Нашли ссылку "+f),b=AnyBalance.requestGet(g_baseurl+f,g_headers));if(f=getParam(b,null,null,/<div[^>]*class="errorMessage"[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(f,
null,/Неверные учетные данные/i.test(f));return b}
function login(){var b=AnyBalance.getPreferences();AnyBalance.setDefaultCharset("utf-8");checkEmpty(b.login,"Пожалуйста, укажите логин для входа в интернет-банк Промсвязбанка!");checkEmpty(b.password,"Пожалуйста, укажите пароль для входа в интернет-банк Промсвязбанка!");var a=AnyBalance.requestGet(g_baseurl+"/n/Main/Home.aspx",g_headers);if(/logout/i.test(a))AnyBalance.trace("Уже внутри. Используем существующую сессию.");else{var c=getEventValidation(a),d=getViewState(a),a=getViewStateGenerator(a);
...
requestAndCheckForErrors("POST",g_baseurl+"/n/Default.aspx",{ctl00$ScriptManager:"ctl00$mainArea$upLogin|ctl00$mainArea$btnLogin",__EVENTTARGET:"",__EVENTARGUMENT:"",__VIEWSTATE:d,__VIEWSTATEENCRYPTED:"",__EVENTVALIDATION:c,__VIEWSTATEGENERATOR:a,ctl00$mainArea$LoginInput$vtcLogin:b.login,ctl00$mainArea$vtcPassword:b.password,__ASYNCPOST:!0,ctl00$mainArea$btnLogin:"Войти"},addHeaders({Referer:"https://retail.payment.ru/n/Default.aspx"}),!0);a=requestAndCheckForErrors("GET",g_baseurl+"/n/Main/Home.aspx",
...
function processAccounts(b,a){if(AnyBalance.isAvailable("accounts")){var c=getEventValidation(b),d=getViewState(b);/<div[^>]*isDetExpandBtn[^>]*accountList[^>]*>\s*подробно/i.test(b)&&(b=AnyBalance.requestPost(g_baseurl+"/n/Main/Home.aspx",{ctl00$ctl00$ScriptManager:"ctl00$ctl00$mainArea$main$upAccounts|ctl00$ctl00$mainArea$main$accountList",__EVENTTARGET:"ctl00$ctl00$mainArea$main$accountList",__EVENTARGUMENT:"_det_exp",__VIEWSTATE:d,__VIEWSTATEENCRYPTED:"",__EVENTVALIDATION:c,ctl00$ctl00$mainArea$right$OperationSearchRightColumn$OperationSearch$tbSearch:"",
ctl00_ctl00_mainArea_right_OperationSearchRightColumn_OperationSearch_tbSearch_InitialTextMode:"True",__ASYNCPOST:"true","":""},addHeaders({Referer:g_baseurl+"/n/Main/Home.aspx"})));c=getElements(b,/<div[^>]+class="infoUnit"[^>]*>/ig);AnyBalance.trace("Найдено счетов: "+c.length);a.accounts||(a.accounts=[]);for(d=0;d<c.length;++d){var f=c[d],g=getElement(f,/<div[^>]+"infoUnitCaption[^>]*>/i,replaceTagsAndSpaces),e=getElement(f,/<a[^>]+"infoUnitObject[^>]*>/i,[replaceTagsAndSpaces,/\s+/g,""]),g={__id:e,
__name:g+" *"+e.substr(-4),num:e};__shouldProcess("accounts",g)&&processAccount(f,g);a.accounts.push(g)}}}
function processAccount(b,a){AnyBalance.trace("Обработка счета "+a.__name);getParam(b,a,"accounts.balance",/"balanceAmountM"[^>]*>([^<]*)/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,["accounts.currency","accounts.balance","accounts.blocked","accounts.balance_own"],/"balanceAmountCurrencyM"[^>]*>([^<]*)/i,replaceTagsAndSpaces);AnyBalance.isAvailable("accounts.blocked","accounts.balance_own","accounts.unused_credit","accounts.minpay","accounts.minpay_till","accounts.gracepay","accounts.gracepay_till",
"accounts.contract","accounts.limit","accounts.pct","accounts.transactions")&&processAccountDetails(b,a)}
function processAccountDetails(b,a){AnyBalance.trace("Получаем детальную информацию по счету "+a.__name);var c=getParam(b,null,null,/"infoUnitObject"[^>]*href="([^"]*)/i),c=AnyBalance.requestGet(g_baseurl+c,g_headers);getParam(c,a,"accounts.balance_own",/"ctl00_ctl00_mainArea_main_lblAccountBalance"[^>]*>([^<]*)/i,replaceTagsAndSpaces,parseBalance);getParam(c,a,"accounts.blocked",/"ctl00_ctl00_mainArea_main_lblReserved"[^>]*>([^<]*)/i,replaceTagsAndSpaces,parseBalance);getParam(c,a,"accounts.unused_credit",
...
function processAccountsPreliminaryDetails(b,a,c){var d=getElement(b,/<div[^>]+accountDetailTextBlock[^>]*>/i);if(d)for(var f=getElement(b,/<div[^>]+accountDetailValueBlock[^>]*>/i),d=getElements(d.substr(1),/<div[^>]*>/ig),f=getElements(f.substr(1),/<div[^>]*>/ig),g=0;g<d.length;g++){var e=d[g],h=f[g];/ИТОГО/i.test(e)?(getParam(h,a,[c+"currency",c+"balance",c+"blocked",c+"balance_own"],/<div[^>]+curNameISO[^>]*>([\s\S]*?)<\/div>/i,replaceTagsAndSpaces),getParam(h,a,c+"balance",null,replaceTagsAndSpaces,
parseBalance)):/собственные средства/i.test(e)?getParam(h,a,c+"balance_own",null,replaceTagsAndSpaces,parseBalance):/кредитные средства/i.test(e)&&getParam(h,a,c+"unused_credit",null,replaceTagsAndSpaces,parseBalance)}getParam(b,a,c+"balance",/"balanceAmountM"[^>]*>([^<]*)/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,[c+"currency",c+"balance",c+"blocked",c+"balance_own"],/"balanceAmountCurrencyM"[^>]*>([^<]*)/i,replaceTagsAndSpaces);"cards."==c?(getParam(b,a,c+"accnum",/<a[^>]+class="infoUnitObject[^>]*>([\s\S]*?)<\/a>/i,
...
function processCards(b,a){if(AnyBalance.isAvailable("cards","accounts")){a.cards=[];a.accounts||(a.accounts=[]);b=AnyBalance.requestGet(g_baseurl+"/n/Main/CardsAndAccounts.aspx",addHeaders({Referer:g_baseurl+"/n/Main/Home.aspx"}));var c=getElements(b,/<div[^>]+class="(?:cardAccountBlock|card)"/ig);AnyBalance.trace("Найдено карт и карточных счетов: "+c.length);for(var d=null,f=0;f<c.length;++f){var g=c[f];if(/cardAccountBlock/i.test(g)){if(AnyBalance.isAvailable("accounts")&&d){var e=getElement(d,
/<div[^>]+"infoUnitCaption[^>]*>/i,replaceTagsAndSpaces),h=getElement(d,/<a[^>]+"infoUnitObject[^>]*>/i,[replaceTagsAndSpaces,/\s+/g,""]),e={__id:h,__name:e+" *"+h.substr(-4),num:h};__shouldProcess("accounts",e)&&(processAccountsPreliminaryDetails(d,e,"accounts."),processAccount(d,e));a.accounts.push(e)}d=g}else AnyBalance.isAvailable("cards")&&(/emptyAccount/i.test(g)?AnyBalance.trace("Пустая карта, пропускаем"):(e=getParam(g,null,null,/<span[^>]+class="cardnumber[^>]*>([\s\S]*?)<\/span>/i,replaceTagsAndSpaces),
h=getParam(g,null,null,/<a[^>]+class="cardStandAloneCaption[^>]*>([\s\S]*?)<\/a>/i,replaceTagsAndSpaces),e={__id:e,num:e,__name:h+" *"+e.substr(-4)},__shouldProcess("cards",e)&&(processAccountsPreliminaryDetails(d,e,"cards."),processCard(g,e)),a.cards.push(e)))}delayedProcessAccount(d)}}
...
getParam(b,c,"info.smsphone",/<span[^>]+id="[^"]*lblSmsPhone"[^>]*>([\s\S]*?)<\/span>/i,replaceTagsAndSpaces),getParam(b,c,"info.alias",/<input[^>]+vtcUserAlias[^>]*value="([^"]*)/i,replaceHtmlEntities))}
