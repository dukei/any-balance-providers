...
function login(a){AnyBalance.setDefaultCharset("utf-8");a=AnyBalance.getPreferences();checkEmpty(a.login,"Введите номер телефона!");checkEmpty(a.password,"Введите пароль!");AnyBalance.setOptions({SSL_ENABLED_PROTOCOLS:["TLSv1.2"]});AnyBalance.requestGet(baseurl+"payment/main.action");var b=a.login;/^\d{10}$/i.test(a.login)?b="+7"+a.login:/^\s*\+/.test(a.login)||(b="+"+a.login);var c;try{c=requestAPI({action:"cas/tgts",isAuth:!0},{login:b,password:a.password})}catch(d){AnyBalance.trace("Киви затребовало капчу..."),
/робот|robot/i.test(d.message)&&(c=requestAPI({action:"cas/tgts",isAuth:!0},{login:b,password:a.password,captcha:solveRecaptcha("Пожалуйста, докажите, что вы не робот.",baseurl,"6LfjX_4SAAAAAFfINkDklY_r2Q5BRiEqmLjs4UAC")}))}c=requestAPI({action:"cas/sts",isAuth:!0},{ticket:c.entity.ticket,service:baseurl+"j_spring_cas_security_check"});a=AnyBalance.requestGet(baseurl+"j_spring_cas_security_check?ticket="+c.entity.ticket,addHeaders({Referer:baseurl}));if(/Внимание! Срок действия вашего пароля истек/i.test(a))throw new AnyBalance.Error("Внимание! Срок действия вашего пароля истек. Зайдите в кошелек через браузер и следуйте инструкции.",
