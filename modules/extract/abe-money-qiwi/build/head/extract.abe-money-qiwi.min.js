...
function login(a){AnyBalance.setDefaultCharset("utf-8");a=AnyBalance.getPreferences();checkEmpty(a.login,"Введите номер телефона!");checkEmpty(a.password,"Введите пароль!");AnyBalance.setOptions({SSL_ENABLED_PROTOCOLS:["TLSv1.2"]});AnyBalance.requestGet(baseurl+"payment/main.action");var b=a.login;/^\d{10}$/i.test(a.login)?b="+7"+a.login:/^\s*\+/.test(a.login)||(b="+"+a.login);a=requestAPI({action:"cas/tgts",isAuth:!0},{login:b,password:a.password});a=requestAPI({action:"cas/sts",isAuth:!0},{ticket:a.entity.ticket,
service:baseurl+"j_spring_cas_security_check"});a=AnyBalance.requestGet(baseurl+"j_spring_cas_security_check?ticket="+a.entity.ticket,addHeaders({Referer:baseurl}));if(/Внимание! Срок действия вашего пароля истек/i.test(a))throw new AnyBalance.Error("Внимание! Срок действия вашего пароля истек. Зайдите в кошелек через браузер и следуйте инструкции.",null,!0);AnyBalance.trace("Успешно вошли...");return a}
...
function requestAPI(a,b,c){b=AnyBalance.requestPost(a.isAuth?baseurlAuth+a.action:baseurl+a.action,JSON.stringify(b),c?addHeaders(g_headers,c):g_headers);AnyBalance.trace("Request result: "+b);c=getJson(b);if(a.isAuth){if(!c.entity||!c.entity.ticket){if(c.entity&&(a=c.entity.error.message,/робот/i.test(a)&&(a="Qiwi просит подтвердить, что вы не робот. Зайдите с вашим логином и паролем на вебсайт https://w.qiwi.com, пройдите проверку, что вы не робот. После этого ещё раз обновите провайдер."),a))throw new AnyBalance.Error(a,
null,/Неверный логин или пароль|Неправильный номер телефона или пароль/i.test(a));AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось войти в Qiwi Visa Wallet, сайт изменен?");}}else if(!c.data){if(a=c.message)throw new AnyBalance.Error("Сайт qiwi.ru сообщает: "+a+". Попробуйте обновить данные позже.");AnyBalance.trace(b);throw new AnyBalance.Error("Ошибка при обработке запроса к API!");}return c}var g_currency={RUB:"р",USD:"$",EUR:"€",KZT:"〒",UAH:"₴"};
function parseCurrencyMy(a){a=parseCurrency(a);return g_currency[a]?" "+g_currency[a]:a}function processAccounts(a){var b=requestAPI({action:"person/state.action"},{},{Accept:"application/json, text/javascript","X-Requested-With":"XMLHttpRequest"});a.accounts=[];var c=0,d;for(d in g_currency)if(isset(b.data.balances[d])){var e=d,f={__id:e,__name:e};__shouldProcess("accounts",f)&&processAccount(b.data.balances,e,f);a.accounts.push(f);c++}AnyBalance.trace("Найдено счетов: "+c);return b}
