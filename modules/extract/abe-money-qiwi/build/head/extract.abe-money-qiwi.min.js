var g_headers={Accept:"application/vnd.qiwi.sso-v1+json",Origin:"https://qiwi.com","Accept-Language":"ru;q=0.8,en-US;q=0.6,en;q=0.4","User-Agent":"Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/43.0.2357.132 Safari/537.36"};
function login(a){AnyBalance.setDefaultCharset("utf-8");a=AnyBalance.getPreferences();checkEmpty(a.login,"Введите номер телефона!");checkEmpty(a.password,"Введите пароль!");AnyBalance.setOptions({SSL_ENABLED_PROTOCOLS:["TLSv1.2"]});AnyBalance.requestGet(baseurl+"payment/main.action",g_headers);var b=a.login;/^\d{10}$/i.test(a.login)?b="+7"+a.login:/^\s*\+/.test(a.login)||(b="+"+a.login);var c;try{c=requestAPI({action:"cas/tgts",isAuth:!0},{login:b,password:a.password})}catch(d){if(!d.fatal&&/робот|robot/i.test(d.message))AnyBalance.trace("Киви затребовало капчу..."),
c=requestAPI({action:"cas/tgts",isAuth:!0},{login:b,password:a.password,captcha:solveRecaptcha("Пожалуйста, докажите, что вы не робот.",baseurl,"6LfjX_4SAAAAAFfINkDklY_r2Q5BRiEqmLjs4UAC")});else throw d;}c=requestAPI({action:"cas/sts",isAuth:!0},{ticket:c.entity.ticket,service:baseurl+"j_spring_cas_security_check"});a=AnyBalance.requestPost(baseurl+"j_spring_cas_security_check",{ticket:c.entity.ticket},addHeaders({accept:"application/json","x-requested-with":"XMLHttpRequest",Referer:baseurl}));if("0"!=
getJson(a).code.value)throw AnyBalance.trace(a),new AnyBalance.Error("Не удалось войти в личный кабинет. Сайт изменен?");a=AnyBalance.requestGet(baseurl+"main",addHeaders({Referer:baseurl}));if(/Внимание! Срок действия вашего пароля истек/i.test(a))throw new AnyBalance.Error("Внимание! Срок действия вашего пароля истек. Зайдите в кошелек через браузер и следуйте инструкции.",null,!0);AnyBalance.trace("Успешно вошли...");return a}
...
function requestAPI(a,b,c){c=joinObjects({"Content-Type":"application/json"},c||{});b=AnyBalance.requestPost(a.isAuth?baseurlAuth+a.action:baseurl+a.action,JSON.stringify(b),addHeaders(g_headers,c));AnyBalance.trace("Request result: "+b);c=getJson(b);if(a.isAuth){if(!c.entity||!c.entity.ticket){if(c.entity&&(a=c.entity.error.message))throw new AnyBalance.Error(a,null,/Неверный логин или пароль|Неправильный номер телефона или пароль/i.test(a));AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось войти в Qiwi Visa Wallet, сайт изменен?");
...
function processAccounts(a){var b=requestAPI({action:"person/state.action"},{},{Accept:"application/json, text/javascript","X-Requested-With":"XMLHttpRequest"});if(!b.data.balances)throw new AnyBalance.Error("Киви требует сменить пароль. Пожалуйста, зайдите на https://qiwi.com через браузер, смените пароль и введите новый пароль в настройки провайдера.",null,!0);a.accounts=[];var c=0,d;for(d in g_currency)if(isset(b.data.balances[d])){var e=d,f={__id:e,__name:e};__shouldProcess("accounts",f)&&processAccount(b.data.balances,
e,f);a.accounts.push(f);c++}AnyBalance.trace("Найдено счетов: "+c);return b}function processAccount(a,b,c){AnyBalance.trace("Обработка счета "+b);getParam(a[b]+"",c,"accounts.balance",null,replaceTagsAndSpaces,parseBalance);getParam(g_currency[b]+"",c,["accounts.currency","accounts.balance"],null,replaceTagsAndSpaces);processAccountTransactions(b,c)}
...
"templates.prov_id");getParam(c.provider.name,a,"templates.prov_name");var d=sumParam(html,null,null,/<input[^>]*data-title[^>]*class=['"][^"']+dataField[^>]*>/ig);AnyBalance.trace("Найдено полей: "+d.length);a.fields=[];for(b=0;b<d.length;b++){var e=getParam(d[b],null,null,/data-form-field='([^']+)/i,replaceTagsAndSpaces,html_entity_decode),f=getParam(d[b],null,null,/data-title="([^'"]+)/i,replaceTagsAndSpaces),g=getParam(d[b],null,null,/name="([^"]+)/i,replaceTagsAndSpaces),k=getParam(d[b],null,
null,/value="([^"]+)/i,replaceTagsAndSpaces);a.fields.push({name:f,id:g,value:k,extra:e})}}catch(h){AnyBalance.trace(h.message+"\n"+h.stack)}}
function processCards(a,b){a=AnyBalance.requestGet(nodeUrl+"/PhizIC/private/cards/list.do");a=getElements(a,/<div[^>]+class="productCover[^"]*activeProduct[^>]*">/ig);AnyBalance.trace("Найдено карт: "+a.length);b.cards=[];for(var c=0;c<a.length;++c){var d=getParam(a[c],null,null,/<div[^>]+id="card_(\d+)/i),e=getParam(a[c],null,null,/<[^>]*class="accountNumber\b[^"]*">([^<]+)/i,replaceTagsAndSpaces,html_entity_decode),e={__id:d,__name:e};__shouldProcess("cards",e)&&processCard(a[c],d,e);b.cards.push(e)}}
...
replaceTagsAndSpaces);AnyBalance.trace("У карты "+a+" найдено транзакций: "+d.length);for(a=0;a<d.length;++a){var e={},f=-1*(getParam(d[a],null,null,/debit([^>]*>){3}/i,replaceTagsAndSpaces,parseBalance)||0),g=getParam(d[a],null,null,/credit([^>]*>){3}/i,replaceTagsAndSpaces,parseBalance)||0;getParam(f+g,e,"cards.transactions.sum");getParam(c,e,"cards.transactions.currency");getParam(d[a],e,"cards.transactions.time",/operationDate([^>]*>){2}/i,replaceTagsAndSpaces,parseDate);getParam(d[a],e,"cards.transactions.orderNum",
/documentNumber([^>]*>){2}/i,replaceTagsAndSpaces,html_entity_decode);getParam(d[a],e,"cards.transactions.correspond",/accNumber([^>]*>){2}/i,replaceTagsAndSpaces,html_entity_decode);getParam(d[a],e,"cards.transactions.name",/operationDate([^>]*>){8}/i,replaceTagsAndSpaces,html_entity_decode);b.transactions.push(e)}b.transactions=sortObject(b.transactions,"time")}else AnyBalance.trace(c),AnyBalance.trace("Не удалось найти таблицу операций!")}}
function processCardLast10Transactions(a,b){AnyBalance.trace("Получаем последние 10 операций по карте...");html=AnyBalance.requestGet(nodeUrl+"/PhizIC/private/cards/info.do?id="+a);if(/<table[^>]*class="tblInf"/i.test(html)){b.transactions10=[];var c=sumParam(html,null,null,/<tr[^>]*class="ListLine\d+">(?:[^>]*>){6}\s*<\/tr>/ig);AnyBalance.trace("У карты "+a+" найдено транзакций: "+c.length);for(a=0;a<c.length;++a){var d={};getParam(c[a],d,"cards.transactions10.sum",/([^>]*>){7}/i,replaceTagsAndSpaces,
parseBalance);getParam(c[a],d,"cards.transactions10.currency",/([^>]*>){7}/i,replaceTagsAndSpaces,parseCurrency);getParam(c[a],d,"cards.transactions10.name",/([^>]*>){3}/i,replaceTagsAndSpaces,html_entity_decode);getParam(c[a],d,"cards.transactions10.time",/([^>]*>){5}/i,replaceTagsAndSpaces,parseSmallDate);b.transactions10.push(d)}b.transactions10=sortObject(b.transactions10,"time")}else AnyBalance.trace(html),AnyBalance.trace("Не удалось найти таблицу операций!")}
