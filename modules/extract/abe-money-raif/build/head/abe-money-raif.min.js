...
if(!/loginResponse/i.test(c)){var f=getParam(c,null,null,/<faultstring>([\s\S]*?)<\/faultstring>/i,replaceTagsAndSpaces,html_entity_decode);if(f&&(f=e(f)))throw new AnyBalance.Error(f,null,/Неправильный логин или пароль|срок действия Вашего пароля истёк/i.test(f));AnyBalance.trace(c);throw new AnyBalance.Error("Не удалось зайти в интернет банк. Обратитесь к разработчикам.");}a.profile={};getParam(c,a.profile,"profile.fio",/<name>([\s\S]*?)<\/name>/i,replaceTagsAndSpaces,capitalFirstLetters);getParam(c,
a.profile,"profile.passportnumber",/<passportnumber>([\s\S]*?)<\/passportnumber>/i,replaceTagsAndSpaces,html_entity_decode);getParam(c,a.profile,"profile.passportissuer",/<passportissuer>([\s\S]*?)<\/passportissuer>/i,replaceTagsAndSpaces,html_entity_decode);getParam(c,a.profile,"profile.passportissuedate",/<passportissuedate>([\s\S]*?)<\/passportissuedate>/i,replaceTagsAndSpaces,parseDateISO);getParam(c,a.profile,"profile.login",/<login>([\s\S]*?)<\/login>/i,replaceTagsAndSpaces,html_entity_decode);
return c}
...
parseBalance);getParam(b,a,"cards.limit",/<creditLimit>([\s\S]*?)<\/creditLimit>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"cards.holdedFunds",/<holdedFunds>([\s\S]*?)<\/holdedFunds>/i,replaceTagsAndSpaces,parseBalance);var c=getParam(b,null,null,/<accountType>([\s\S]*?)<\/accountType>/i,replaceTagsAndSpaces,parseBalance);getParam(c,a,"cards.type_code");getParam(b,a,"cards.type",/<type>([\s\S]*?)<\/type>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"cards.cardnum",/<number>([\s\S]*?)<\/number>/i,
replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"cards.accnum",/<accountNumber>([\s\S]*?)<\/accountNumber>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"cards.minpaytill",/<nextCreditPaymentDate>([\s\S]*?)<\/nextCreditPaymentDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"cards.till",/<expirationDate>([\s\S]*?)<\/expirationDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"cards.state",/<state>([\s\S]*?)<\/state>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,
a,"cards.cardName",/<cardName>([\s\S]*?)<\/cardName>/i,replaceTagsAndSpaces,function(a){return html_entity_decode(a)+" ("+getParam(b,null,null,/<number>([\s\S]*?)<\/number>/i,[replaceTagsAndSpaces,/^[\s\S]*?(\d{4})$/,"$1"])+")"});getParam(b,a,"cards.holderName",/<holderName>([\s\S]*?)<\/holderName>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"cards.isCorporate",/<isCorporate>([\s\S]*?)<\/isCorporate>/i,replaceTagsAndSpaces,parseBoolean);getParam(b,a,"cards.isMain",/<main>([\s\S]*?)<\/main>/i,
replaceTagsAndSpaces,parseBoolean);getParam(b,a,"cards.openDate",/<openDate>([\s\S]*?)<\/openDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"cards.shortType",/<shortType>([\s\S]*?)<\/shortType>/i,replaceTagsAndSpaces,html_entity_decode);if("3"==c&&(isAvailable(["cards.totalCreditDebtAmount","cards.clearBalance","cards.ownFunds"])||e("cards.limit")||e("cards.minpay"))){var c=AnyBalance.requestPost(baseurl+"RCCardService",'<?xml version=\'1.0\' encoding=\'UTF-8\' standalone=\'yes\' ?><soapenv:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://entry.rconnect/xsd" xmlns:ser="http://service.rconnect" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:soapenc="http://schemas.xmlsoap.org/soap/encoding/"><soapenv:Header /><soapenv:Body><ser:getCreditStatementPeriods2><cardId>'+
...
c);a.accounts.push(c)}}}
function processAccount(b,a){getParam(b,a,"accounts.balance",/<balance>([\s\S]*?)<\/balance>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"accounts.heldFunds",/<heldFunds>([\s\S]*?)<\/heldFunds>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,["accounts.currency","accounts.minpay","accounts.limit","accounts.balance"],/<currency>([\s\S]*?)<\/currency>/i,replaceTagsAndSpaces,toUpperCaseMy);getParam(b,a,"accounts.accnum",/<number>([\s\S]*?)<\/number>/i,replaceTagsAndSpaces,html_entity_decode);
getParam(b,a,"accounts.minpaytill",/<nextCreditPaymentDate>([\s\S]*?)<\/nextCreditPaymentDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"accounts.minpay",/<minimalCreditPayment>([\s\S]*?)<\/minimalCreditPayment>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"accounts.limit",/<creditLimit>([\s\S]*?)<\/creditLimit>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"accounts.till",/<closeDate>([\s\S]*?)<\/closeDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"accounts.openDate",/<openDate>([\s\S]*?)<\/openDate>/i,
replaceTagsAndSpaces,parseDateISO);getParam(b,a,"accounts.accountSubtypeName",/<accountSubtypeName>([\s\S]*?)<\/accountSubtypeName>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"accounts.type",/<type>([\s\S]*?)<\/type>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"accounts.region",/<region>([\s\S]*?)<\/region>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"accounts.owner_id",/<owner_id>([\s\S]*?)<\/owner_id>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"accounts.branchId",
/<branchId>([\s\S]*?)<\/branchId>/i,replaceTagsAndSpaces,html_entity_decode);"undefined"!=typeof processAccountTransactions&&processAccountTransactions(b,a)}
...
c);a.deposits.push(c)}}}
function processDeposit(b,a){getParam(b,a,"deposits.accnum",/<accountNumber>([\s\S]*?)<\/accountNumber>/i,replaceTagsAndSpaces,html_entity_decode);getParam(b,a,"deposits.balance",/<initialAmount>([\s\S]*?)<\/initialAmount>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"deposits.currentAmount",/<currentAmount>([\s\S]*?)<\/currentAmount>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,["deposits.currency","deposits.balance","deposits.currentAmount"],/<currency>[^<]*?([^<\.]*?)<\/currency>/i,replaceTagsAndSpaces,
...
replaceTagsAndSpaces,parseDateISO);getParam(b,a,"deposits.capitalization",/<capitalization>([\s\S]*?)<\/capitalization>/i,replaceTagsAndSpaces,parseBoolean);getParam(b,a,"deposits.name",/<names>([\s\S]*?)<\/names>/i,replaceTagsAndSpaces,html_entity_decode)}
...
function getFormattedDate(b){var a=new Date,e=10>a.getDate()?"0"+a.getDate():a.getDate(),d=10>a.getMonth()+1?"0"+(a.getMonth()+1):a.getMonth()+1;return(isset(b)?a.getFullYear()-b:a.getFullYear())+"-"+d+"-"+e}function parseBoolean(b){return"true"===b}function toUpperCaseMy(b){return html_entity_decode(b+"").toUpperCase()};
