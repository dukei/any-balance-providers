...
if(!/loginResponse/i.test(c)){var f=getParam(c,null,null,/<faultstring>([\s\S]*?)<\/faultstring>/i,replaceTagsAndSpaces);if(f&&(f=e(f)))throw new AnyBalance.Error(f,null,/Неправильный логин или пароль|срок действия Вашего пароля истёк/i.test(f));AnyBalance.trace(c);throw new AnyBalance.Error("Не удалось зайти в интернет банк. Обратитесь к разработчикам.");}AnyBalance.isAvailable("info")&&(a.info={},getParam(c,a.info,"info.fio",/<name>([\s\S]*?)<\/name>/i,replaceTagsAndSpaces,capitalFirstLetters),
getParam(c,a.info,"info.passport",/<passportnumber>([\s\S]*?)<\/passportnumber>/i,replaceTagsAndSpaces),getParam(c,a.info,"info.passportissuer",/<passportissuer>([\s\S]*?)<\/passportissuer>/i,replaceTagsAndSpaces),getParam(c,a.info,"info.passportissuedate",/<passportissuedate>([\s\S]*?)<\/passportissuedate>/i,replaceTagsAndSpaces,parseDateISO),getParam(c,a.info,"info.login",/<login>([\s\S]*?)<\/login>/i,replaceTagsAndSpaces));return c}
...
function processCard(b,a){function e(b){return isAvailable(b)&&!isset(a[b])}var d=getParam(b,null,null,/<balance>([\s\S]*?)<\/balance>/i,replaceTagsAndSpaces,parseBalance);getParam(d,a,"cards.balance");getParam(b,a,"cards.currency cards.balance cards.minpay cards.limit cards.totalCreditDebtAmount cards.blocked".split(" "),/<currency>([\s\S]*?)<\/currency>/i,replaceTagsAndSpaces,toUpperCaseMy);getParam(b,a,"cards.minpay",/<minimalCreditPayment>([\s\S]*?)<\/minimalCreditPayment>/i,replaceTagsAndSpaces,
parseBalance);getParam(b,a,"cards.limit",/<creditLimit>([\s\S]*?)<\/creditLimit>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"cards.blocked",/<holdedFunds>([\s\S]*?)<\/holdedFunds>/i,replaceTagsAndSpaces,parseBalance);var c=getParam(b,null,null,/<accountType>([\s\S]*?)<\/accountType>/i,replaceTagsAndSpaces,parseBalance);getParam(c,a,"cards.type_code");getParam(b,a,"cards.type",/<type>([\s\S]*?)<\/type>/i,replaceTagsAndSpaces);getParam(b,a,"cards.num",/<number>([\s\S]*?)<\/number>/i,replaceTagsAndSpaces);
getParam(b,a,"cards.accnum",/<accountNumber>([\s\S]*?)<\/accountNumber>/i,replaceTagsAndSpaces);getParam(b,a,"cards.minpay_till",/<nextCreditPaymentDate>([\s\S]*?)<\/nextCreditPaymentDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"cards.till",/<expirationDate>([\s\S]*?)<\/expirationDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"cards.status",/<state>([\s\S]*?)<\/state>/i,replaceTagsAndSpaces);getParam(b,a,"cards.name",/<cardName>([\s\S]*?)<\/cardName>/i,replaceTagsAndSpaces,function(a){return a+
" ("+getParam(b,null,null,/<number>([\s\S]*?)<\/number>/i,[replaceTagsAndSpaces,/^[\s\S]*?(\d{4})$/,"$1"])+")"});getParam(b,a,"cards.holder",/<holderName>([\s\S]*?)<\/holderName>/i,replaceTagsAndSpaces);getParam(b,a,"cards.isCorporate",/<isCorporate>([\s\S]*?)<\/isCorporate>/i,replaceTagsAndSpaces,parseBoolean);getParam(b,a,"cards.isMain",/<main>([\s\S]*?)<\/main>/i,replaceTagsAndSpaces,parseBoolean);getParam(b,a,"cards.dateStart",/<openDate>([\s\S]*?)<\/openDate>/i,replaceTagsAndSpaces,parseDateISO);
getParam(b,a,"cards.shortType",/<shortType>([\s\S]*?)<\/shortType>/i,replaceTagsAndSpaces);if("3"==c&&(isAvailable(["cards.totalCreditDebtAmount","cards.clearBalance","cards.own"])||e("cards.limit")||e("cards.minpay"))){var c=AnyBalance.requestPost(baseurl+"RCCardService",'<?xml version=\'1.0\' encoding=\'UTF-8\' standalone=\'yes\' ?><soapenv:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://entry.rconnect/xsd" xmlns:ser="http://service.rconnect" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:soapenc="http://schemas.xmlsoap.org/soap/encoding/"><soapenv:Header /><soapenv:Body><ser:getCreditStatementPeriods2><cardId>'+
...
a.__id+"</cardId><id>"+f+"</id><isPrime>"+g+"</isPrime></ser:getCurrentCreditStatement></soapenv:Body></soapenv:Envelope>",addHeaders({SOAPAction:""})),f=getParam(c,null,null,/<availableCreditLimit>([\s\S]*?)<\/availableCreditLimit>/i,replaceTagsAndSpaces,parseBalance),g=getParam(c,null,null,/<ownFunds>([\s\S]*?)<\/ownFunds>/i,replaceTagsAndSpaces,parseBalance),getParam(f,a,"cards.limit"),getParam(g,a,"cards.own"),getParam(d-f,a,"cards.clearBalance"),getParam(c,a,"cards.totalCreditDebtAmount",/<totalDebtAmount>([\s\S]*?)<\/totalDebtAmount>/i,
replaceTagsAndSpaces,parseBalance),getParam(c,a,"cards.minpay",/<minAmount>([\s\S]*?)<\/minAmount>/i,replaceTagsAndSpaces,parseBalance),getParam(c,a,"cards.gracePeriodOutstanding",/<gracePeriodOutstanding>([\s\S]*?)<\/gracePeriodOutstanding>/i,replaceTagsAndSpaces,parseBalance),getParam(c,a,"cards.gracepay",/<unpaidGracePeriodDue>([\s\S]*?)<\/unpaidGracePeriodDue>/i,replaceTagsAndSpaces,parseBalance),getParam(c,a,"cards.gracepay_till",/<gracePeriodEnd>([\s\S]*?)<\/gracePeriodEnd>/i,replaceTagsAndSpaces,
parseDateISO)):(AnyBalance.trace("Не удалось найти доп информацию по карте "+a.__name),AnyBalance.trace(c))}"undefined"!=typeof processCardTransactions&&processCardTransactions(b,a)}
...
c);a.accounts.push(c)}}}
function processAccount(b,a){getParam(b,a,"accounts.balance",/<balance>([\s\S]*?)<\/balance>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"accounts.blocked",/<heldFunds>([\s\S]*?)<\/heldFunds>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,["accounts.currency","accounts.minpay","accounts.limit","accounts.balance"],/<currency>([\s\S]*?)<\/currency>/i,replaceTagsAndSpaces,toUpperCaseMy);getParam(b,a,"accounts.num",/<number>([\s\S]*?)<\/number>/i,replaceTagsAndSpaces);getParam(b,a,"accounts.minpay_till",
/<nextCreditPaymentDate>([\s\S]*?)<\/nextCreditPaymentDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"accounts.minpay",/<minimalCreditPayment>([\s\S]*?)<\/minimalCreditPayment>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"accounts.limit",/<creditLimit>([\s\S]*?)<\/creditLimit>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"accounts.till",/<closeDate>([\s\S]*?)<\/closeDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"accounts.date_start",/<openDate>([\s\S]*?)<\/openDate>/i,
...
c);a.deposits.push(c)}}}
function processDeposit(b,a){getParam(b,a,"deposits.num",/<accountNumber>([\s\S]*?)<\/accountNumber>/i,replaceTagsAndSpaces);getParam(b,a,"deposits.balance",/<initialAmount>([\s\S]*?)<\/initialAmount>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"deposits.currentAmount",/<currentAmount>([\s\S]*?)<\/currentAmount>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,["deposits.currency","deposits.balance","deposits.currentAmount"],/<currency>[^<]*?([^<\.]*?)<\/currency>/i,replaceTagsAndSpaces,toUpperCaseMy);
getParam(b,a,"deposits.pct",/<interestRate>([\s\S]*?)<\/interestRate>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"deposits.period",/<daysQuantity>([\s\S]*?)<\/daysQuantity>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"deposits.pcts",/<totalInterest>([\s\S]*?)<\/totalInterest>/i,replaceTagsAndSpaces,parseBalance);getParam(b,a,"deposits.till",/<closeDate>([\s\S]*?)<\/closeDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"deposits.date_start",/<openDate>([\s\S]*?)<\/openDate>/i,replaceTagsAndSpaces,
parseDateISO);getParam(b,a,"deposits.capitalization",/<capitalization>([\s\S]*?)<\/capitalization>/i,replaceTagsAndSpaces,parseBoolean);getParam(b,a,"deposits.name",/<names>([\s\S]*?)<\/names>/i,replaceTagsAndSpaces)}
function processLoans(b,a){if(isAvailable("credits")){b=AnyBalance.requestPost(baseurl+"RCLoanService",g_xml_loans,addHeaders({SOAPAction:""}));var e=getElements(b,/<return>/ig);AnyBalance.trace("Найдено кредитов: "+e.length);a.credits=[];for(var d=0;d<e.length;++d){var c=getParam(e[d],null,null,/<id>([\s\S]*?)<\/id>/i,replaceTagsAndSpaces),c={__id:c,__name:"Кредит "+c,num:c};__shouldProcess("credits",c)&&processLoan(e[d],c);a.credits.push(c)}}}
...
/<closeDate>([\s\S]*?)<\/closeDate>/i,replaceTagsAndSpaces,parseDateISO);getParam(b,a,"credits.date_start",/<openDate>([\s\S]*?)<\/openDate>/i,replaceTagsAndSpaces,parseDateISO);"undefined"!=typeof processLoanTransactions&&processLoanTransactions(b,a)}function sortByKey(b,a){return b.sort(function(b,d){var c=b[a],f=d[a];return c>f?-1:c<f?1:0})}
