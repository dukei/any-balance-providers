...
break}catch(g){if(/SSLHandshakeException/i.test(g))if(10>++f)AnyBalance.trace("Райффайзен дурит, выдаёт ошибку SSL. Пробуем ещё раз (попытка "+(f+1)+")");else throw AnyBalance.trace("За "+f+" попыток не удалось соединиться..."),g;}while(1);if(!/loginResponse/i.test(d)){if(a=getParam(d,null,null,/<faultstring>([\s\S]*?)<\/faultstring>/i,replaceTagsAndSpaces))if(a=e(a))throw new AnyBalance.Error(a,null,/Неправильный логин или пароль|срок действия Вашего пароля истёк/i.test(a));AnyBalance.trace(d);throw new AnyBalance.Error("Не удалось зайти в интернет банк. Обратитесь к разработчикам.");
}AnyBalance.isAvailable("info")&&(b.info={},getParam(d,b.info,"info.fio",/<name>([\s\S]*?)<\/name>/i,replaceTagsAndSpaces,capitalFirstLetters),getParam(d,b.info,"info.passport",/<passportnumber>([\s\S]*?)<\/passportnumber>/i,replaceTagsAndSpaces),getParam(d,b.info,"info.passportissuer",/<passportissuer>([\s\S]*?)<\/passportissuer>/i,replaceTagsAndSpaces),getParam(d,b.info,"info.passportissuedate",/<passportissuedate>([\s\S]*?)<\/passportissuedate>/i,replaceTagsAndSpaces,parseDateISO),getParam(d,b.info,
"info.login",/<login>([\s\S]*?)<\/login>/i,replaceTagsAndSpaces));return d}
