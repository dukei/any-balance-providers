...
function processCard(b,a){getParam(b.balance,a,"cards.balance");getParam(b.currencyId,a,"cards.currency cards.balance cards.minpay cards.limit cards.totalCreditDebtAmount cards.blocked".split(" "));getParam(b.hold,a,"cards.blocked");getParam(b.type.id,a,"cards.type_code");getParam(b.type.name,a,"cards.type");getParam(b.pan,a,"cards.num");getParam(b.account.cba,a,"cards.accnum");getParam(b.expire,a,"cards.till",null,replaceTagsAndSpaces,parseDateISO);getParam(b.status.name,a,"cards.status");getParam(b.product,
a,"cards.name");getParam(b.cardholder,a,"cards.holder");getParam(1==b.main.id,a,"cards.isMain");getParam(b.open,a,"cards.dateStart",null,null,parseDateISO);getParam(b.paymentSystem.name,a,"cards.shortType");getParam(b.rate,a,"cards.rate");if("2"==b.type.id&&(isAvailable(["cards.totalCreditDebtAmount","cards.clearBalance","cards.own"])||isAvailableButUnset("cards.limit")||isAvailableButUnset("cards.minpay"))){var d=callApi("rest/account/"+b.accountId+"/statement/current?alien=false"),c=getParam(d.creditLimit,
null,null,null,replaceTagsAndSpaces,parseBalance),e=getParam(d.ownFunds,null,null,null,replaceTagsAndSpaces,parseBalance);getParam(c,a,"cards.limit");getParam(e,a,"cards.own");getParam(d.minAmount,a,"cards.minpay",null,replaceTagsAndSpaces,parseBalance);getParam(b.balance-c,a,"cards.clearBalance");getParam(d.totalDebt,a,"cards.totalCreditDebtAmount",null,replaceTagsAndSpaces,parseBalance);getParam(d.unpaidGracePeriodDue,a,"cards.gracepay",null,replaceTagsAndSpaces,parseBalance);getParam(d.longGraceDueDate,
a,"cards.gracepay_till",null,replaceTagsAndSpaces,parseDateISO)}"undefined"!=typeof processCardTransactions&&processCardTransactions(b,a)}function processAccounts(b){if(isAvailable("accounts")){var a=callApi("rest/account?alien=false");AnyBalance.trace("Найдено счетов: "+a.length);b.accounts=[];for(var d=0;d<a.length;++d){var c=a[d],c={__id:c.id,__name:c.type.name+" "+c.currencyId+" "+c.cba};__shouldProcess("accounts",c)&&processAccount(a[d],c);b.accounts.push(c)}}}
...
a,"deposits.date_start",null,null,parseDateISO);getParam(b.product.name.name,a,"deposits.name")}function processLoans(b){if(isAvailable("credits")){var a=callApi("rest/loan");AnyBalance.trace("Найдено кредитов: "+a.length);b.credits=[];for(var d=0;d<a.length;++d){var c=a[d].id,c={__id:c,__name:"Кредит "+c};__shouldProcess("credits",c)&&processLoan(a[d],c);b.credits.push(c)}}}
...
parseBalance);getParam(b.currencyId,a,"credits.currency credits.limit credits.balance credits.paid credits.paidLoanIntrest credits.minpay".split(" "));getParam(b.next,a,"credits.minpay_till","",replaceTagsAndSpaces,parseDateISO);getParam(b.close,a,"credits.till","",replaceTagsAndSpaces,parseDateISO);getParam(b.open,a,"credits.date_start","",replaceTagsAndSpaces,parseDateISO)}function isAvailableButUnset(b){return isAvailable(b)&&!isset(result[b])?!0:!1};
