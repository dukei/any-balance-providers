...
function findExactWicketAction(a,b,c){if(a){for(var d=[],e=0;e<a.length;e++){var f=getJsonEval(a[e].replace(/\^/g,""),"");f.c===b&&d.push(f)}for(var g,e=0;e<d.length;++e)if(f=d[e],!g||c&&f.e==c)g=(f.u||"").replace(/^.\/main/,"main").replace(/;jsessionid[^?]+/i,"");return g}}
...
addHeaders(g_Xml_Headers))}function checkForRedirect(a){/<redirect>/i.test(a)&&((a=getParam(a,null,null,/main;[^\]]+/i))||AnyBalance.trace("Запрошен редиретк, но ссылка на него не найдена, сайт изменен?"),a=AnyBalance.requestGet(g_baseurl+a,g_headers));return a}
function login(){var a=AnyBalance.getPreferences();AnyBalance.setDefaultCharset("utf-8");var b=AnyBalance.requestGet(g_baseurl,g_headers);if(/actionLogout/i.test(b))AnyBalance.trace("Найдена активная сессия. Используем её.");else{var c=getElement(b,/<form[^>]+operation/i);if(!c)throw new AnyBalance.Error("Не удалось найти форму входа. Сайт изменен?");c=AB.createFormParams(c,function(b,c,d,e){if(!/actionComponent/i.test(d))return/login/i.test(d)?a.login:/password/i.test(d)?a.password:e});c["actions:list:2:actionComponent"]=
"1";var d=findWicketActions(b),e=AnyBalance.getLastUrl(),f=getParam(b,null,null,/Wicket.Ajax.baseUrl="([^"]*)/i)||"home?0",b=requestWicketAction(d[4],{"fields:field:1:login":a.login,"":""},e,f),b=requestWicketAction(d[5],{"fields:field:2:password":a.password,"":""},e,f),b=requestWicketAction(d[2],c,e,f);if(/AcceptTerms/i.test(b))throw new AnyBalance.Error("Вы ещё ни разу не входили в интернет-банк или вам требуется сменить пароль. Зайдите в интернет банк через браузер на https://online.rosbank.ru/ibank/, затем попробуйте выполнить провайдер ещё раз");
...
AnyBalance.trace("Нашли "+d.length+" счетов"),b=0;b<d.length;++b){var c=d[b],e=getElement(c,/<[^>]*class="number"/i,replaceTagsAndSpaces),f=getElement(c,/<[^>]*class="name/i,replaceTagsAndSpaces),e={__id:e,__name:f+" "+e.substr(-4),num:e};__shouldProcess("accounts",e)&&processAccount(c,e);a.accounts.push(e)}else AnyBalance.trace(b),AnyBalance.trace("Не удалось получить таблицу счетов")}}
