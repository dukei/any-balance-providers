var g_headers={"User-Agent":"OnePlus ONEPLUS A3010/android: 9/TCSMB/5.2.1"},g_baseurl="https://api.tinkoff.ru/v1/",g_sessionid,g_postParams={mobile_device_model:"ONEPLUS A3010",mobile_device_os:"android",appVersion:"5.2.1",screen_width:"1080",root_flag:"false",appName:"mobile",origin:"mobile,ib5,loyalty,platform",connectionType:"WiFi",platform:"android",screen_dpi:"420",mobile_device_os_version:"9",screen_height:"1920",fingerprint:"OnePlus ONEPLUS A3010/android: 9/TCSMB/5.2.1###1080x1920x32###180###false###false###"};
...
new AnyBalance.Error("Не удалось войти в кабинет с нужным уровнем доступа");var b=createPass(64),c=getFormattedDate({format:"YYYY-MM-DD HH:NN:SS"}),a=requestJson("auth/pin/set",{},{post:joinObjects({pinHash:b,auth_type_set_date:c,deviceId:getDeviceId()},g_postParams)}),a=a.payload.key,d=AnyBalance.getData(getDataName());AnyBalance.setData(getDataName(),joinObjects({key:a,pinHash:b,auth_type_set_date:c,oldSessionId:g_sessionid},d));AnyBalance.saveData()}}else if("OK"==a.resultCode)g_sessionid=a.payload.sessionid;
...
function login(){var b=AnyBalance.getPreferences();checkEmpty(b.login,"Введите ваш номер телефона!");checkEmpty(/^\d{10}$/.test(b.login),"Введите ваш номер телефона, 10 цифр без пробелов и разделителей, например, 9261234567 !");checkEmpty(b.password,"Введите пароль в интернет-банк!");g_sessionid?AnyBalance.trace("сессия уже установлена. Используем её."):(g_sessionid=requestJson("auth/session",{},{post:joinObjects({phone:b.login,deviceId:getDeviceId()},g_postParams)}).payload.sessionid,requestJson("warmup_cache",
{},{post:joinObjects({phone:"+7"+b.login,deviceId:getDeviceId()},g_postParams)}),loginByPin()||loginByPhone(),__setLoginSuccessful());return g_sessionid}
...
function processCard(b,a,c){AnyBalance.trace("Обработка карты "+c.__name);var d=jspath1(b,"$.availableBalance")||jspath1(a,"$.moneyAmount")||jspath1(a,"$.accountBalance");getParam(jspath1(a,"$.accountBalance.value"),c,"cards.balance");getParam(jspath1(d,"$.value"),c,"cards.available");getParam(jspath1(d,"$.currency.name"),c,"cards.currency");getParam(jspath1(b,"$.expiration.milliseconds"),c,"cards.till");getParam(jspath1(b,"$.activated"),c,"cards.active");getParam(jspath1(b,"$.primary"),c,"cards.primary");
getParam(jspath1(b,"$.reissued"),c,"cards.reissued");getParam(jspath1(b,"$.status"),c,"cards.status");getParam(jspath1(b,"$.statusCode"),c,"cards.status_code");getParam(jspath1(b,"$.name"),c,"cards.name");getParam(jspath1(a,"$.externalAccountNumber"),c,"cards.accnum");AnyBalance.isAvailable("cards.transactions")&&processCardsTransactions(c)}
...
function processAccount(b,a){AnyBalance.trace("Обработка счета "+a.__name);var c=jspath1(b,"$.moneyAmount")||jspath1(b,"$.accountBalance");getParam(jspath1(b,"$.accountBalance.value"),a,"accounts.balance");getParam(jspath1(b,"$.moneyAmount.value"),a,"accounts.available");getParam(jspath1(c,"$.currency.name"),a,"accounts.currency");getParam(jspath1(b,"$.creationDate.milliseconds"),a,"accounts.date_start");getParam(jspath1(b,"$.accountGroup"),a,"accounts.type");getParam(jspath1(b,"$.accountType"),a,
"accounts.type_code");getParam(jspath1(b,"$.authorizationsAmount.value"),a,"accounts.blocked");getParam(jspath1(b,"$.defaultMonthlyCashLimit.value"),a,"accounts.free_cash_limit");getParam(jspath1(b,"$.defaultRenewalAmountLeft.value"),a,"accounts.free_add_limit");getParam(jspath1(b,"$.monthlyCashLimit.value"),a,"accounts.free_cash_left");getParam(jspath1(b,"$.renewalAmountLeft.value"),a,"accounts.free_add_left");getParam(jspath1(b,"$.rate"),a,"accounts.pct");getParam(jspath1(b,"$.status"),a,"accounts.status_code");
getParam(jspath1(b,"$.creditLimit.value"),a,"accounts.limit");getParam(jspath1(b,"$.debtAmount.value"),a,"accounts.debt");getParam(jspath1(b,"$.currentRecommendedPayment.value"),a,"accounts.recompay");getParam(jspath1(b,"$.currentMinimalPayment.value"),a,"accounts.minpay");getParam(jspath1(b,"$.duedate.milliseconds"),a,"accounts.minpay_till");getParam(jspath1(b,"$.marketingName"),a,"accounts.name");getParam(jspath1(b,"$.loyalty[0].amount"),a,"accounts.cashback");AnyBalance.isAvailable("accounts.transactions")&&
processAccountsTransactions(a)}
...
getParam(jspath1(b,"$.startAmount"),a,"deposits.balance_start");getParam(jspath1(b,"$.period"),a,"deposits.period");getParam(jspath1(b,"$.status"),a,"deposits.status_code");AnyBalance.isAvailable("deposits.accnum")&&(b=findAccountById(jspath1(b,"$.currentLinkedAccount")))&&getParam(jspath1(b,"$.externalAccountNumber"),a,"deposits.accnum");AnyBalance.isAvailable("deposits.transactions")&&processDepositsTransactions(a)}
function processSaving(b,a){AnyBalance.trace("Обработка накопительного счета "+a.__name);getParam(jspath1(b,"$.accountType"),a,"savings.type_code");getParam(jspath1(b,"$.marketingName"),a,"savings.name");getParam(jspath1(b,"$.creationDate.milliseconds"),a,"savings.date_start");getParam(jspath1(b,"$.interest.value"),a,"savings.pct_sum");getParam(jspath1(b,"$.tariffInfo.interestRate")||jspath1(b,"$.rate"),a,"savings.pct");getParam(jspath1(b,"$.moneyAmount.value"),a,"savings.balance");getParam(jspath1(b,
"$.moneyAmount.currency.name"),a,"savings.currency");getParam(jspath1(b,"$.status"),a,"savings.status_code");AnyBalance.isAvailable("savings.transactions")&&processSavingsTransactions(a)}function findAccountById(b){for(var a=requestAccountsJson(),c=0;c<a.payload.length;c++){var d=a.payload[c];if(d.id==b)return d}}
function findCardById(b){for(var a=requestAccountsJson(),c=0;c<a.payload.length;c++)for(var d=a.payload[c],e=0;d.cardNumbers&&e<d.cardNumbers.length;e++){var f=d.cardNumbers[e];if(f.id==b)return f}}
