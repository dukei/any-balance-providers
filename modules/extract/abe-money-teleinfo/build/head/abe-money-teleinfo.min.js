...
function getCardsAndAccounts(){function a(d){for(var g=0;d&&g<d.length;g++){var f=d[g];c[f.id]||(b.payload.products.push(f),c[f.id]=f,a(f.masterAccountCards),a(f.cards),a(f.cardAccount&&[f.cardAccount]))}}if(g_cardsAndAccounts)return g_cardsAndAccounts;var b=request(new Message({__type:"ru.vtb24.mobilebanking.protocol.product.PortfolioRequest",refreshImmediately:!0,portfolioType:{__type:"ru.vtb24.mobilebanking.protocol.product.PortfolioTypeMto",id:"ACCOUNTS_AND_CARDS"}},null,g_commonProperties));
b.payload.products=[];for(var c={},d=0;d<b.payload.portfolio.productGroups.length;++d)a(b.payload.portfolio.productGroups[d].items);AnyBalance.trace("Найдено "+b.payload.products.length+" карт и счетов");return g_cardsAndAccounts=b}
