...
function processDeposit(a,b){getParam(a.account.amount.sum,b,"deposits.balance");getParam(a.account.amount.currency.currencyCode,b,["deposits.currency","deposits.balance"]);getParam(a.endDate.getTime(),b,"deposits.till");if(AnyBalance.isAvailable("deposits.pct","deposits.own")){var c=request(new Message({__type:"ru.vtb24.mobilebanking.protocol.ObjectRequest",identity:{__type:"ru.vtb24.mobilebanking.protocol.ObjectIdentityMto",id:a.id,type:a.__type}},null,g_commonProperties)).payload.info;getParam(c.interestRate,
b,"deposits.pct");getParam(c.depositSum,b,"deposits.own")}}
