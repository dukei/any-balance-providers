...
function login(prefs) {
    ...
    if(!/overview-menu/i.test(html)){
        //Не зашли
		var error = getParam(html, null, null, /<div[^>]+id="alert-error"[^>]*>([\s\S]*?)(?:<\/div>|<script)/i, replaceTagsAndSpaces);
		if (error)
...
function processAccounts(html, result) {
	...
	for(var i=0; i < accounts.length; ++i){
        var acc = accounts[i];
        var id = getParam(acc, null, null, /accountId=([^"&]*)/i, replaceHtmlEntities);
		var name = getElement(acc, /<h3[^>]*>/i, replaceTagsAndSpaces);
...
function processCards(html, result) {
	...
	for(var i=0; i < cards.length; ++i){
        ...
        var name = getElement(card, /<h3[^>]*>/i, replaceTagsAndSpaces);
        var num = getParam(name, null, null, /\*?\d+$/, replaceTagsAndSpaces);
...
function processCard(card, result) {
    getParam(card, result, 'cards.accnum', /<div[^>]+list-item-content[^>]*>\s*((?:[\d\s]|&nbsp;?){20,})/i, replaceTagsAndSpaces);
	...
	if(AnyBalance.isAvailable('cards.type', 'cards.num', 'cards.till', 'cards.status')) {
		...
		getParam(html, result, 'cards.type', /Тип([\s\S]*?)<\/li>/i, replaceTagsAndSpaces);
		getParam(html, result, 'cards.num', /Номер([\s\S]*?)<\/li>/i, replaceTagsAndSpaces);
        getParam(html, result, 'cards.till', /Номер([\s\S]*?)<\/li>/i, replaceTagsAndSpaces, parseDate);
		getParam(html, result, 'cards.status', /<li[^>]+id="card-state"[^>]*>([\s\S]*?)<\/li>/i, replaceTagsAndSpaces); //Действует
...
function processInfo(html, result){
    if(AnyBalance.isAvailable('info')) {
        ...
        getParam(html, info, 'fam', /<input[^>]+application.lastName[^>]*value="([^"]*)/i, replaceHtmlEntities);
        getParam(html, info, 'name', /<input[^>]+application.firstName[^>]*value="([^"]*)/i, replaceHtmlEntities);
        getParam(html, info, 'patronym', /<input[^>]+application.patronym[^>]*value="([^"]*)/i, replaceHtmlEntities);
        getParam(html, info, 'birthday', /<input[^>]+application.birthDate[^>]*value="([^"]*)/i, null, parseDate);
        getParam(html, info, 'phone', /<input[^>]+application.mobilePhone[^>]*value="([^"]*)/i, replaceHtmlEntities);
        getParam(html, info, 'email', /<input[^>]+application.email[^>]*value="([^"]*)/i, replaceHtmlEntities);
    }
}
