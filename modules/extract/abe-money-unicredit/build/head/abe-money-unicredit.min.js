...
function processDeposits(a,b){var c=AnyBalance.requestPost(baseurl,{SID:a.SID,tic:1,T:"RT_2IC.form",nvgt:1,SCHEMENAME:"DEPOSITSLIST",XACTION:""},g_headers);if(getParam(c,null,null,/<REDIRECT>[\s\S]*?'(NO)DEPOSITS'/i))throw new AnyBalance.Error("У вас нет ни одного вклада");var d=getElements(c,/<div[^>]+div-corner div-cards[^>]*>/ig);if(!d.length){var g=getParam(c,null,null,/<REDIRECT>[\s\S]*?DepID=([0-9A-Z]+)/i);if(!g){AnyBalance.trace(c);AnyBalance.trace("Не удалось найти депозиты!");return}d[0]=
AnyBalance.requestPost(baseurl,{SID:a.SID,tic:1,T:"RT_2IC.form",nvgt:1,SCHEMENAME:"DEPOSIT",XACTION:"",DepID:g},g_headers)}AnyBalance.trace("Найдено депозитов: "+d.length);b.deposits=[];for(c=0;c<d.length;c++){var e=getParam(d[c],null,null,/Номер сделки[\s\S]*?<span[^>]+fat[^>]*>([^<]*)/i),f=getParam(d[c],null,null,/Номер сделки[\s\S]*?<span[^>]+fat[^>]*>([^<]*)/i,replaceTagsAndSpaces),h=getParam(d[c],null,null,/<span[^>]+span-actual(?:[^>]*>){4}([\s\S]*?)<\/div>/i,replaceTagsAndSpaces),e={__id:e,
__name:h,num:f};__shouldProcess("deposits",e)&&processDeposit(d[c],e,a,g);b.deposits.push(e)}}
function processDeposit(a,b,c,d){AnyBalance.trace("Обработка депозита "+b.__name);if(!d){d=getParam(a,null,null,/AccID=(\d+)/i);if(!d){AnyBalance.trace(a);AnyBalance.trace("Не удалось найти ID депозита для запроса!");return}account=AnyBalance.requestPost(baseurl,{SID:c.SID,tic:1,T:"RT_2IC.form",nvgt:1,SCHEMENAME:"DEPOSIT",XACTION:"",DepID:d},g_headers)}getParam(a,b,"deposits.balance",/<span[^>]+veryBig fat[^>]*>([\s\S]*?)<\/span>/i,replaceTagsAndSpaces,parseBalance);getParam(a,b,"deposits.pct",/<span[^>]+fat toLowerCase[^>]*>([^%]*)/i,
