var g_headers={Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8","Accept-Charset":"windows-1251,utf-8;q=0.7,*;q=0.3","Accept-Language":"ru-RU,ru;q=0.8,en-US;q=0.6,en;q=0.4",Connection:"keep-alive","User-Agent":"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.181 Safari/537.36"},baseurl;function fnRnd(){var a=new Date;return"B"+(Date.parse(a.toGMTString())+a.getMilliseconds()).toString(32)}
...
new AnyBalance.Error("Ошибка при подключении к сайту интернет-банка! Попробуйте обновить данные позже.");if(/exit/i.test(b))AnyBalance.trace("Уже залогинены, используем существующую сессию");else var b=AnyBalance.requestPost(baseurl,{tic:0,T:"RT_2Auth.getLoginMap"},addHeaders({BSSHTTPRequest:1,Referer:baseurl+"T=RT_2Auth.BF"})),c=getElement(b,/<i[^>]*N="MapID"/i,replaceTagsAndSpaces),d=getElement(b,/<i[^>]*N="Map"/i,replaceTagsAndSpaces),d=encryptPass(a.password,d),b=AnyBalance.requestPost(baseurl,
{tic:0,T:"RT_2Auth.CL",A:a.login,B:d,L:"RUSSIAN",IdCaptcha:"",C:"",MapID:c||"",BROWSER:"Chrome",BROWSERVER:"66.0.3359.181"},addHeaders({Referer:baseurl+"T=RT_2Auth.BF"}));if(!/ClientInfo/i.test(b)){var f=getElement(b,/<i[^>]+n="idCaptcha"/i,replaceTagsAndSpaces);f&&(AnyBalance.trace("Требуется ввод капчи"),b=AnyBalance.requestGet(baseurl+"T=laCaptcha.getCaptchaImage&ID="+encodeURIComponent(f)+"&tms="+fnRnd(),{Referer:baseurl+"T=RT_2Auth.BF"}),b=AnyBalance.retrieveCode("Пожалуйста, введите код с картинки",
b,{time:12E4}),b=AnyBalance.requestPost(baseurl,{tic:0,T:"RT_2Auth.CL",A:a.login,B:d,L:"RUSSIAN",IdCaptcha:f,C:b,MapID:c||"",BROWSER:"Chrome",BROWSERVER:"66.0.3359.181"},addHeaders({Referer:baseurl+"T=RT_2Auth.BF"})))}if(!/ClientInfo/i.test(b)){if(a=getParam(b,null,null,/<BSS_ERROR>\d*\|?([\s\S]*?)<\/BSS_ERROR>/i,[replaceTagsAndSpaces,/\[br\]/ig,""]))throw new AnyBalance.Error(a,null,/Ошибка при вводе логина или пароля/i.test(a));AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в интернет-банк. Сайт изменен?");
}a=getParam(b,null,null,/ClientInfo=(\{[\s\S]*?\})\s*(?:<\/div>|$)/i);if(!a)throw new AnyBalance.Error("Не удалось найти информацию о сессии в ответе банка.");a=JSON.parse(a);__setLoginSuccessful();return a}
