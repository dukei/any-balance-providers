...
new AnyBalance.Error("Ошибка при подключении к сайту интернет-банка! Попробуйте обновить данные позже.");if(/exit/i.test(b))AnyBalance.trace("Уже залогинены, используем существующую сессию");else var b=AnyBalance.requestPost(baseurl,{tic:0,T:"RT_2Auth.getLoginMap"},addHeaders({BSSHTTPRequest:1,Referer:baseurl+"T=RT_2Auth.BF"})),c=getElement(b,/<i[^>]*N="MapID"/i,replaceTagsAndSpaces),b=getElement(b,/<i[^>]*N="Map"/i,replaceTagsAndSpaces),b=encryptPass(a.password,b),b=AnyBalance.requestPost(baseurl,
{tic:0,T:"RT_2Auth.CL",A:a.login,B:b,L:"RUSSIAN",IdCaptcha:"",C:"",MapID:c||"",BROWSER:"Chrome",BROWSERVER:"66.0.3359.181"},addHeaders({Referer:baseurl+"T=RT_2Auth.BF"}));if(!/ClientInfo/i.test(b)){if(a=getParam(b,null,null,/<BSS_ERROR>\d*\|?([\s\S]*?)<\/BSS_ERROR>/i,[replaceTagsAndSpaces,/\[br\]/ig,""]))throw new AnyBalance.Error(a,null,/Ошибка при вводе логина или пароля/i.test(a));AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в интернет-банк. Сайт изменен?");}a=getParam(b,null,
null,/ClientInfo=(\{[\s\S]*?\})\s*(?:<\/div>|$)/i);if(!a)throw new AnyBalance.Error("Не удалось найти информацию о сессии в ответе банка.");a=JSON.parse(a);__setLoginSuccessful();return a}
function processAccounts(a,b){if(AnyBalance.isAvailable("accounts")){var c=AnyBalance.requestPost(baseurl,{SID:a.SID,tic:1,T:"RT_2IC.form",nvgt:1,SCHEMENAME:"ACCOUNTSLIST",XACTION:""},g_headers);if(getParam(c,null,null,/<REDIRECT>[\s\S]*?'(NO)ACCOUNTS'/i))AnyBalance.trace("У Вас нет ни одного счёта!");else{var d=getElements(c,/<div[^>]+div-corner div-cards[^>]*>/ig);if(!d.length){var g=getParam(c,null,null,/<REDIRECT>[\s\S]*?AccID=(\d+)/i);if(!g){AnyBalance.trace(c);AnyBalance.trace("Не удалось найти cчета!");
return}d[0]=AnyBalance.requestPost(baseurl,{SID:a.SID,tic:1,T:"RT_2IC.form",nvgt:1,SCHEMENAME:"ACCOUNT",XACTION:"",AccID:g},g_headers)}AnyBalance.trace("Найдено счетов: "+d.length);b.accounts=[];for(c=0;c<d.length;c++){var e=d[c],f=getParam(e,null,null,[/Номер счета[\s\S]*?<span[^>]+fat[^>]*>([^<]*)/i,/Номер счета:\s*([^<]*)/i]),h=getParam(e,null,null,[/Номер счета[\s\S]*?<span[^>]+fat[^>]*>([^<]*)/i,/Номер счета:\s*([^<]*)/i]),k=getParam(e,null,null,[/<div[^>]+formheader(?:[^>]*>){8}([\s\S]*?)<\//i,
/<span[^>]+div-b-span point[^>]*>([^<]*)/i],[replaceTagsAndSpaces,/"/ig,""]),f={__id:f,__name:k,num:h};__shouldProcess("accounts",f)&&processAccount(e,f,a,g);b.accounts.push(f)}}}}
...
function processDeposits(a,b){var c=AnyBalance.requestPost(baseurl,{SID:a.SID,tic:1,T:"RT_2IC.form",nvgt:1,SCHEMENAME:"DEPOSITSLIST",XACTION:""},g_headers);if(getParam(c,null,null,/<REDIRECT>[\s\S]*?'(NO)DEPOSITS'/i))AnyBalance.trace("У вас нет ни одного вклада");else{var d=getElements(c,/<div[^>]+div-corner div-cards[^>]*>/ig);if(!d.length){var g=getParam(c,null,null,/<REDIRECT>[\s\S]*?DepID=([0-9A-Z]+)/i);if(!g){AnyBalance.trace(c);AnyBalance.trace("Не удалось найти депозиты!");return}d[0]=AnyBalance.requestPost(baseurl,
{SID:a.SID,tic:1,T:"RT_2IC.form",nvgt:1,SCHEMENAME:"DEPOSIT",XACTION:"",DepID:g},g_headers)}AnyBalance.trace("Найдено депозитов: "+d.length);b.deposits=[];for(c=0;c<d.length;c++){var e=getParam(d[c],null,null,/Номер сделки[\s\S]*?<span[^>]+fat[^>]*>([^<]*)/i),f=getParam(d[c],null,null,/Номер сделки[\s\S]*?<span[^>]+fat[^>]*>([^<]*)/i,replaceTagsAndSpaces),h=getParam(d[c],null,null,/<span[^>]+span-actual(?:[^>]*>){4}([\s\S]*?)<\/div>/i,replaceTagsAndSpaces),e={__id:e,__name:h,num:f};__shouldProcess("deposits",
e)&&processDeposit(d[c],e,a,g);b.deposits.push(e)}}}
