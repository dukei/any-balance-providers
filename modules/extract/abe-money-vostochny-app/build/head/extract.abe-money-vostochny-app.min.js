...
function callApi(b,a){a.appver="3.10";a.locale="ru";a.reqId||(a.reqId=Math.ceil(99999*Math.random()));var c=AnyBalance.requestPost(g_baseurl+"mobws/3.0/json/"+b,a,g_headers),d=getJson(c);5==d.response.result&&(c=d.response.message&&Base64.decode(d.response.message),d=d.response.object.warnings.map(function(a){return Base64.decode(a.message)}).join("\n"),AnyBalance.trace(b+" warnings: "+d),c=AnyBalance.requestPost(g_baseurl+"mobws/3.0/json/"+b,joinObjects(a,{sure:!0}),g_headers),d=getJson(c));if(0!=
d.response.result&&1!=d.response.result)throw AnyBalance.trace(c),c=d.response.message&&Base64.decode(d.response.message),new AnyBalance.Error(c||"Ошибка выполнения запроса "+b,null,/парол/i.test(c));return d.response}
