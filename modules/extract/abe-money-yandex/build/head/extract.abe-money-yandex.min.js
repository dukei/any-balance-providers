...
if(g.result.nextStep)return g.result};AnyBalance.trace("Logging in anew");a=AnyBalance.requestGet(baseurl+"yooid/signin?origin=Wallet&returnUrl=https%3A%2F%2Fyoomoney.ru%2F",g_headers);g_csrf=getParam(a,/__secretKey__="([^"]*)/);if(!g_csrf)throw AnyBalance.trace(a),new AnyBalance.Error("Не удаётся найти информацию для входа. Сайт изменен?");c=getJsonObject(a,/__data__=/);a=callApi("yooid/signin/api/process/start/standard",{origin:"Wallet",tmxSessionId:"groupib-"+c.staticData.antifraudProfiler.sessionId,
isAutoStart:!1,login:b.login});var e=0;do var f=callApiProgress("yooid/signin/api/process/start/standard",{origin:"Wallet",tmxSessionId:"groupib-"+c.staticData.antifraudProfiler.sessionId,isAutoStart:!1,login:b.login,processId:a.result.processId,loginType:a.result.loginType});while(!f.result.isLoginSet&&5>++e);if(!f.result.isLoginSet)throw AnyBalance.trace(JSON.stringify(f)),new AnyBalance.Error("Не удалось войти в личный кабинет. Сайт изменен?");for(b=f;b.result&&d(b);)b=processStep(a.result,d(b));
a=AnyBalance.requestGet(baseurl)}g_savedData.setCookies();g_savedData.save();return a}function callApiProgress(b,a){do{c&&AnyBalance.sleep(1E3);var c=callApi(b,a);AnyBalance.trace(b+": "+JSON.stringify(c))}while("progress"===c.status);if((b=c.result&&c.result.status)&&!/^(SUCCESS|ok)$/i.test(b))throw c=c.result.message||c.result.status,new AnyBalance.Error(c,null,/ACCOUNT|PASSWORD/i.test(c));return c}
