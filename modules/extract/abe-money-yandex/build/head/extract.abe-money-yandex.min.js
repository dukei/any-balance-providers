...
getParam(e.bonus.availableAmount,a,"bonus",null,null,parseBalance):(b=AnyBalance.requestGet(baseurl+"loyalty",g_headers),e=getJsonObject(b,/window.__data\s*?=/),getParam(e.bonusBalance,a,"bonus",null,null,parseBalance));try{b=AnyBalance.requestGet(baseurl+"cards",g_headers);var f=getJsonObject(b,/window.__data__\s*?=/).userCards;if(f&&0<f.length){card=f[0];getParam(card.panFragment.panLast4,a,"cardNumber",/\d{4}$/,[/(\d{4})/,"**** $1"]);getParam(g_system[card.paymentSystem]||card.paymentSystem,a,
"paymentSystem");getParam(g_type[card.media]||card.media,a,"cardType");var g=getParam(card.expirationDate,null,null,null,null,parseDateISO);if(g){a.cardDate=g;var k=Math.ceil((g-(new Date).getTime())/86400/1E3);0<=k?a.cardDays=k:(AnyBalance.trace("Дата деактивации уже наступила"),a.cardDays=0)}else AnyBalance.trace("Не удалось получить дату действия карты")}}catch(h){AnyBalance.trace("Ошибка получения информации по картам"+h.message)}d&&d.user?(getParam(d.user.accountId,a,"number"),getParam(d.user.accountId,
