...
0;c<this.options.additionalRequestHeaders.length;++c){var h=this.options.additionalRequestHeaders[c];if(!h.url||h.url.test(g.url)){var m=this.headerMatchCounts;if(!h.maxCount||(m[c]||0)<h.maxCount){f=h.headers||{};h.maxCount&&(m[c]=(m[c]||0)+1);break}}}p=AnyBalance.requestPost(g.url,g.body,addHeaders(f,p),{HTTP_METHOD:g.method,options:{FORCE_CHARSET:e}});f=AnyBalance.getLastResponseParameters();c={};h=void 0;for(m=0;m<f.headers.length;++m){var n=f.headers[m],k=n[0].toLowerCase();0<=["transfer-encoding",
"content-encoding"].indexOf(k)||("content-length"===k&&(n[1]=""+unescape(encodeURIComponent(p||"")).length),void 0===c[k]?c[k]=n[1]:Array.isArray(c[k])?c[k].push(n[1]):c[k]+=[n[1]],"content-type"===k&&(h=n[1]))}g={id:g.id,page:a,r:{status:AnyBalance.getLastStatusCode(),headers:c,charset:e,contentType:h,body:p}};this.requestAPI("base/response",g)}else if("load"===l.loadStatus)break}while(15>b);if("load"!==l.loadStatus)throw new AnyBalance.Error("Waiting for page "+a+" loading timeout!");};d.prototype.content=
function(a){return this.requestAPI("base/content?page="+a)};d.prototype.cookies=function(a,b){b&&(Array.isArray(b)||(b=[b]));return this.requestAPI("base/cookies",{page:a,urls:b}).cookies};d.prototype.close=function(a){return this.requestAPI("base/close",{page:a})};d.useCookies=function(a){for(var b=0;b<a.length;++b){var e=a[b];AnyBalance.setCookie(e.domain,e.name,e.value,e)}};return d}();
