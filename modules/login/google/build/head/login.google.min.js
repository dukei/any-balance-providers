...
if(!b)throw AnyBalance.trace(a),new AnyBalance.Error("Can`t find find login form, is site changed?");if(/<input[^>]+name="Email"(?:[^<](?!readonly))*>/i.test(b)&&(AnyBalance.trace("found login form, proceeding"),a=createFormParams(b,function(a,b,c,d){return"Email"==c?e.login:d}),a=AnyBalance.requestPost("https://accounts.google.com/AccountLoginInfo",a,addHeaders({Referer:AnyBalance.getLastUrl()})),b=getElement(a,/<form[^>]+gaia_loginform/i),!b))throw AnyBalance.trace(a),new AnyBalance.Error("Can`t find find password form, is site changed?");
if(/<input[^>]+name="Passwd"/i.test(b))AnyBalance.trace("found password form, proceeding"),a=createFormParams(b,function(a,b,c,d){return"Passwd"==c?e.password:d}),a=AnyBalance.requestPost("https://accounts.google.com/signin/challenge/sl/password",a,addHeaders({Referer:AnyBalance.getLastUrl()}));else throw AnyBalance.trace(a),new AnyBalance.Error("Can`t find find password in form, is site changed?");b=getElement(a,/<ol[^>]+challengePickerList/i);!b&&e.__dbg_test_list&&(AnyBalance.trace("Moving to a list of available challenges"),
a=getElement(a,/<form[^>]+challenge\/skip/i),a=createFormParams(a),a=AnyBalance.requestPost("https://accounts.google.com/signin/challenge/skip",a,addHeaders({Referer:AnyBalance.getLastUrl()})),b=getElement(a,/<ol[^>]+challengePickerList/i));if(b){AnyBalance.trace("Google presented challenges list...");b=getElements(b,/<form[^>]+challenge/ig);AnyBalance.trace("Found "+b.length+" possible challenges");for(var d=0;d<b.length;++d){var f=b[d],h=getElement(f,/<button[^>]+submit/i,replaceTagsAndSpaces),
