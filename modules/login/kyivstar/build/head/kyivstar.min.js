...
function loginSite(a){var d=AnyBalance.getPreferences();initializeLogin();AnyBalance.trace("Логин на сайт.");var c=a;a="https://my.kyivstar.ua/";AnyBalance.trace("Соединение с "+a);var b=AnyBalance.requestGet(a+"tbmb/disclaimer/show.do",g_headers);if(!b||400<AnyBalance.getLastStatusCode())throw AnyBalance.trace(b),new AnyBalance.Error("Ошибка при подключении к сайту провайдера! Попробуйте обновить данные позже.");AnyBalance.trace("Успешное соединение.");isLoggedIn(b)&&(AnyBalance.trace("Уже в системе."),
0>b.indexOf(d.login)&&(AnyBalance.trace("Не тот аккаунт, выход."),AnyBalance.requestGet(a+"tbmb/logout/perform.do",g_headers),AnyBalance.requestGet(a+"tbmb/service/logout.do",g_headers),AnyBalance.trace("Переход на страницу входа."),b=AnyBalance.requestGet(c+"cas/login",g_headers)));isLoggedIn(b)||(b=loginBasic(b));if(!isLoggedIn(b)){if(a=getParam(b,null,null,/<td[^>]+class="(?:redError|casError)"[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(a,null,/Перевірте правильність введення логіну|введіть правильний пароль/i.test(a));
