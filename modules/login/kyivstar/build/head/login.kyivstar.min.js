...
function loginSite(a){var d=AnyBalance.getPreferences();initializeLogin();AnyBalance.trace("Логин на сайт.");AnyBalance.trace("Соединение с "+a);var b=AnyBalance.requestGet(a+"tbmb/disclaimer/show.do",g_headers);if(!b||400<AnyBalance.getLastStatusCode())throw AnyBalance.trace(b),new AnyBalance.Error("Ошибка при подключении к сайту провайдера! Попробуйте обновить данные позже.");AnyBalance.trace("Успешное соединение.");isLoggedIn(b)&&(AnyBalance.trace("Уже в системе."),0>b.indexOf(d.login)&&(AnyBalance.trace("Не тот аккаунт, выход."),
b=AnyBalance.requestGet(a+"tbmb/logout/perform.do",g_headers),a=getParam(b,null,null,/<a[^>]+id="submitBtn"[^>]*href="([^"]*)/i,replaceHtmlEntities)))&&(AnyBalance.trace("Переход на страницу входа."),b=AnyBalance.requestGet(a,g_headers));isLoggedIn(b)||(isThereLoginForm(b)||(b=AnyBalance.requestGet("https://account.kyivstar.ua/cas/login?service=http%3A%2F%2Fmy.kyivstar.ua%3A80%2Ftbmb%2Fdisclaimer%2Fshow.do&locale=ua",g_headers)),b=loginBasic(b));if(!isLoggedIn(b)){if(a=getParam(b,null,null,/<td[^>]+class="(?:redError|casError)"[^>]*>([\s\S]*?)<\/td>/i,
replaceTagsAndSpaces))throw new AnyBalance.Error(a,null,/Перевірте правильність введення логіну|введіть правильний пароль/i.test(a));if(/<form[^>]+action="[^"]*perform.do"/i.test(b))throw new AnyBalance.Error("Киевстар показал форму входа без ошибки. Возможно, вы пытаетесь войти в кабинет через мобильный интернет. На стороне Киевстара сейчас с этим проблема. Попробуйте обновить провайдер через вайфай.");AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в систему. Сайт изменен?");}AnyBalance.trace("Успешный вход.");
return b}
