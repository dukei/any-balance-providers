...
function loadAuthorizationPage(a){clearAllCookies();return AnyBalance.requestGet("https://account.kyivstar.ua/cas/login?"+a,g_headers)}
...
g_headers);if(!b||400<AnyBalance.getLastStatusCode())throw AnyBalance.trace(b),new AnyBalance.Error("Ошибка при подключении к сайту провайдера! Попробуйте обновить данные позже.");AnyBalance.trace("Успешное соединение.");isLoggedIn(b)&&(AnyBalance.trace("Уже в системе."),0>b.indexOf(c.login)&&(AnyBalance.trace("Не тот аккаунт, выход."),b=d()));/<a[^>]+new.kyivstar.ua[^>]+ks-button|FMK2.do/i.test(b)&&(AnyBalance.trace("Новый лк или его реклама, переходим на старый"),b=AnyBalance.requestGet("https://account.kyivstar.ua/cas/login?service=http%3A%2F%2Fmy.kyivstar.ua%3A80%2Ftbmb%2FMK2.do",
g_headers));isLoggedIn(b)||(isThereLoginForm(b)||(AnyBalance.trace(b),AnyBalance.trace("Не найдена форма входа. Выходим и явно переходим на авторизацию."),b=d(),AnyBalance.trace("Сейчас мы на "+AnyBalance.getLastUrl()+". Переходим на авторизацию."),b=loadAuthorizationPage("service=http%3A%2F%2Fmy.kyivstar.ua%3A80%2Ftbmb%2Fdisclaimer%2Fshow.do&locale=ua")),b=loginBasic(b));if(!isLoggedIn(b)){if(c=getParam(b,null,null,/<td[^>]+class="(?:redError|casError)"[^>]*>([\s\S]*?)<\/td>/i,replaceTagsAndSpaces))throw new AnyBalance.Error(c,
null,/Перевірте правильність введення логіну|введіть правильний пароль/i.test(c));if(/<form[^>]+action="[^"]*perform.do"/i.test(b))throw new AnyBalance.Error("Киевстар показал форму входа без ошибки. Возможно, вы пытаетесь войти в кабинет через мобильный интернет. На стороне Киевстара сейчас с этим проблема. Попробуйте обновить провайдер через вайфай.");AnyBalance.trace(b);throw new AnyBalance.Error("Не удалось зайти в систему. Сайт изменен?");}AnyBalance.trace("Успешный вход.");return b}
...
function clearAllCookies(){for(var a=AnyBalance.getCookies(),d=0;d<a.length;++d){var c=a[d];AnyBalance.setCookie(c.domain,c.name,null)}};
