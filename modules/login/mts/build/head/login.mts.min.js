...
function enterMtsLK(a){var c=a.url||g_baseurlLogin,b=AnyBalance.requestGet(c,g_headers);500<=AnyBalance.getLastStatusCode()&&(AnyBalance.trace("МТС вернул 500. Пробуем ещё разок..."),b=AnyBalance.requestGet(c,g_headers));if(500<=AnyBalance.getLastStatusCode())throw new AnyBalance.Error("Ошибка на сервере МТС, сервер не смог обработать запрос. Можно попытаться позже...");/Произошла ошибка при попытке авторизации/i.test(b)&&(AnyBalance.trace("Куки протухли :( Придется авторизоваться заново"),clearAllCookies(),
b=AnyBalance.requestGet(c,g_headers));fixCookies()&&(AnyBalance.trace("Куки исправлены, перезагружаем страницу. Пробуем ещё разок..."),b=AnyBalance.requestGet(AnyBalance.getLastUrl(),g_headers));b=redirectIfNeeded(b);if(c=getParam(b,/Продолжить вход[^<]+с номером\s*<b[^>]*>([\s\S]*?)<\/b>/i,[replaceTagsAndSpaces,/\D+/g,""])){AnyBalance.trace("Предлагает автоматически залогиниться на "+c);var b=getElement(b,/<form[^>]+name="Login"/i),d;c&&endsWith(c,a.login)?(AnyBalance.trace("А нам этот номер и нужен. Соглашаемся..."),
d="Login"):(AnyBalance.trace("А нам нужен номер "+a.login+". Отказываемся..."),d="Ignore");b=createFormParams(b,function(a,b,c,g){"IDButton"==c&&(g=d);return g});b=AnyBalance.requestPost(AnyBalance.getLastUrl(),b,addHeaders({Referer:AnyBalance.getLastUrl()}))}isOnLogin()?(a.html=b,a.url||(a.url=AnyBalance.getLastUrl()),b=enterMTS(a)):AnyBalance.trace("Мы не на странице логина. Внутри уже?");if(isOnLogin())throw AnyBalance.trace(b),new AnyBalance.Error("Не удаётся зайти в личный кабинет");return b}
function fixCookies(){for(var a=AnyBalance.getCookies(),c=!1,b=0;b<a.length;++b){var d=a[b];/^login$/i.test(d.name)&&!/^"/.test(d.value)&&(c='"'+d.value+'"',AnyBalance.trace("Исправляем куки "+d.name+" на "+c),AnyBalance.setCookie(d.domain,d.name,c,d),c=!0)}return c}function saveLoginCookies(){var a=AnyBalance.getPreferences();AnyBalance.setData("login",a.login);AnyBalance.saveCookies();AnyBalance.saveData()}
function restoreLoginCookies(){var a=AnyBalance.getPreferences();AnyBalance.getData("login")==a.login&&AnyBalance.restoreCookies()}
function enterMTS(a){var c=a.baseurl||g_baseurl,b=a.url||g_baseurlLogin+"/amserver/UI/Login?goto="+c,d=a.allowRetry,e=a.html;fixCookies();if(!e||!/<form[^>]+name="Login"/i.test(e))if(e=AnyBalance.requestGet(b,g_headers),500<=AnyBalance.getLastStatusCode()&&(AnyBalance.trace("МТС вернул 500. Пробуем ещё разок..."),e=AnyBalance.requestGet(b,g_headers)),500<=AnyBalance.getLastStatusCode())throw new AnyBalance.Error("Ошибка на сервере МТС, сервер не смог обработать запрос. Можно попытаться позже...",d);
if(0==AnyBalance.getLastUrl().indexOf(c))return e;var e=redirectIfNeeded(e),f=getParam(e,null,null,/<form[^>]+name="Login"[^>]*>([\s\S]*?)<\/form>/i);if(!f){if(!e)throw new AnyBalance.Error("Личный кабинет МТС временно недоступен. Попробуйте ещё раз позже");if(/<h1[^>]*>\s*Request Error/i.test(e))throw new AnyBalance.Error("Личный кабинет МТС временно не работает. Попробуйте ещё раз позже");AnyBalance.trace(e);throw new AnyBalance.Error("Не удаётся найти форму входа! Сайт изменен?",d);}c=createFormParams(f,
