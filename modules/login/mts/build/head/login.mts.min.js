...
function enterMTS(a){var c=a.baseurl||g_baseurl,d=a.url||g_baseurlLogin+"/amserver/UI/Login?service="+(a.service||"lk")+"&goto="+c+"/",e=a.allowRetry,b=a.html;if(!b||!/<form[^>]+name="Login"/i.test(b))if(b=AnyBalance.requestGet(d,g_headers),500<=AnyBalance.getLastStatusCode()&&(AnyBalance.trace("МТС вернул 500. Пробуем ещё разок..."),b=AnyBalance.requestGet(d,g_headers)),500<=AnyBalance.getLastStatusCode())throw new AnyBalance.Error("Ошибка на сервере МТС, сервер не смог обработать запрос. Можно попытаться позже...",e);
if(0==AnyBalance.getLastUrl().indexOf(c))return b;b=redirectIfNeeded(b);c=getParam(b,null,null,/<form[^>]+name="Login"[^>]*>([\s\S]*?)<\/form>/i);if(!c){if(!b)throw new AnyBalance.Error("Личный кабинет МТС временно недоступен. Попробуйте ещё раз позже");if(/<h1[^>]*>\s*Request Error/i.test(b))throw new AnyBalance.Error("Личный кабинет МТС временно не работает. Попробуйте ещё раз позже");AnyBalance.trace(b);throw new AnyBalance.Error("Не удаётся найти форму входа! Сайт изменен?",e);}c=createFormParams(c,
function(b,c,d,e){"IDToken1"==d?e=a.login:"IDToken2"==d?e=a.password:"noscript"==d?e=void 0:"IDButton"==d&&(e="Submit");return e});AnyBalance.trace("Логинимся с заданным номером");b=AnyBalance.requestPost(d,c,addHeaders({Origin:g_baseurlLogin,Referer:d}));500<=AnyBalance.getLastStatusCode()&&(AnyBalance.trace("МТС вернул 500 при попытке логина. Пробуем ещё разок..."),b=AnyBalance.requestPost(d,c,addHeaders({Origin:g_baseurlLogin,Referer:d})));if(500<=AnyBalance.getLastStatusCode())throw new AnyBalance.Error("Ошибка на сервере МТС при попытке зайти, сервер не смог обработать запрос! Можно попытаться позже...",
e);0==AnyBalance.getLastUrl().indexOf(g_baseurlLogin)&&(b=checkLoginError(b,d));if(0==AnyBalance.getLastUrl().indexOf(g_baseurlLogin))throw AnyBalance.trace(b),new AnyBalance.Error("Не удаётся зайти в личный кабинет. Он изменился или проблемы на сайте.",e);return b};
