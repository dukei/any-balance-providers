...
d);var c=getElement(c,/<form[^>]+name="Login"/i),e;endsWith(d.replace(/\D+/g,""),a.login)?(AnyBalance.trace("А нам этот номер и нужен. Соглашаемся..."),e="Login"):(AnyBalance.trace("А нам нужен номер "+a.login+". Отказываемся..."),e="Ignore");c=createFormParams(c,function(a,c,d,g){"IDButton"==d&&(g=e);return g});c=AnyBalance.requestPost(AnyBalance.getLastUrl(),c,addHeaders({Referer:AnyBalance.getLastUrl()}));fixCookies()}isOnLogin()?(a.html=c,a.url||(a.url=AnyBalance.getLastUrl()),c=enterMTS(a)):
...
e);isOnLogin()&&(b=checkLoginError(b,c));if(isOnLogin())throw AnyBalance.trace(b),new AnyBalance.Error("Не удаётся зайти в личный кабинет. Он изменился или проблемы на сайте.",e);return b};
