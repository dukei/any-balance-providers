...
try{
	...
	AnyBalance.trace('Токен устарел. Нужна авторзация');
	var token='';
...
if (!token){
	...
	AnyBalance.trace('Начинаем авторизацию');

//var html=AnyBalance.requestGet('https://www.aeroflot.ru/', g_headers);
//var url=getParam(html,/  src="(\/[^"]*)"/);
//AnyBalance.trace('Ссылка на вход '+(url?'':'не')+' найдена на ' + AnyBalance.getLastUrl());
//AnyBalance.trace('url='+url);
//var html=AnyBalance.requestGet('https://www.aeroflot.ru/aac/app/ru-ru/login', g_headers);
//var url1=getParam(html,/  src="(\/[^"]*)"/);
//AnyBalance.trace('Ссылка на вход '+(url1?'':'не')+' найдена на ' + AnyBalance.getLastUrl());
//AnyBalance.trace('url='+url);
//if (!url&&url1) url=url1;
//var html=AnyBalance.requestGet('https://www.aeroflot.ru/aac/app/ru-ru/login?client_id='+CLIENT_ID+'&nonce='+Math.random().toString(),+'&redirect_uri=https%3A%2F%2Fwww.aeroflot.ru%2Fru-ru&response_type=code%20id_token%20token&scope=openid%20user-loyalty-profile%20personal-cabinet%20feedback', g_headers);
//var url1=getParam(html,/  src="(\/[^"]*)"/);
//AnyBalance.trace('Ссылка на вход '+(url1?'':'не')+' найдена на ' + AnyBalance.getLastUrl());
//AnyBalance.trace('url='+url);
//if (!url&&url1) url='https://gw.aeroflot.ru/api/pr/AAC/Authorization/v1/get';

//if (!url) {
//	AnyBalance.trace(html);
//        throw new AnyBalance.Error('Ссылка на вход не найдена');
//	}
...
var params={
    ...
    "data": {
        ...
        "tfa": {
            "fingerprint": "65f937e9e6822cf3249efa338ea4edcd",
...
AnyBalance.saveData();
	...
	getParam(tier_level[json.data.loyaltyInfo.tierLevel]||json.data.loyaltyInfo.tierLevel, result, 'level');
	getParam(json.data.loyaltyInfo.loyaltyId , result, '__tariff');
	getParam(json.data.loyaltyInfo.loyaltyId, result, 'partnum');
...
function callAPI(location, params) {
	AnyBalance.trace('Запрос к '+location);
