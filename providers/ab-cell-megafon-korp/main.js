    ...
    if(!/accountInfo/i.test(AnyBalance.getLastUrl())){
        ...
        for(var i=0; i<json.account.length; ++i){
        	...
        	if(!accnum || acc.number.endsWith(accnum)){
        		...
        		getParam(acc.conditionBalance.value, result, 'cond_balance');
    	...
    	getParam(html, result, 'cond_balance', /<dt[^>]*>\s*Текущий условный баланс[\s\S]*?class="money[^>]*>([\s\S]*?)<span/i, replaceTagsAndSpaces, parseBalance);
