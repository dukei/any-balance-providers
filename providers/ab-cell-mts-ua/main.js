...
function main(){
    ...
    sumParam (html, result, 'min_net_70', /<li>(?:70|200) минут в день для внутрисетевых звонков:[^<]*осталось\s*([\d\.,]+) бесплатных секунд<\/li>/ig, replaceTagsAndSpaces, parseBalance, aggregate_sum);
    ...
    // 3000/6000 региональных минут в сети
    sumParam (html, result, 'min_reg_3000', /<li>(?:3|6)000 региональных минут в сети:[^<]*осталось\s*([^<]*)/ig, replaceTagsAndSpaces, parseBalance, parseTime, aggregate_sum);
    ...
    sumParam (html, result, 'min_all1', /<li>\d+\s*Хвилин на інших операторів по Україні \(RED .\), осталось:\s*([\d\.,]+)\s*мин.<\/li>/ig, replaceTagsAndSpaces, parseTime, aggregate_sum);
    ...
    // СМС С услугой «Супер 3D Ноль» и "Смарфон 0.50" "Команда 3D"
    ...
    sumParam (html, result, 'mms_used', /<li>MMS по Украине по услуге "Супер МТС Команда 3D", израсходовано:\s*(\d+)\s*(?:mms|ммс)/ig, replaceTagsAndSpaces, parseBalance, aggregate_sum);
    ...
    // Интернет Vodafone
    sumParam (html, result, 'traffic_used', /<li>\d+ (?:Г|М)Б 3G в месяц RED ., использовано:\s*(\d+,?\d* *(Кб|kb|mb|gb|кб|мб|гб|байт|bytes)).<\/li>/ig, null, parseTraffic, aggregate_sum);
    ...
    sumParam (html, result, 'traffic_used2', /<li>Передача даних у Роумінгу, використано:\s*(\d+,?\d* *(Кб|Мб|Гб|kb|mb|gb|кб|мб|гб|байт|bytes)).<\/li>/ig, null, parseTraffic, aggregate_sum);
    ...
    sumParam (html, result, 'min_used', /<li>Минуты в сети МТС по услуге "Супер МТС Команда 3D", израсходовано\s*([\d\.,]+) сек.<\/li>/ig, replaceTagsAndSpaces, parseBalance, aggregate_sum);
    ...
    sumParam (html, result, 'PZS_first_russia', /<li>Плата за первое GPRS событие в России:\s*([\d\.,]+)<\/li>/ig, replaceTagsAndSpaces, parseBalance, aggregate_sum);
