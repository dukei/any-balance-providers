...
var LKs = {
    dnevnik: loginDnevnikRU,
    school:  loginSchoolMosreg
};
...
function main() {
    ...
    AB.checkEmpty(prefs.type,     'Выберите тип личного кабинета!');

    if(!LKs[lk])
        throw new AnyBalance.Error("ЛК не найден.");

    var func = LKs[lk];
    AnyBalance.trace('Личный кабинет: ' + lk);
...
function loginDnevnikRU(result) {
    var prefs         = AnyBalance.getPreferences(),
        baseurl       = 'https://dnevnik.ru/',
        baseurl_login = 'https://login.dnevnik.ru/';

    AnyBalance.setDefaultCharset('utf-8');

    var html = AnyBalance.requestGet(baseurl_login, g_headers);
    ...
    html = AnyBalance.requestPost(baseurl_login + 'login', params, addHeaders({
        Referer: baseurl_login + 'login'
    }));
    ...
    return getInfo(html, result);
}

function loginSchoolMosreg(result) {
    var prefs         = AnyBalance.getPreferences(),
    ...
    html = AnyBalance.requestPost(baseurl_login + 'user/login', {
        login:    prefs.login,
