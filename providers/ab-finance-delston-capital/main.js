...
function main(){
	...
	var params = createFormParams(form, function(params, str, name, value){
		if(name == 'login') return prefs.login;
		if(name == 'password') return prefs.password;
		if(name == 'salt2') return randstr(5);
		return value;
	});

	var serialize = [];
	for(var i in params) serialize.push( i + '=' + encodeURIComponent(params[i]) );
	...
	if(html.success) {
		// тут возможно надо https,
		// а возможно действительно баг хрома
		html = AnyBalance.requestGet('http://delston-capital.com/ru/cabinet/', g_headers);
	} else {
		AnyBalance.trace(html);
	...
	html = getElements(html, /<td[^>]*>/gi);
	
	getParam(html[0], result, 'name', null, replaceTagsAndSpaces);
