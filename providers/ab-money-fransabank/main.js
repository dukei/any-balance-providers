...
function main(){
    ...
    var baseurl = "https://ibank.fransabank.by/v1/cgi/bsi.dll?";
	...
	var sid = getParam(html, null, null, /SID[^"]*"([^"]*)/i);
	var usr = getParam(html, null, null, /USR[^"]*"([^"]*)/i);
...
function fetchCard(jsonInfo, baseurl){
	...
	// (\d{4}X{4,}8834(?:[^>]*>){15})
	var re = new RegExp('(\\d{4}X{4,}' + cardnum + '(?:[^>]*>){15})', 'i');
	...
	getParam(tr, result, 'cardnum', /(\d{4}X{4,}\d{4})/i, replaceTagsAndSpaces);
	getParam(tr, result, '__tariff', /(\d{4}X{4,}\d{4})/i, replaceTagsAndSpaces);
	getParam(tr, result, 'balance', /(?:[\s\S]*?<TD[^>]*>){1}([\s\S]*?)<\//i, replaceTagsAndSpaces, parseBalance);
	getParam(tr, result, 'limit', /(?:[\s\S]*?<TD[^>]*>){2}([\s\S]*?)<\//i, replaceTagsAndSpaces, parseBalance);
	getParam(tr, result, ['currency', 'balance'], /(?:[\s\S]*?<TD[^>]*>){3}([\s\S]*?)<\//i, replaceTagsAndSpaces);
	getParam(tr, result, 'type', /(?:[\s\S]*?<TD[^>]*>){4}([\s\S]*?)<\//i, replaceTagsAndSpaces);
