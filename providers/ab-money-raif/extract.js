...
function login(prefs, result) {
	...
	function html_encode(str) {return str.replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;");}
	...
	getParam(html, result.profile, 'profile.passportnumber', /<passportnumber>([\s\S]*?)<\/passportnumber>/i, replaceTagsAndSpaces, html_entity_decode);
	getParam(html, result.profile, 'profile.passportissuer', /<passportissuer>([\s\S]*?)<\/passportissuer>/i, replaceTagsAndSpaces, html_entity_decode);
	...
	getParam(html, result.profile, 'profile.login', /<login>([\s\S]*?)<\/login>/i, replaceTagsAndSpaces, html_entity_decode);
...
function processCard(info, result) {
	...
	getParam(info, result, 'cards.state', /<state>([\s\S]*?)<\/state>/i, replaceTagsAndSpaces, html_entity_decode);
	getParam(info, result, 'cards.cardName', /<cardName>([\s\S]*?)<\/cardName>/i, replaceTagsAndSpaces, function(str) {return html_entity_decode(str) + ' (' + getParam(info, null, null, /<number>([\s\S]*?)<\/number>/i, [replaceTagsAndSpaces, /^[\s\S]*?(\d{4})$/, '$1']) + ')';});
	getParam(info, result, 'cards.holderName', /<holderName>([\s\S]*?)<\/holderName>/i, replaceTagsAndSpaces, html_entity_decode);
...
function processAccount(info, result) {
    ...
    getParam(info, result, 'accounts.accnum', /<number>([\s\S]*?)<\/number>/i, replaceTagsAndSpaces, html_entity_decode);
	...
	getParam(info, result, 'accounts.accountSubtypeName', /<accountSubtypeName>([\s\S]*?)<\/accountSubtypeName>/i, replaceTagsAndSpaces, html_entity_decode);
	getParam(info, result, 'accounts.type', /<type>([\s\S]*?)<\/type>/i, replaceTagsAndSpaces, html_entity_decode);
	getParam(info, result, 'accounts.region', /<region>([\s\S]*?)<\/region>/i, replaceTagsAndSpaces, html_entity_decode);
...
function processDeposit(info, result){
    getParam(info, result, 'deposits.accnum', /<accountNumber>([\s\S]*?)<\/accountNumber>/i, replaceTagsAndSpaces, html_entity_decode);
    ...
    getParam(info, result, 'deposits.name', /<names>([\s\S]*?)<\/names>/i, replaceTagsAndSpaces, html_entity_decode);            
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Кредиты
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
