...
function initDetailsUrls(html){
    g_url.detailsCard = getParam(html, null, null, /"([^"]*card-detail\?[^"]*\{\{id\}\}[^"]*)/, null, html_entity_decode);
    g_url.detailsDep = getParam(html, null, null, /"([^"]*deposit-details\?[^"]*\{\{id\}\}[^"]*)/, null, html_entity_decode);
    g_url.detailsAcc = getParam(html, null, null, /"([^"]*current-account-details\?[^"]*\{\{id\}\}[^"]*)/, null, html_entity_decode);
    g_url.detailsLoan = getParam(html, null, null, /"([^"]*loan-detail\?[^"]*\{\{id\}\}[^"]*)/, null, html_entity_decode);
}
...
function processCard(card, result){
    ...
    if(g_url.detailsCard && AnyBalance.isAvailable('cards.minpay', 'cards.minpaytill', 'cards.limit', 'cards.userName', 'cards.own', 'cards.accnum', 'cards.contract', 'cards.cash', 'cards.blocked', 'cards.type_product')){
    	...
    	getParam(html, result, 'cards.userName', /<span[^>]+class="[^>]*fio[^>]*>([\s\S]*?)<\/span>/i, replaceTagsAndSpaces, html_entity_decode);
    	getParam(html, result, 'cards.accnum', /<div[^>]+class="cell[^>]*>Номер счета[\s\S]*?<div[^>]+class="cell[^>]*>([\s\S]*?)<\/div>/i, replaceTagsAndSpaces, html_entity_decode);
    	getParam(html, result, 'cards.contract', /<div[^>]+class="cell[^>]*>Номер договора[\s\S]*?<div[^>]+class="cell[^>]*>([\s\S]*?)<\/div>/i, replaceTagsAndSpaces, html_entity_decode);
        getParam(html, result, 'cards.type_product', /<div[^>]+class="cell[^>]*>Тип продукта[\s\S]*?<div[^>]+class="cell[^>]*>([\s\S]*?)<\/div>/i, replaceTagsAndSpaces, html_entity_decode);
    }
...
function processInfo(html, result){
    ...
    getParam(html, info, 'info.fio', /<span[^>]+fio[^>]*>([^]*?)<\/span>/i, replaceTagsAndSpaces, html_entity_decode);
    ...
    if(AnyBalance.isAvailable('info.name_last', 'info.name', 'info.name_patronymic', 'info.hphone', 'info.mphone', 'info.wphone', 'info.email')) {
        ...
        getParam(html, info, 'info.name_last', />\s*Фамилия[^]*?<td[^>]*>([^]*?)<\/td>/i, replaceTagsAndSpaces, html_entity_decode);
        getParam(html, info, 'info.name', />\s*Имя[^]*?<td[^>]*>([^]*?)<\/td>/i, replaceTagsAndSpaces, html_entity_decode);
        getParam(html, info, 'info.name_patronymic', />\s*Отчество[^]*?<td[^>]*>([^]*?)<\/td>/i, replaceTagsAndSpaces, html_entity_decode);
        getParam(html, info, 'info.hphone', /<td[^>]*>\s*Домашний[^]*?<td[^>]*>([^]*?)<\/td>/i, replaceTagsAndSpaces, html_entity_decode); //+7000*****00
        getParam(html, info, 'info.mphone', /<td[^>]*>\s*Мобильный[^]*?<td[^>]*>([^]*?)<\/td>/i, replaceTagsAndSpaces, html_entity_decode); //+7905*****42
        getParam(html, info, 'info.wphone', /<td[^>]*>\s*Рабочий[^]*?<td[^>]*>([^]*?)<\/td>/i, replaceTagsAndSpaces, html_entity_decode); //+7861*****25
        getParam(html, info, 'info.email', /<td[^>]*>\s*Email[^]*?<td[^>]*>([^]*?)<\/td>/i, replaceTagsAndSpaces, html_entity_decode); //K*********@MAIL.RU
    }
}
