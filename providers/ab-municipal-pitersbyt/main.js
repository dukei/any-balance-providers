...
function main(){
    ...
    try {
        html = getBalance(baseurl, cfg);
        gwtGetJSON(html);
    }
    catch(e) {
        postLogin(baseurl, cfg);
        html = getBalance(baseurl, cfg);
    }
...
function postLogin(url, cfg) {
    //Скачиваем скрипт для поиска $strongName
    var html = AnyBalance.requestGet(url + cfg.auth_nocache),
        prefs = AnyBalance.getPreferences();

    var gwtAuthHeaders = {
        'Content-Type': 'text/x-gwt-rpc; charset=UTF-8',
        'X-GWT-Module-Base': url + cfg.auth_url,
        'X-GWT-Permutation': gwtGetStrongName(html, cfg)
    };

    try{
        html = AnyBalance.requestPost(url + cfg.auth_file,
...
function getBalance(url, cfg) {
    //Скачиваем новый скрипт для поиска $strongName
    var html = AnyBalance.requestGet(url + cfg.user_nocache);
    ...
    return AnyBalance.requestPost(url + cfg.user_file,
        makeReplaces(cfg.user_data, cfg),
        {
            'Content-Type': 'text/x-gwt-rpc; charset=UTF-8',
            'X-GWT-Module-Base': url + cfg.user_url,
