...
function main(){
	...
	var baseurl = "https://www.browsercalls.com/";
	
    AnyBalance.setDefaultCharset('utf-8'); 
	
    var html = AnyBalance.requestGet(baseurl + 'login', g_headers);
	
	var form = getParam(html, null, null, /<form[\s]action="https:\/\/www\.browsercalls\.com\/login" method="post" class="form-detail">([\s\S]*?)<\/form>/i);
    if(!form)
	...
	// т.к. генерируется много форм, чтобы запутать нас, мы возьмем нужные данные из скрипта
    var matches = /<input type="hidden"[^>]+id="([^"]+)" name="([^"]+)" value="([^"]+)"\s*\/>\s*<\/form>/i.exec(html);
    AnyBalance.trace(matches);
    if(!matches)
        throw new AnyBalance.Error("Can`t find session id!");

	params[matches[7]] = matches[8];
