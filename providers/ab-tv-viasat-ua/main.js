...
function main(){
	...
	// Нас бросили не пойми куда, надо вернуться в кабинет
	if(!/<title>\s*Viasat\s*<\/title>/i.test(html)) {
		AnyBalance.trace('Нас бросили не пойми куда, надо вернуться в кабинет');
		html = AnyBalance.requestGet(baseurl, g_headers);
	}
	
	if (!/action_exit/i.test(html)) {
		var error = sumParam(html, null, null, /class="error_form"[^>]*>([\s\S]*?)<\//ig, replaceTagsAndSpaces, html_entity_decode, aggregate_join);
		if (error)
