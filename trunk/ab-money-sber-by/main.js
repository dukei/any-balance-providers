...
function fetchCard(jsonInfo, baseurl){
	...
	getParam(html, result, 'cardnum', /ToSTM\('([\s\S]*?)'/i, replaceTagsAndSpaces);
	getParam(html, result, '__tariff', /ToSTM\('([\s\S]*?)'/i, replaceTagsAndSpaces);
	getParam(html, result, ['currency', 'balance'], /Обновление остатка по карточке[\s\S]*?обновить[\s\S]*?<TD>([\s\S]*?)<\/TD>/i, replaceTagsAndSpaces);
	
	var card = getParam(html, null, null, /GetRest[\s\S]*?(\d+)/i, null);
	var curr = getParam(html, null, null, /GetRest[\s\S]*?(\d+)\)/i, null);
